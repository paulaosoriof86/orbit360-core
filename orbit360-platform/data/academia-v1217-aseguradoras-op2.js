...
Orbit.ACADEMIA_V1223_ASEGURADORAS = (function () {
  ...
  function lesson(role) {
    return {
      t:'Cierre operativo OP-2', min:26, tipo:'lectura', _op2v:1223,
      secciones:[
        ...
        section('🛠','Defecto funcional, contrato o validador','Si dos componentes interpretan de forma distinta una confirmación, se corrige el contrato compartido y su prueba de integración. Si el producto ya cumple pero una prueba exige el comportamiento retirado, se actualiza el validador antes de volver a ejecutar.'),
        section('🗂','Acción por rol', role === 'Asesor'
          ...
          : role === 'Operativo'
            ? 'Revisa el diff, mantiene contactos y recursos operativos según permisos y valida que cada acceso muestre un estado real. No repite una importación ya confirmada ni habilita tarifas por cargar un directorio.'
            : 'Confirma país, identidad, retenidos, trazabilidad, motivo y resultado de lectura posterior. Si aparece una segunda revisión sobre el mismo lote, detiene la operación y reporta el defecto.'),
  ...
  function quiz(role) {
    return {
      t:'Evaluación OP-2', min:15, tipo:'quiz', _op2v:1223,
      preguntas:[
  ...
  function applyOne(role) {
    ...
    next.desc = 'Importar y utilizar el directorio con fuentes separadas, diff, una sola escritura confirmada, verificación, recursos protegidos y estados honestos.';
