...
Orbit.ACADEMIA_V1218_ASEGURADORAS = (function () {
  ...
  function lesson(role) {
    return {
      t:'Cierre operativo OP-2', min:16, tipo:'lectura', _op2v:1218,
      secciones:[
  ...
  function quiz(role) {
    return {
      t:'Evaluación OP-2', min:10, tipo:'quiz', _op2v:1218,
      preguntas:[
  ...
  function applyOne(role) {
    ...
    next.lecciones = (next.lecciones || []).filter(x => x && x._op2v !== 1217 && x._op2v !== 1218);
    next.lecciones.push(lesson(role), quiz(role));
    next.desc = 'Utilizar el directorio operativo con cuentas visibles, credenciales por rol, recursos protegidos y trazabilidad.';
    next.metaLeccion = Object.assign({}, next.metaLeccion || {}, {
      ...
      seguridad:'Cuentas bancarias visibles para usuarios del directorio; credenciales de portales para Dirección/Admin/Operativo; migración no destructiva.'
    });
    next._cv = 1218;
