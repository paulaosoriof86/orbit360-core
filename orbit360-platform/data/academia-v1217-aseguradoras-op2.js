/* ============================================================
   Orbit 360 · Academia profunda Aseguradoras OP-2 v1.222
...
Orbit.ACADEMIA_V1220_ASEGURADORAS = (function () {
  ...
  function lesson(role) {
    return {
      t:'Cierre operativo OP-2', min:24, tipo:'lectura', _op2v:1222,
      secciones:[
        ...
        section('🏦','Cuentas bancarias con estado honesto','Los números bancarios completos se envían al proveedor protegido y la ficha conserva únicamente accountRef y una terminación enmascarada. Una cuenta solo se declara disponible después de confirmar escritura y lectura protegida.'),
  ...
  function quiz(role) {
    return {
      t:'Evaluación OP-2', min:14, tipo:'quiz', _op2v:1222,
      preguntas:[
  ...
  function applyOne(role) {
    ...
    next.lecciones = (next.lecciones || []).filter(x => x && ![1217,1218,1219,1220,1221,1222].includes(x._op2v));
  ...
  return { version:'1.222', COURSE_IDS, apply, directPlatformImport:true, readAfterWriteRequired:true, protectedAccountsHonestState:true };
})();
