/* Orbit 360 · Academia · API 1.221 · contenido M1 1.226 */
(function(){
  'use strict';
  window.Orbit=window.Orbit||{};
  if(Orbit.ACADEMIA_V1221_M1_VISUAL&&Orbit.ACADEMIA_V1221_M1_VISUAL.version==='1.221'&&Orbit.ACADEMIA_V1221_M1_VISUAL.contentVersion==='1.226')return;
  function sec(icon,title,detail){return{icon:icon,t:title,color:'#C5162E',d:detail};}
  function lesson(){return{t:'Estados honestos y operación segura',min:30,tipo:'lectura',_m1visualv:1226,secciones:[
  ...
  function quiz(){return{t:'Evaluación · honestidad operativa M1',min:20,tipo:'quiz',_m1visualv:1226,preguntas:[
  ...
  function update(id){try{var current=Orbit.store&&Orbit.store.get?Orbit.store.get('cursos',id):null;if(!current)return false;var next=JSON.parse(JSON.stringify(current));next.lecciones=(next.lecciones||[]).filter(function(x){return x&&x._m1visualv!==1221&&x._m1visualv!==1222&&x._m1visualv!==1223&&x._m1visualv!==1224&&x._m1visualv!==1225&&x._m1visualv!==1226;});next.lecciones.push(lesson(),quiz());next._cv=Math.max(+next._cv||0,1226);next.progreso=current.progreso||0;next.certificado=!!current.certificado;Orbit.store.update('cursos',id,next);return true;}catch(e){return false;}}
  ...
  Orbit.ACADEMIA_V1221_M1_VISUAL={version:'1.221',contentVersion:'1.226',criticalRelease:'block1-critical-runtime-20260721-4',activeRoleScopeCeiling:true,apply:apply};
})();
