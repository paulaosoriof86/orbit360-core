/* Orbit 360 · Academia · integridad visual M1 v1.221 */
(function(){
  'use strict';
  window.Orbit=window.Orbit||{};
  if(Orbit.ACADEMIA_V1221_M1_VISUAL)return;
  function sec(icon,title,detail){return{icon:icon,t:title,color:'#C5162E',d:detail};}
  function lesson(){return{t:'Estados honestos y revisión visual',min:18,tipo:'lectura',_m1visualv:1221,secciones:[
    ...
    sec('🧪','Defecto y validador','FUNCTIONAL_DEFECT describe una falla del producto; VALIDATOR_STALE describe una prueba que no cubre la experiencia real.')
  ]};}
  function quiz(){return{t:'Evaluación · integridad visual M1',min:10,tipo:'quiz',_m1visualv:1221,preguntas:[
  ...
  function update(id){try{var current=Orbit.store&&Orbit.store.get?Orbit.store.get('cursos',id):null;if(!current)return false;var next=JSON.parse(JSON.stringify(current));next.lecciones=(next.lecciones||[]).filter(function(x){return x&&x._m1visualv!==1221;});next.lecciones.push(lesson(),quiz());next._cv=Math.max(+next._cv||0,1221);next.progreso=current.progreso||0;next.certificado=!!current.certificado;Orbit.store.update('cursos',id,next);return true;}catch(e){return false;}}
  ...
  Orbit.ACADEMIA_V1221_M1_VISUAL={version:'1.221',apply:apply};
})();
