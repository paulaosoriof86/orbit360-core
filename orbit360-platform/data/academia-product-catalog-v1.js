/* Gravicentra Insurance · Academia product catalog V1 · derived from approved Academia PLUS content.
   Pure content only: no store writes, no seed, no timers, no DOM hooks, no backend side effects. */
(function(){
  'use strict';
  ...
  function cleanText(value){
    return String(value==null?'':value)
      .replace(/Orbit Clientes/g,'Cliente 360')
      .replace(/Orbit Ops/g,'Ops')
      .replace(/Orbit Marketing/g,'Marketing')
      .replace(/Orbit IA/g,'IA de Gravicentra Insurance')
      .replace(/Orbit 360/g,'Gravicentra Insurance')
      .replace(/\bOrbit\b/g,'Gravicentra Insurance');
  }
  function sanitize(value){
    if(Array.isArray(value))return value.map(sanitize);
    if(value&&typeof value==='object'){
      var out={};Object.keys(value).forEach(function(k){out[k]=sanitize(value[k]);});return out;
    }
    return typeof value==='string'?cleanText(value):value;
  }
  function visibleBlob(course){try{return JSON.stringify(course);}catch(e){return '';}}
  var FORBIDDEN=/\bLAB\b|\bSHA\b|validator|release gate|release mechanics|bridge owner|seed writer|hardcod/i;
  var safe=cursos
    .filter(function(course){return course&&course.id&&course.titulo&&!FORBIDDEN.test(visibleBlob(course));})
    .map(function(course){return Object.assign({progreso:0,certificado:false,recursos:[],_staticCourse:true,_cv:CONTENT_V},sanitize(course));});
  if(!safe.some(function(c){return c.id==='cur_p_clientes';}))throw new Error('ACADEMIA_PRODUCT_CLIENT360_COURSE_MISSING');
  if(!safe.some(function(c){return c.id==='cur_p_aseg_cotiz';}))throw new Error('ACADEMIA_PRODUCT_INSURER_COURSE_MISSING');
  window.Orbit.academiaProductCatalogV1=Object.freeze({
    version:'20260923.1',
    source:'academia-plus-approved-content-v8',
    automaticWrites:false,
    seed:false,
    lab:false,
    courses:Object.freeze(safe)
  });
})();
