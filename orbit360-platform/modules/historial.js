            ...
            <div style="margin-bottom:18px">
              ...
              <div style="display:grid;gap:9px">
                ${items.map(a => {
                  ...
                  return `<div class="clickable" onclick="location.hash='#/cliente360?c=${a.clienteId}'" style="display:flex;gap:12px;align-items:flex-start;padding:11px 13px;border:1px solid var(--line);border-radius:var(--r-sm);cursor:pointer;background:var(--card)">
                    ...
                    <div style="flex:1;min-width:0">
                      <div style="display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap">
                        ...
                        <span class="muted" style="font-size:11.5px;white-space:nowrap">${U.esc(cli ? cli.nombre : '')}</span>
                      </div>
                      <div class="muted" style="font-size:12.5px;margin-top:3px">${U.esc(a.detalle)}</div>
                      <div style="display:flex;align-items:center;gap:6px;margin-top:6px"><span class="dot-s" style="background:${ase ? ase.color : '#999'}"></span><span class="muted" style="font-size:11px">${U.esc(ase ? ase.nombre : '')}</span></div>
                    </div>
                  </div>`;
                }).join('')}
              </div>
