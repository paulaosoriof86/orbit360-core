...
(function () {
  ...
  var VERSION = 'p0-20260814-required-optional-1';
  var MARKER = 'PRODUCT_HYDRATION_REQUIRED_OPTIONAL_P0';
  ...
  function wrapStore(base, hydration) {
    var baseStatus = base._productStatus.bind(base);
    var baseAttach = base._attachSnapshots.bind(base);
    var baseRaw = base.raw.bind(base);
    var baseAll = base.all.bind(base);
    var baseGet = base.get.bind(base);
    var baseWhere = base.where.bind(base);
    var baseFind = base.find.bind(base);

    function status() {
      var raw = baseStatus() || {};
      var attached = unique(raw.attachedCollections);
      ...
      var requiredMissing = hydration.required.filter(function (name) { return attached.indexOf(name) < 0 && requiredFailed.indexOf(name) < 0; });
      var optionalFailed = hydration.optional.filter(function (name) { return !!errors[name] || denied.indexOf(name) >= 0; });
      var optionalMissing = hydration.optional.filter(function (name) { return attached.indexOf(name) < 0 && optionalFailed.indexOf(name) < 0; });
    ...
    base._attachSnapshots = function () {
      baseAttach();
      var s = status();
      return s.requiredFailed.length === 0 && ['blocked-tenant', 'blocked-no-collections', 'blocked-no-database'].indexOf(s.status) < 0;
    };
    base._productStatus = status;
    base.raw = function () {
      var out = baseRaw() || {};
      out.__backend = status();
      return out;
    };
    ...
    function advisorProjection() {
      ...
      function add(id, name, extra) {
        ...
        if (!id) return;
        var prior = map[id] || {};
        var label = text(name || prior.nombre) || 'Asesor asignado';
        map[id] = Object.assign({}, prior, extra || {}, {
          id: id,
          nombre: label,
          name: label,
          displayName: label,
      ...
      try {
        var active = window.Orbit.auth && window.Orbit.auth.productUser || {};
        if (active.advisorId) add(active.advisorId, active.nombre, { email: active.email, roles: active.roles || [], rol: active.activeRole || active.rol || 'Asesor' });
      } catch (error) {}
      ['clientes', 'polizas', 'cobros', 'recibosEsperados', 'carteraPrimas'].forEach(function (collection) {
        (baseAll(collection) || []).forEach(function (row) {
          add(row && (row.asesorId || row.advisorId || row.vendedorId || row.responsableId), row && (row.asesorNombre || row.advisorName || row.vendedorNombre || row.responsableNombre), {});
        });
      });
    ...
    base.get = function (collection, id) {
      if (collection !== 'asesores') return baseGet(collection, id);
      return advisorProjection().find(function (row) { return rowId(row) === id; }) || null;
    };
    base.where = function (collection, fieldOrPredicate, opOrValue, maybeValue) {
      if (collection !== 'asesores') return baseWhere.apply(null, arguments);
      var rows = advisorProjection();
      if (typeof fieldOrPredicate === 'function') return rows.filter(fieldOrPredicate);
      if (fieldOrPredicate && typeof fieldOrPredicate === 'object') return rows.filter(function (row) { return Object.keys(fieldOrPredicate).every(function (key) { return row[key] === fieldOrPredicate[key]; }); });
      var op = arguments.length >= 4 ? opOrValue : '==';
      var value = arguments.length >= 4 ? maybeValue : opOrValue;
      return rows.filter(function (row) { return (op === '==' || op === '=') ? row[fieldOrPredicate] === value : op === '!=' ? row[fieldOrPredicate] !== value : false; });
    };
    base.find = function (collection, predicate) {
      if (collection !== 'asesores') return baseFind(collection, predicate);
      return typeof predicate === 'function' ? (advisorProjection().find(predicate) || null) : null;
    };
    base.__productHydrationRequiredOptionalP0 = Object.freeze({ version: VERSION, marker: MARKER, writes: 0, noFallback: true });
    return base;
  }
  ...
  window.Orbit.createFirestoreProductReadOnlyStoreP0 = function (deps, options) {
    ...
    var next = Object.assign({}, options || {}, { collections: hydration.all.slice() });
    return wrapStore(originalCreate(deps, next), hydration);
  };
  window.Orbit.productHydrationRequiredOptionalP0 = Object.freeze({
    VERSION: VERSION,
    MARKER: MARKER,
    contract: contract,
