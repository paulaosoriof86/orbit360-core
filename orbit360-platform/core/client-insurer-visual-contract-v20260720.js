...
(function () {
  ...
  var scheduled=false,enhancing=false;
  function runEnhance(){if(enhancing)return;enhancing=true;try{enhance();}finally{enhancing=false;}}
  function schedule(){if(scheduled)return;scheduled=true;requestAnimationFrame(function(){scheduled=false;runEnhance();});}
  function mutationTouchesCanonicalView(records){return Array.prototype.some.call(records||[],function(record){var target=record.target;if(target&&target.nodeType===1&&(target.id==='host'||target.id==='asg-ficha'||target.id==='af-body'||target.closest&&target.closest('#asg-ficha')))return true;return Array.prototype.some.call(record.addedNodes||[],function(node){if(!node||node.nodeType!==1)return false;if(node.id==='asg-ficha'||node.id==='af-body')return true;if(node.matches&&node.matches('.asg-row[data-cont],.asg-row[data-portal],.asg-row[data-cta]'))return true;return !!(node.querySelector&&node.querySelector('#asg-ficha,#af-body,.asg-row[data-cont],.asg-row[data-portal],.asg-row[data-cta]'));});});}
  ...
  var host=document.getElementById('host');if(host&&window.MutationObserver)new MutationObserver(handleCanonicalMutations).observe(host,{childList:true,subtree:true});window.addEventListener('hashchange',schedule);document.addEventListener('orbit:session',schedule);window.addEventListener('orbit:store:emit',schedule);
  document.documentElement.classList.add('orbit-m1-stable-ui');
  Orbit.clientInsurerVisualContractV20260720={version:'20260720.2',clientProjection:true,countryEvidenceProposalOnly:true,segmentationReadOnly:true,insurerSemanticView:true,secureCredentialActions:true,completeBankCopy:true,visualStability:true,synchronousMutationOwner:true,mutationMode:'same-microtask',writesStore:false,reimportsData:false,exposesSecrets:false,enhance:runEnhance};
  schedule();
})();
