...
(function () {
  ...
  function available(){const p=provider();return!!(enabled()&&tenantId()&&p&&typeof p.callFunction==='function');}
  ...
  async function command(operation,options){options=options||{};if(!available())throw new Error('OPS_LEADS_DOMAIN_BACKEND_NOT_ACTIVE');const data={tenantId:tenantId(),operation,entityId:text(options.entityId||(options.payload&&options.payload.id)),payload:options.payload||{},reason:text(options.reason||options.motivo||'Cambio realizado desde Gravicentra Insurance'),requestId:text(options.requestId||requestId(operation,options.entityId,options.payload))};return provider().callFunction(functionName(),data,region());}
  function status(){return Object.freeze({version:VERSION,functionName:functionName(),tenantId:tenantId(),region:region(),enabled:enabled(),available:available(),transport:'firebase-functions-modular'});}
