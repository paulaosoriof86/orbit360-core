/* ============================================================
   ...
   -----------------------------------------------------------------
   El dry-run del directorio solo se construye después de una lectura
   explícita de las colecciones críticas del tenant.

   Evita comparar o escribir usando una proyección anterior a una
   migración o a una actualización protegida.
   ============================================================ */
(function () {
  'use strict';
    ...
    return new Promise(function (resolve, reject) {
      (function check() {
        if (window.OrbitLabImportReadiness && typeof OrbitLabImportReadiness.loadCriticalCollections === 'function') {
          resolve(OrbitLabImportReadiness);
          return;
        }
        if (Date.now() - started >= timeoutMs) {
          reject(new Error('CRITICAL_READ_OWNER_NOT_READY'));
          return;
        }
  ...
  function install() {
    ...
    D.__freshReadGuardV20260721 = {
      version: VERSION,
      originalParseFile: originalParseFile,
  ...
  Orbit.__insurerDirectoryFreshReadGuardV20260721 = {
    version: VERSION,
    install: install,
    refreshCritical: refreshCritical,
    failClosed: true
