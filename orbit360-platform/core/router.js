...
Orbit.router = (function () {
  ...
  function moduleVisible(route) {
    try {
      if (Orbit.access && Orbit.access.can) return Orbit.access.can(route, 'view');
      if (Orbit.accessScope && Orbit.accessScope.puedeVerModulo) return Orbit.accessScope.puedeVerModulo(route);
      return (!(Orbit.tenant && Orbit.tenant.isActive) || Orbit.tenant.isActive(route)) && (!(Orbit.session && Orbit.session.canSee) || Orbit.session.canSee(route));
  ...
  function buildSidebar() {
    ...
    Orbit.NAV.forEach(blk => {
      if (blk.type === 'home') {
        if (!moduleVisible(blk.route)) return;
        ...
        const items = blk.items.filter(it => moduleVisible(it.route));
        ...
        items.forEach(it => {
          h += `<div class="nav-link" data-route="${it.route}"><span class="nav-ico">${it.icon}</span><span class="nav-txt">${it.label}</span>${badgeHtml(it.estado)}</div>`;
        });
        h += `</div></div>`;
      }
    });
    ...
    sidebar.querySelectorAll('.nav-toggle').forEach(el => el.addEventListener('click', () => {
  ...
  function setActive(route) {
    sidebar.querySelectorAll('.nav-link').forEach(el => el.classList.toggle('active', el.dataset.route === route));
  }
  ...
  function render(raw, options) {
    ...
    host.scrollTop = 0;
    if (route !== 'inicio' && !moduleVisible(route)) {
      host.innerHTML = `<div class="page"><div class="modstate"><div class="ms-ico">🔒</div><h2>No tienes acceso con el rol activo</h2><p>Tu rol activo no incluye este módulo. Si crees que deberías tenerlo, pedile a Dirección/Admin que lo habilite desde Configuración → Usuarios y permisos.</p><button class="btn primary" style="margin-top:14px" onclick="location.hash='#/inicio'">‹ Volver a Inicio</button></div></div>`;
      return;
    }
    const mod = Orbit.modules[route];
    if (mod && typeof mod.render === 'function') { host.innerHTML = ''; mod.render(host); }
    else host.innerHTML = placeholder(route);
    if (preserveScroll) window.scrollTo(0, previousScroll); else window.scrollTo(0, 0);
  }

  function onHash() { render((location.hash || '').replace(/^#\/?/, '') || 'inicio'); }
  ...
  function wireGlobalSearch() {
    ...
    function buscar(qstr) {
      ...
      const res = [];
      S.all('clientes').forEach(raw => {
        const c = Orbit.clientProjection && Orbit.clientProjection.project ? Orbit.clientProjection.project(raw) : raw;
        if (((c.nombre || '') + ' ' + (c.identificacion || '') + ' ' + (c.email || '') + ' ' + (c.telefono || '')).toLowerCase().includes(t)) res.push({ ic: '🧑', t: c.nombre || 'Cliente', s: 'Cliente · ' + (c.identificacion || c.tipo || ''), go: '#/cliente360?c=' + c.id });
      });
      S.all('polizas').forEach(p => {
        const veh = S.all('vehiculos').find(v => v.polizaId === p.id);
        const placa = (veh && veh.placa) || p.placa || '';
        const cli = clientView(p.clienteId);
        if ((p.numero + ' ' + p.producto + ' ' + placa + ' ' + (cli ? cli.nombre : '') + ' ' + (veh ? veh.marca + ' ' + veh.linea : '')).toLowerCase().includes(t)) res.push({ ic: '📑', t: p.numero + (placa ? ' · ' + placa : ''), s: 'Póliza · ' + p.ramo + (cli ? ' · ' + cli.nombre : ''), go: '#/cliente360?c=' + p.clienteId });
      });
      S.all('vehiculos').forEach(v => {
        if (((v.placa || '') + ' ' + (v.marca || '') + ' ' + (v.linea || '')).toLowerCase().includes(t)) {
          const cli = clientView(v.clienteId);
          res.push({ ic: '🚗', t: (v.placa || (v.marca + ' ' + v.linea)), s: 'Vehículo · ' + v.marca + ' ' + v.linea + (cli ? ' · ' + cli.nombre : ''), go: '#/cliente360?c=' + v.clienteId });
        }
      });
      S.all('cobros').forEach(cb => {
        const cli = clientView(cb.clienteId);
        if (((cli ? cli.nombre : '') + ' ' + (cb.numeroRecibo || '')).toLowerCase().includes(t)) res.push({ ic: '💳', t: (cb.numeroRecibo || 'Recibo') + ' · ' + U.money(cb.monto || 0, cb.moneda), s: 'Cobro · ' + cb.estado + (cli ? ' · ' + cli.nombre : ''), go: '#/cliente360?c=' + cb.clienteId });
      });
