/* ============================================================
   ...
   Contrato reusable: no persiste secretos, no depende de Firebase,
   no reemplaza Orbit.store y nunca registra el valor consultado.
   ============================================================ */
(function () {
  'use strict';
  ...
  var BLOCKED_METADATA = [
    'value', 'valor', 'password', 'pass', 'contrasena', 'token', 'secret', 'secreto',
    'clipboard', 'numero', 'numero_cuenta', 'numerocuenta', 'accountnumber', 'account_number'
  ];
  ...
  function currentActor(explicit) {
    var actor = explicit || {}, authUser = {}, activeRole = '';
    ...
    var roles = [].concat(actor.roles || [], authUser.roles || []);
  ...
  function roleAllowed(actor, allowed) {
    return currentActor(actor).roles.some(function (role) { return allowed.indexOf(role) >= 0; });
  }
  ...
  function normalizePortal(portal) {
    portal = portal || {};
    return {
      id: clean(portal.id), nombre: clean(portal.nombre || portal.name || 'Portal'),
      url: clean(portal.url), usuario: clean(portal.usuario || portal.user),
  ...
  function normalizeAccount(account) {
    account = account || {};
    return {
      id: clean(account.id), banco: clean(account.banco), tipo: clean(account.tipo),
      titular: clean(account.titular), moneda: clean(account.moneda).toUpperCase(),
      pais: clean(account.pais).toUpperCase(), uso: clean(account.uso || account.referencia),
      numero: clean(account.numero), numeroRef: clean(account.numeroRef || account.accountRef),
      estado: clean(account.estado || 'activa')
    };
  }

  function provider(explicit) { return explicit || window.OrbitSensitiveProvider || Orbit.secureSecrets || Orbit.secrets || null; }
  function method(source, names) {
    for (var i = 0; i < names.length; i += 1) if (source && typeof source[names[i]] === 'function') return source[names[i]].bind(source);
    return null;
  }

  function auditBase(input, actor, type, id, field) {
    return {
      tenantId: input.tenantId, aseguradoraId: input.aseguradoraId,
      resourceType: type, resourceId: id, field: field, actor: actor,
      reason: input.reason, metadata: safeMetadata(input.metadata)
    };
  }
  ...
  async function requestCredential(input) {
    ...
    var ref = clean(input.credentialRef);
    var base = auditBase(input, actor, 'portal', clean(input.portalId), clean(input.field || 'password'));
    ...
    var resolve = method(provider(input.provider), ['resolveCredential', 'getCredential', 'getSecret', 'reveal']);
    if (!resolve) {
    ...
    try {
      var response = await resolve({
        tenantId: resolveTenantId(input.tenantId), aseguradoraId: clean(input.aseguradoraId),
        portalId: clean(input.portalId), credentialRef: ref, field: clean(input.field || 'password'), actor: actor
      });
      ...
      writeAudit(store, Object.assign({}, base, {
        action: 'consultar_credencial', result: 'error_backend',
        metadata: safeMetadata({ credentialRef: ref, errorCode: clean(error && error.code) })
      }));
  ...
  async function requestAccountNumber(input) {
    ...
    var base = auditBase(input, actor, 'cuenta_bancaria', account.id || clean(input.accountId), 'numero_cuenta');
    ...
    var resolve = method(provider(input.provider), ['resolveAccount', 'getAccountNumber', 'getSecret', 'reveal']);
    if (!resolve) {
    ...
    try {
      var response = await resolve({
        tenantId: resolveTenantId(input.tenantId), aseguradoraId: clean(input.aseguradoraId),
        accountId: account.id || clean(input.accountId), accountRef: account.numeroRef, actor: actor
      });
      var value = clean(response && response.value != null ? response.value : response);
      ...
      writeAudit(store, Object.assign({}, base, {
        action: 'consultar_cuenta', result: 'error_backend',
        metadata: safeMetadata({ banco: account.banco, errorCode: clean(error && error.code) })
      }));
  ...
  function credentialView(portal, actor) {
    ...
    return {
      portal: item, allowed: allowed, status: exists ? 'registrada' : 'sin_credencial',
      display: allowed ? (exists ? '••••••••••••' : 'Sin credencial') : 'Acceso restringido',
      canReveal: allowed && exists, canCopyUser: allowed && !!item.usuario, canCopyPassword: allowed && exists
    };
  }
  ...
  function accountView(account, actor) {
    ...
    return {
      account: item, allowed: allowed, status: exists ? 'registrada' : 'sin_cuenta',
      display: allowed ? (item.numero ? maskAccount(item.numero) : (item.numeroRef ? '•••• cuenta protegida' : 'Sin número')) : 'Acceso restringido',
      canReveal: allowed && exists, canCopy: allowed && exists
    };
  }
  ...
  function neutralSourceDraft(country) {
    ...
    return {
      nombre: 'Nueva fuente', tipoFuente: 'otro', pais: pais,
      moneda: pais === 'GT' ? 'GTQ' : (pais === 'CO' ? 'COP' : ''),
      version: 'v1', estado: 'inventario_fuentes', contieneTarifas: false,
      contieneReglasCalculo: false, contieneHojaSalida: false,
      contieneFormatoCotizacion: false, contieneAreaImpresion: false, usos: []
    };
  }
