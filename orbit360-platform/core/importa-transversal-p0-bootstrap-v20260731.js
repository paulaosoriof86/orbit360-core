/* ============================================================
   ...
   Hace activos los owners P0 reutilizables ya existentes sin tocar
   core/importa.js. Carga normalizadores -> identidad/upsert -> wires
   -> writer -> dry-run wire, en orden determinista.
   ============================================================ */
(function(){
  'use strict';
  ...
  function markup(src){
    return '<script data-orbit-p0-owner="'+src+'" src="'+src+'?v='+VERSION+'"><\/script>';
  }
  function loadSequential(index){
    if(index>=scripts.length){
      Orbit.__importaTransversalP0Ready=true;
      try{document.dispatchEvent(new CustomEvent('orbit:importa-p0-ready',{detail:{version:VERSION}}));}catch(e){}
      return;
    }
    ...
    s.src=src+'?v='+VERSION;
    s.dataset.orbitP0Owner=src;
    s.onload=()=>loadSequential(index+1);
    s.onerror=()=>{Orbit.__importaTransversalP0LoadError=src;};
