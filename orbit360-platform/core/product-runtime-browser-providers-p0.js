...
(function () {
  ...
  function loadModules(){
    ...
    ]).then(function(parts){return{app:parts[0],auth:parts[1],store:parts[2]};});
  ...
  function initialize(){
    ...
    contextPromise=loadModules().then(function(m){
      ...
      return{modules:m,app:app,auth:m.auth.getAuth(app),db:m.store.getFirestore(app)};
    });
    return contextPromise;
  }
  function publicDescriptor(){
    var c=config();
    return Promise.resolve({projectId:'configured',authDomain:'configured',appId:'configured',hasApiKey:true,storageBucket:c.storageBucket?'configured':'',environmentRef:c.environmentRef||'product-runtime',controlledExistingIdentity:true,existingProjectReconciled:true,identitySource:'membership_only',readOnly:true,writeAuthorized:false});
  }
