...
(function(){
  ...
  function renderReceiptDetail(receiptId,cid){
    ...
    var cell=function(k,val){var shown=val==null||clean(val)===''||/^(undefined|null)$/i.test(clean(val))?'Pendiente de completar':val;return'<div><div class="muted" style="font-size:11px;text-transform:uppercase;letter-spacing:.04em">'+esc(k)+'</div><div style="font-weight:500;line-height:1.42;margin-top:2px">'+esc(shown)+'</div></div>';};
    ...
    var vehicle=[v.marca,v.linea,v.placa].filter(function(x){return x&&!/^(undefined|null)$/i.test(clean(x));}).join(' ');
    target.innerHTML='<div class="page orbit-receipt-fullpage" data-rp-receipt-detail="1" data-rp-owner="v920">'
      ...
      +'</div></section><div style="display:grid;gap:16px"><section class="card pad"><h3 style="margin-top:0">Estado y conciliación</h3><div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">'+badges+'</div><div class="muted" style="line-height:1.5">'+esc(receiptStateNote(r,portfolio))+'</div>'
      ...
      +'</section><section class="card pad"><h3 style="margin-top:0">Origen del dato</h3><div class="orbit-detail-grid" style="display:grid;grid-template-columns:1fr;gap:12px">'+trace+'</div></section></div></div></div>';
    return true;
  }
