/* Orbit 360 · Bootstrap genérico de configuración tenant para el Router.
   ...
   owner visual operativo y Academia antes de router.js.
   No contiene datos de tenants, secretos ni credenciales. */
(function () {
  'use strict';

  var CRITICAL_RELEASE = 'block1-critical-runtime-20260722-6';
  ...
  var sources = {
    ...
    visualStability: ['core/client-insurer-visual-stability-barrier-v20260721.js?v=20260721-4', '/core/client-insurer-visual-stability-barrier-v20260721.js'],
  ...
  window.OrbitTenantBootstrapState = {
    ...
    visualContractDeliveryRevision: '20260722.6',
  ...
  function writeScript(target, attribute) {
    document.write('<script src="' + escaped(target) + '" ' + attribute + '="1"><\/script>');
  }

  if (document.readyState === 'loading') {
    document.write('<link rel="stylesheet" href="' + escaped(resolved.visualStyle) + '" data-orbit-m1-visual-style="1">');
  ...
  if (!document.querySelector('link[data-orbit-m1-visual-style]')) {
    ...
    link.rel = 'stylesheet';
    link.href = resolved.visualStyle.pathname + resolved.visualStyle.search;
  ...
  function loadOperationalAcademy() {
    loadScript(resolved.operationalAcademy, 'data-orbit-operational-directory-academy', function () {
      return !!(window.Orbit && Orbit.academiaOperationalDirectoryV20260722 && Orbit.academiaOperationalDirectoryV20260722.version === '1.230');
    }, loadTenantConfig, 'operational-directory-academy-error');
  }
  function loadOperationalOwner() {
    loadScript(resolved.operationalOwner, 'data-orbit-operational-directory-owner', function () {
      return !!(window.Orbit && Orbit.clientInsurerOperationalDirectoryOwnerV20260722 && Orbit.clientInsurerOperationalDirectoryOwnerV20260722.version === '20260722.1');
    }, loadOperationalAcademy, 'operational-directory-owner-error');
  }
  function loadVisualBase() {
    loadScript(resolved.visualBase, 'data-orbit-m1-visual-contract', function () {
      return !!(window.Orbit && Orbit.clientInsurerVisualContractV20260720 && Orbit.clientInsurerVisualContractV20260720.version === '20260720.2');
    }, loadOperationalOwner, 'visual-error');
  }
  function loadVisualStability() {
    loadScript(resolved.visualStability, 'data-orbit-insurer-visual-stability', function () {
      return !!(window.Orbit && Orbit.__clientInsurerVisualStabilityBarrierV20260721 && Orbit.__clientInsurerVisualStabilityBarrierV20260721.version === '20260721.4');
    }, loadVisualBase, 'visual-stability-error');
  }
  function loadOperationalPolicy() {
    loadScript(resolved.operationalPolicy, 'data-orbit-operational-directory-policy', function () {
      return !!(window.Orbit && Orbit.operationalDirectoryFieldPolicyV20260722 && Orbit.operationalDirectoryFieldPolicyV20260722.version === '20260722.1');
    }, loadVisualStability, 'operational-directory-policy-error');
  }
  function loadSecureTargetBridge() {
    loadScript(resolved.secureTargetBridge, 'data-orbit-insurer-secure-target-bridge', function () {
      return !!(window.Orbit && Orbit.__insurerSecureTargetBridgeV20260720);
    }, loadOperationalPolicy, 'secure-target-bridge-error');
  }
  function loadCredentialProvider() {
    loadScript(resolved.credentialProvider, 'data-orbit-insurer-credential-provider', function () {
      return !!(window.Orbit && Orbit.__insurerCredentialProviderLabV20260720);
    }, loadSecureTargetBridge, 'credential-provider-error');
  }
  function loadImporterAcademy() {
    loadScript(resolved.importerAcademy, 'data-orbit-importers-e2e-academy', function () {
      return !!(window.Orbit && Orbit.ACADEMIA_V1225_IMPORTERS_E2E && Orbit.ACADEMIA_V1225_IMPORTERS_E2E.version === '1.227');
    }, loadCredentialProvider, 'importer-academy-error');
  }
  function loadImporterContract() {
    loadScript(resolved.importerContract, 'data-orbit-importer-execution-contract', function () {
      return !!(window.Orbit && Orbit.importerExecutionContractV20260720 && Orbit.importerExecutionContractV20260720.version === '20260720.2');
    }, loadImporterAcademy, 'importer-contract-error');
  }
  loadScript(resolved.session, 'data-orbit-session-readiness-contract', function () {
    return !!(window.Orbit && Orbit.sessionReadinessContractV20260720 && Orbit.sessionReadinessContractV20260720.version === '20260720.1');
  }, loadImporterContract, 'session-readiness-error');
})();
/* Preflight 1.0.38: el Router carga primero proveedores y bridge, luego política operativa, owner visual y Academia. */
