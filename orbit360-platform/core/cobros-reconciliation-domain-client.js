...
(function () {
  ...
  const available=()=>{const p=provider();return!!(enabled()&&tenantId()&&p&&typeof p.callFunction==='function');};
  ...
  async function command(operation,options){options=options||{};if(!available())throw new Error('COBROS_RECONCILIATION_BACKEND_NOT_ACTIVE');const payload=options.payload||{};return provider().callFunction(functionName(),{tenantId:tenantId(),operation,payload,reason:text(options.reason||options.motivo||'Acción de conciliación desde Gravicentra Insurance'),requestId:text(options.requestId||makeRequestId(operation,payload))},region());}
