...
(function(){
  ...
  function apply(next){
    ...
    if(form){var card=form.querySelector('[data-login-company-brand]');if(!card){card=document.createElement('div');card.setAttribute('data-login-company-brand','1');form.insertBefore(card,form.firstChild);}card.style.cssText='display:grid;place-items:center;padding:12px 16px;margin-bottom:14px;border:1px solid var(--line);border-radius:14px;background:rgba(255,255,255,.72);min-height:96px;overflow:hidden';card.innerHTML='';if(b.logo){var wrap=document.createElement('div');wrap.style.cssText='width:min(230px,82%);height:50px;display:grid;place-items:center;overflow:hidden';wrap.appendChild(img(b.logo,'data-login-tenant-logo','width:100%;height:100%;max-width:100%;max-height:100%;object-fit:contain;object-position:center;display:block'));card.appendChild(wrap);}card.setAttribute('aria-label',b.displayName);var eyebrow=form.querySelector('.lg-eyebrow');if(eyebrow)eyebrow.textContent='Acceso del equipo';}
  ...
  var api=Object.freeze({VERSION:'b1-r6-20260920.1',load:load,apply:apply,save:save,current:current,source:'build-snapshot+tenant-config-live',companyConfigAuthority:'server'});Orbit.publicTenantBranding=api;window.OrbitPublicTenantBranding=api;
