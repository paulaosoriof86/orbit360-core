...
(function(){
  ...
  function apply(next){
    ...
    if(top)top.style.cssText='visibility:hidden;display:flex;align-items:center;justify-content:center;gap:0;min-width:118px;max-width:138px;height:38px;overflow:hidden';
    if(topSlot){topSlot.innerHTML='';topSlot.style.cssText='width:128px;height:34px;display:grid;place-items:center;overflow:hidden;border:0;background:transparent';if(b.logo)topSlot.appendChild(img(b.logo,'data-company-brand-logo','width:100%;height:100%;object-fit:contain;object-position:center;display:block'));}
    ...
    if(form){var card=form.querySelector('[data-login-company-brand]');if(!card){card=document.createElement('div');card.setAttribute('data-login-company-brand','1');form.insertBefore(card,form.firstChild);}card.style.cssText='display:grid;place-items:center;padding:12px 16px;margin-bottom:14px;border:1px solid var(--line);border-radius:14px;background:rgba(255,255,255,.72);min-height:96px;overflow:hidden';card.innerHTML='';if(b.logo){var wrap=document.createElement('div');wrap.style.cssText='width:min(270px,100%);height:62px;display:grid;place-items:center;overflow:hidden';wrap.appendChild(img(b.logo,'data-login-tenant-logo','width:100%;height:100%;object-fit:contain;object-position:center;display:block'));card.appendChild(wrap);}card.setAttribute('aria-label',b.displayName);var eyebrow=form.querySelector('.lg-eyebrow');if(eyebrow)eyebrow.textContent='Acceso del equipo';}
