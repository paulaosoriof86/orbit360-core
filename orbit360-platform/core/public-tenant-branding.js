...
(function(){
  ...
  function img(src,attr,style){var i=document.createElement('img');i.src=src;i.alt='';i.style.cssText=style||'max-width:100%;max-height:100%;object-fit:contain;display:block';if(attr)i.setAttribute(attr,'1');return i;}
  ...
  function apply(next){
    ...
    if(bottom)bottom.style.cssText='visibility:hidden;min-height:72px;display:flex;align-items:center;justify-content:flex-start;padding:12px 18px;overflow:hidden';
    if(slot){slot.innerHTML='';slot.style.cssText='width:min(250px,70vw);height:48px;display:grid;place-items:center;overflow:hidden;border:0;background:transparent';if(b.logo)slot.appendChild(img(b.logo,'data-company-brand-logo','width:100%;height:100%;object-fit:contain;object-position:left center;display:block'));}
    ...
    if(top)top.style.cssText='visibility:hidden;display:flex;align-items:center;gap:0;min-width:150px;max-width:190px;height:44px;overflow:hidden';
    if(topSlot){topSlot.innerHTML='';topSlot.style.cssText='width:170px;height:38px;display:grid;place-items:center;overflow:hidden;border:0;background:transparent';if(b.logo)topSlot.appendChild(img(b.logo,'data-company-brand-logo','width:100%;height:100%;object-fit:contain;object-position:left center;display:block'));}
  ...
  var api=Object.freeze({VERSION:'b1-r2-20260919.1',load:load,apply:apply,save:save,current:current,source:'tenant-config-live',companyConfigAuthority:'server'});Orbit.publicTenantBranding=api;window.OrbitPublicTenantBranding=api;
