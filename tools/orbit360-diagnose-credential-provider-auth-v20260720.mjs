...
function summarizeBindings(policy) {
  const bindings = Array.isArray(policy?.bindings) ? policy.bindings : [];
  return bindings.map(binding => {
    const members = Array.isArray(binding.members) ? binding.members : [];
    return {
      role: clean(binding.role, 160),
      public: members.includes('allUsers'),
      authenticatedPublic: members.includes('allAuthenticatedUsers'),
      userMembers: members.filter(value => /^user:/i.test(value)).length,
      serviceAccountMembers: members.filter(value => /^serviceAccount:/i.test(value)).length,
      groupMembers: members.filter(value => /^group:/i.test(value)).length,
      domainMembers: members.filter(value => /^domain:/i.test(value)).length,
      otherMembers: members.filter(value => !/^(allUsers|allAuthenticatedUsers|user:|serviceAccount:|group:|domain:)/i.test(value)).length
    };
...
const report = {
  schemaVersion: 'orbit360-provider-authorization-diagnostic-v1',
...
try {
  ...
  const assignedRoles = unique(rolesFrom(member).concat(rolesFrom(advisor))).sort();
...
try {
  ...
  const functionData = await api(token, `https://cloudfunctions.googleapis.com/v2/${functionName}`);
  ...
  try {
    const functionPolicy = await api(token, `https://cloudfunctions.googleapis.com/v2/${functionName}:getIamPolicy`, { method: 'POST', body: {} });
    report.functionIam = { readable: true, bindings: summarizeBindings(functionPolicy) };
  } catch (error) {
  ...
  if (serviceResource) {
    try {
      const runService = await api(token, `https://run.googleapis.com/v2/${serviceResource}`);
    ...
    try {
      const runPolicy = await api(token, `https://run.googleapis.com/v2/${serviceResource}:getIamPolicy`, { method: 'POST', body: {} });
      const bindings = summarizeBindings(runPolicy);
      const invokerBindings = bindings.filter(binding => binding.role === 'roles/run.invoker');
      report.cloudRunIam = {
        readable: true,
        bindings,
        invokerBindingCount: invokerBindings.length,
        publicInvoker: invokerBindings.some(binding => binding.public),
        authenticatedPublicInvoker: invokerBindings.some(binding => binding.authenticatedPublic)
      };
...
if (report.checks.membershipAuthorizationWouldPass === true && report.checks.publicCallableIngress === false) {
  report.classification = 'ENVIRONMENT_FAILURE_CALLABLE_INVOKER';
  report.rootCauseCandidate = 'Cloud Run no permite la invocación pública necesaria para que el protocolo callable entregue el Firebase ID token al handler.';
} else if (report.checks.membershipAuthorizationWouldPass === false) {
  report.classification = 'DATA_CONTRACT_FAILURE_MEMBERSHIP';
  report.rootCauseCandidate = 'La membresía efectiva aún no cumple la autorización del proveedor.';
  ...
  report.classification = 'PROVIDER_AUTHORIZATION_REQUIRES_DEEPER_LOG_DIAGNOSTIC';
  report.rootCauseCandidate = 'La membresía y la invocación parecen compatibles; se requieren logs sanitizados del handler para identificar el rechazo exacto.';
}
report.ok = report.classification !== 'PROVIDER_AUTHORIZATION_REQUIRES_DEEPER_LOG_DIAGNOSTIC';
write(report);
console.log(`ORBIT360_PROVIDER_AUTH_DIAGNOSTIC:${report.classification}`);
if (!report.ok) process.exit(65);
