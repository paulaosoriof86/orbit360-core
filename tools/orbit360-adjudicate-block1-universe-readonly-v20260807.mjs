...
export const SCHEMA = 'orbit360-block1-universe-adjudication-v22-readonly-v1';
...
const EVIDENCE = process.env.ORBIT360_UNIVERSE_EVIDENCE || 'orbit360-platform/runtime-gate-crm-v20260716/v22-block1-universe-adjudication-sanitized-v20260807.json';
...
export async function runAdjudication(db) {
  ...
  const classification = failed.some(x => x.classification === 'DATA_CONTRACT_FAILURE') ? 'DATA_CONTRACT_FAILURE'
    : failed.some(x => x.classification === 'VALIDATOR_STALE') ? 'VALIDATOR_STALE'
      : 'PASS_DATA_CONTRACT';
