...
async function probe(page,target){
  ...
  need(state.ownerVersion==='20260829.1','I4A_INSURER_UI_OWNER_VERSION_MISMATCH');need(state.compositionRevision==='20260902.1-predecessor-row-reclaim','I4A_INSURER_UI_OWNER_COMPOSITION_MISMATCH');need(state.barrierRevision==='20260902.1-latest-operational-owner-precedence','I4A_INSURER_UI_BARRIER_MISMATCH');need(state.rows===candidate.portalCount&&state.cards===candidate.portalCount,'I4A_INSURER_UI_PORTAL_CARD_COUNT_MISMATCH');need(state.credentialBoxes===candidate.portalCount,'I4A_INSURER_UI_CREDENTIAL_BOX_COUNT_MISMATCH');need(state.stable,'I4A_INSURER_UI_VIEW_NOT_STABLE');if(candidate.userBearing>0&&PRIV.has(target))need(state.userVisible>0,'I4A_INSURER_UI_USERNAME_NOT_VISIBLE');
