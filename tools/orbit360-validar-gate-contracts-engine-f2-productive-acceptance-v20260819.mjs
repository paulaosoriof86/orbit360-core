...
const textAbs=p=>fs.readFileSync(p,'utf8');
const text=p=>textAbs(path.join(ROOT,p));
...
const workflowSource=()=>{const raw=String(process.env.ORBIT360_F2_WORKFLOW_SOURCE_FILE||'').trim();return raw?path.resolve(raw):path.join(ROOT,DEFAULT_WORKFLOW);};
...
try{
  ...
  const durableCertificationPass=
    /^orbit360-macro2-candidate-artifact-metadata-v\d+$/.test(String(cert.schemaVersion||''))&&
    cert.status==='CANDIDATE_ARTIFACT_PUBLISHED_SOURCE_ONLY'&&cert.sourcePublished===true&&
    Number(cert.artifactId)===Number(c.artifactId)&&cert.sourceHead===c.sourceHead&&String(cert.artifactDigest)===String(c.artifactDigest)&&
    cert.zipSha256===c.zipSha256&&cert.manifestSha256===c.manifestSha256&&Number(cert.fileCount)===Number(c.fileCount)&&
    Number(cert.deltaCount)===9&&Number(cert.unchangedFileCount)===185&&
    cert.runtimeExecuted===false&&cert.browserExecuted===false&&cert.secretAccess===false&&cert.firestoreRead===false&&Number(cert.writes)===0&&
    cert.deployExecuted===false&&cert.productionTouched===false&&cert.containsPII===false&&cert.containsSecrets===false&&
    closure.status==='TRANSVERSAL_SOURCE_ACCEPTANCE_PASS'&&closure.evidencePath===certPath&&Number(closure.runId)===Number(cert.runId)&&
    Number(closure.candidateArtifactId)===Number(c.artifactId)&&closure.sourceHead===c.sourceHead&&String(closure.artifactDigest)===String(c.artifactDigest)&&
    Number(closure.checksPassed)===107&&Number(closure.deltaCount)===9&&Number(closure.fileCount)===194&&Number(closure.unchangedFileCount)===185;
  ...
  need(authPath&&fs.existsSync(path.join(ROOT,authPath)),'SECURITY_FAILURE:F2_AUTHORIZATION_RECORD_INVALID');
  const a=read(authPath);
  ...
  const s=a.scopeAuthorized||{};
  need(s.runtime===true&&s.browser===true&&s.firestoreRead===true&&s.secrets===true&&s.customTokenMint===true,'SECURITY_FAILURE:F2_AUTH_READ_CAPABILITIES_MISSING');
  ...
  const wfPath=workflowSource();need(fs.existsSync(wfPath),'PIPELINE_MECHANISM_FAILURE:F2_EXECUTING_WORKFLOW_SOURCE_MISSING');
  const wf=textAbs(wfPath),ex=text(EXECUTOR),gateA='Mandatory canonical F2 gate before artifact or provider/browser',gateB='Mandatory F2 gate before artifact/provider/browser',providerA='Bind provider after F2 GO',providerB='Bind read-only provider only after GO';
