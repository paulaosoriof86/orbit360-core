...
try{
  ...
  const requestOrdinal=Number(request.requestOrdinal||0);
  need(Number.isInteger(requestOrdinal)&&requestOrdinal>=11,'SECURITY_FAILURE:F2_REQUEST_ORDINAL_INVALID');
  ...
  need(request.scope?.writes===false&&request.scope?.authWrites===false&&request.scope?.membershipWrites===false&&request.scope?.dataWrites===false&&request.scope?.operationalWrites===false&&request.scope?.deploy===false&&request.scope?.publication===false&&request.scope?.productionMutation===false,'SECURITY_FAILURE:F2_REQUEST_EXCESS_CAPABILITY');

  const authorizationRecordPath=String(request.authorizationRecordPath||'').trim();
  need(/^\.github\/orbit360-authorizations\/f2-productive-acceptance-runtime-browser-readonly-request\d+-v\d+\.json$/.test(authorizationRecordPath),'SECURITY_FAILURE:F2_AUTHORIZATION_RECORD_PATH_INVALID');
  need(fs.existsSync(path.join(ROOT,authorizationRecordPath)),'SECURITY_FAILURE:F2_AUTHORIZATION_RECORD_MISSING');
  const authorization=readJson(authorizationRecordPath),authorizationDigest=digest(authorizationRecordPath);
  need(request.authorizationRecordSha256===authorizationDigest,'SECURITY_FAILURE:F2_AUTHORIZATION_RECORD_DIGEST_MISMATCH');
  need(String(request.authorizationBasis||'').length>20&&request.authorizationBasis===authorization.authorizationBasis,'SECURITY_FAILURE:F2_AUTHORIZATION_BASIS_MISMATCH');
  need(authorization.schemaVersion==='orbit360-f2-runtime-authorization-v1'&&authorization.status==='AUTHORIZED_PERSISTED_PENDING_REQUEST'&&authorization.approved===true&&authorization.requestVersion===EXPECT.requestVersion&&Number(authorization.requestOrdinal)===requestOrdinal&&authorization.gateId===GATE,'SECURITY_FAILURE:F2_PERSISTED_AUTHORIZATION_IDENTITY_INVALID');
  need(authorization.branch==='ays/backend-tenant-lab-v99-20260703'&&Number(authorization.pullRequest)===5,'SECURITY_FAILURE:F2_PERSISTED_AUTHORIZATION_BRANCH_PR_INVALID');
