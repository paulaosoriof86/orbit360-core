...
try{
  ...
  const wf=text(WORKFLOW),ex=text(EXECUTOR);
  need(wf.includes('MACRO3_INLINE_F2_V1')&&wf.includes('Mandatory canonical F2 gate before artifact or provider/browser')&&wf.indexOf('Mandatory canonical F2 gate before artifact or provider/browser')<wf.indexOf('Bind provider after F2 GO'),'SECURITY_FAILURE:F2_GATE_ORDER_INVALID');
  need(wf.includes(REQUEST_VERSION)&&!wf.includes('/dispatches')&&!wf.includes('workflow_run:'),'PIPELINE_MECHANISM_FAILURE:F2_INLINE_RUNTIME_NOT_SINGLE_TRIGGER');
