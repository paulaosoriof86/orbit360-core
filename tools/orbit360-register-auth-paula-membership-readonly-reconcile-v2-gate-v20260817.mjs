...
fs.writeFileSync(file,s,'utf8');const v=fs.readFileSync(file,'utf8');if(!v.includes(marker)||phases.some(([p])=>!v.includes(p)))throw new Error('PIPELINE_MECHANISM_FAILURE:GATE14_3_PHASES_NOT_PERSISTED');console.log(JSON.stringify({ok:true,gateId,phases:phases.map(([p])=>p.replace(/:$/,''))}));
