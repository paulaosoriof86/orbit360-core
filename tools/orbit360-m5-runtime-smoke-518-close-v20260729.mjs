...
const browser=read('m5-runtime-smoke-518-browser-summary.json');
const preflight=read('preflight-sanitizado.json');
const contract=read('m5-runtime-smoke-518-contract-summary.json');
...
const ok=Boolean(
  preflight&&preflight.status==='GO_GATE_CONTRACT'&&preflight.failed===0&&preflight.contractVersion==='5.0.18'&&
  contract&&contract.status==='M5_RUNTIME_SMOKE_518_CONTRACT_PASS'&&contract.failed===0&&contract.releaseCandidateHash===RC_HASH&&
  before&&before.ok===true&&after&&after.ok===true&&
  browser&&browser.ok===true&&browser.releaseCandidateHash===RC_HASH&&browser.contractVersion==='5.0.18'&&
  policyOwnerOk&&guardInstalled&&normalizedOwnerOk&&roleViewsOk&&transientStaticCalls>0&&blockedOperationalCalls===0&&networkWriteCandidates===0&&
  allDigestsStable&&allCountsStable
);
const out={
  schemaVersion:'orbit360-m5-runtime-smoke-summary-v5',generatedAt:new Date().toISOString(),ok,
  status:ok?'M5_RUNTIME_SMOKE_518_CLOSED_SUCCESS':'M5_RUNTIME_SMOKE_518_FAILED_STOP_LINE',
  gateId:'block5-release-candidate-visualization-v20260728',contractVersion:'5.0.18',releaseCandidateHash:RC_HASH,
  ...
  beforeCounts:before&&before.counts||{},afterCounts:after&&after.counts||{},digestComparisons,allDigestsStable,allCountsStable,policyOwnerReady:policyOwnerOk,storeGuardInstalled:guardInstalled,normalizedBootstrapOwner:normalizedOwnerOk,roleViewsCompleted:roleViewsOk,
...
fs.writeFileSync(path.join(DIR,'m5-runtime-smoke-518-summary.json'),JSON.stringify(out,null,2)+'\n');
