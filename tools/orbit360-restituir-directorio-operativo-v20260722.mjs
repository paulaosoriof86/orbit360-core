...
function snapshotState(snapshot, vault) {
  ...
  snapshot.docs.forEach(docSnapshot => {
    ...
    accounts.forEach((account, index) => {
      ...
      if (currentNumber !== record.number || clean(account.estado, 80) !== 'Cuenta operativa disponible') {
    ...
    portals.forEach((portal, index) => {
      ...
      if (record.passwordAvailable) afterPortals[index].estadoCredencial = 'registrada';
      if (record.username && record.passwordAvailable) afterPortals[index].estadoAcceso = 'Acceso disponible';
    });
...
function validatePlan(state, vault, phase) {
  ...
  if (s.credentialUnmatchedRefs !== 0) fail(`${phase}_CREDENTIAL_MATCH_MISMATCH`, s.credentialUnmatchedRefs);
...
function sanitizedReportBase(counts, vault, state) {
  return {
    schemaVersion: 'orbit360-operational-directory-repair-v1',
...
async function main() {
  if (APPLY && process.env.ORBIT360_OPERATIONAL_DIRECTORY_APPLY !== 'AUTHORIZED_ONCE') fail('APPLY_AUTHORIZATION_REQUIRED');
  ...
  try {
    ...
    if (after.summary.accountRowsProposed !== 0 || after.summary.usernameRowsProposed !== 0) fail('AFTER_REPAIR_NOT_IDEMPOTENT', `${after.summary.accountRowsProposed}/${after.summary.usernameRowsProposed}`);
...
main().catch(error => {
  const payload = {
    ...
    tenantId: TENANT,
    ok: false,
    classification: String(error && (error.code || '')).includes('PROJECT') || String(error && (error.code || '')).includes('SERVICE_ACCOUNT') ? 'ENVIRONMENT_FAILURE' : 'DATA_CONTRACT_FAILURE',
    errorCode: clean(error && (error.code || error.message), 160).replace(/[^A-Za-z0-9_.:-]/g, '_'),
