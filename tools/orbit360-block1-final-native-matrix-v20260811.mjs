...
export const SOURCE_CONTRACT=Object.freeze({
  ...
  bootstrapNavigationOwner:'document-commit-login-form-firebase-sdk-default-app-auth-readiness-segmented',
  bootstrapInitialWaitUntil:'commit',
  bootstrapContextCloseOnFailure:true,
  firebaseSdkOwner:'firebase-compat-sdk-present',
  firebaseDefaultAppOwner:'firebase-default-app-initialized',
  firebaseAuthOwner:'firebase-default-app-auth-instance-ready',
  routerReadinessOwner:'post-timeout-canonical-state-recheck-with-owner-diagnostics',
  clientLateReadyOwner:'post-timeout-canonical-state-recovery',
  validatorFailClosedOwner:'late-ready-recovered-owner-specific-failure-only',
...
const result={
  ...
  bootstrapNavigationOwner:'document-commit-login-form-firebase-sdk-default-app-auth-readiness-segmented',bootstrapInitialWaitUntil:'commit',bootstrapContextCloseOnFailure:true,
  firebaseSdkOwner:'firebase-compat-sdk-present',firebaseDefaultAppOwner:'firebase-default-app-initialized',firebaseAuthOwner:'firebase-default-app-auth-instance-ready',
  routerReadinessOwner:'post-timeout-canonical-state-recheck-with-owner-diagnostics',clientLateReadyOwner:'post-timeout-canonical-state-recovery',validatorFailClosedOwner:'late-ready-recovered-owner-specific-failure-only',
...
export function firebaseDefaultAppAuthReadyState(state){return state&&state.sdkLoaded===true&&Number(state.appsCount)>0&&state.defaultAppName==='[DEFAULT]'&&state.authAppName==='[DEFAULT]';}
export function routerReadyStateReady(state){return !!(state&&state.orbitPresent===true&&state.routerPresent===true&&state.route==='inicio'&&state.hydrationMounted===true&&state.hydrationReady===true&&state.loading===false&&Number(state.hostTextLength)>60&&state.authInside===true&&state.membershipReady===true);}
export function clientDetailReadyState(state){return !!(state&&state.route==='cliente360'&&state.paramPresent===true&&state.header===true&&state.tabs===true&&state.body===true);}
export function clientRouteReadyState(state,kind='client-empty'){if(!(state&&state.route==='cliente360'&&state.paramMatch===true))return false;if(kind==='client-empty')return state.body===true;return state.header===true&&state.tabs===true&&state.body===true;}
export function syntheticBootstrapNavigationContract(){const simulated={documentCommitMs:120,loginFormReadyMs:180,externalScriptSettledMs:60000,legacyDomContentLoadedTimeoutMs:45000};const legacyWouldTimeout=simulated.externalScriptSettledMs>simulated.legacyDomContentLoadedTimeoutMs;const segmentedDocumentPass=simulated.documentCommitMs<15000;const segmentedLoginPass=simulated.loginFormReadyMs<15000;const sdkWithoutDefaultAppBlocked=firebaseDefaultAppAuthReadyState({sdkLoaded:true,appsCount:0,defaultAppName:'',authAppName:''})===false;const defaultAppAuthReady=firebaseDefaultAppAuthReadyState({sdkLoaded:true,appsCount:1,defaultAppName:'[DEFAULT]',authAppName:'[DEFAULT]'})===true;return {legacyWouldTimeout,segmentedDocumentPass,segmentedLoginPass,sdkWithoutDefaultAppBlocked,defaultAppAuthReady,externalResourceDiagnosedSeparately:true,contextCloseGuaranteed:true,ok:legacyWouldTimeout&&segmentedDocumentPass&&segmentedLoginPass&&sdkWithoutDefaultAppBlocked&&defaultAppAuthReady};}
export function syntheticLateReadyRecoveryContract(){const routerLateReadyRecovered=routerReadyStateReady({orbitPresent:true,routerPresent:true,route:'inicio',hydrationMounted:true,hydrationReady:true,loading:false,hostTextLength:220,authInside:true,membershipReady:true});const routerUnreadyBlocked=routerReadyStateReady({orbitPresent:true,routerPresent:true,route:'inicio',hydrationMounted:true,hydrationReady:false,loading:true,hostTextLength:20,authInside:true,membershipReady:true})===false;const clientDetailLateReadyRecovered=clientDetailReadyState({route:'cliente360',paramPresent:true,header:true,tabs:true,body:true});const clientRouteLateReadyRecovered=clientRouteReadyState({route:'cliente360',paramMatch:true,header:false,tabs:false,body:true},'client-empty');const clientRouteWrongParamBlocked=clientRouteReadyState({route:'cliente360',paramMatch:false,header:true,tabs:true,body:true},'client-empty')===false;return {routerLateReadyRecovered,routerUnreadyBlocked,clientDetailLateReadyRecovered,clientRouteLateReadyRecovered,clientRouteWrongParamBlocked,ok:routerLateReadyRecovered&&routerUnreadyBlocked&&clientDetailLateReadyRecovered&&clientRouteLateReadyRecovered&&clientRouteWrongParamBlocked};}
async function bootstrapInitialDocument(page,role){const prefix=role.toUpperCase();const failures=[];let mainStatus=0;page.on('requestfailed',request=>{if(failures.length<12)failures.push(sanitizedRequestFailure(request));});mark(prefix+'_BOOTSTRAP_DOCUMENT_COMMIT_WAIT');const started=Date.now();try{const response=await page.goto(BASE_URL+'#/inicio',{waitUntil:'commit',timeout:15000});mainStatus=Number(response&&response.status?response.status():0);mark(prefix+'_BOOTSTRAP_DOCUMENT_COMMIT_PASS',{elapsedMs:Date.now()-started,status:mainStatus});if(mainStatus>=400)throw classified('ENVIRONMENT_FAILURE',`ENVIRONMENT_BOOTSTRAP_MAIN_DOCUMENT_HTTP_${mainStatus}`,'BOOTSTRAP_MAIN_DOCUMENT_HTTP_ERROR');}catch(error){if(error&&error.orbitClassification)throw error;throw classified('PIPELINE_MECHANISM_FAILURE',`PIPELINE_BOOTSTRAP_DOCUMENT_COMMIT_FAILED:${clean(error&&error.message||error)}`,'BOOTSTRAP_DOCUMENT_COMMIT_FAILED');}mark(prefix+'_BOOTSTRAP_LOGIN_FORM_WAIT');try{await page.waitForSelector('#login-form',{state:'attached',timeout:15000});mark(prefix+'_BOOTSTRAP_LOGIN_FORM_PASS',{failedResources:failures});}catch(error){throw classified('PIPELINE_MECHANISM_FAILURE',`PIPELINE_BOOTSTRAP_LOGIN_FORM_TIMEOUT:failedResources=${JSON.stringify(failures)}`,'BOOTSTRAP_LOGIN_FORM_TIMEOUT');}mark(prefix+'_BOOTSTRAP_FIREBASE_SDK_WAIT');try{await page.waitForFunction(()=>!!(window.firebase&&typeof firebase.auth==='function'),null,{timeout:30000});mark(prefix+'_BOOTSTRAP_FIREBASE_SDK_PASS',{failedResources:failures});}catch(error){throw classified('PIPELINE_MECHANISM_FAILURE',`PIPELINE_BOOTSTRAP_FIREBASE_SDK_TIMEOUT:failedResources=${JSON.stringify(failures)}`,'BOOTSTRAP_FIREBASE_SDK_TIMEOUT');}mark(prefix+'_BOOTSTRAP_FIREBASE_DEFAULT_APP_WAIT');try{await page.waitForFunction(()=>{try{if(!(window.firebase&&Array.isArray(firebase.apps)&&firebase.apps.length>0&&typeof firebase.auth==='function'))return false;const app=firebase.app();const auth=firebase.auth();return !!(app&&app.name==='[DEFAULT]'&&auth&&auth.app&&auth.app.name==='[DEFAULT]');}catch{return false;}},null,{timeout:30000});const firebaseState=await page.evaluate(()=>{try{const app=firebase.app();const auth=firebase.auth();return {appsCount:Array.isArray(firebase.apps)?firebase.apps.length:0,defaultAppName:app&&app.name||'',authAppName:auth&&auth.app&&auth.app.name||''};}catch{return {appsCount:0,defaultAppName:'',authAppName:''};}});mark(prefix+'_BOOTSTRAP_FIREBASE_DEFAULT_APP_PASS',{...firebaseState,failedResources:failures});}catch(error){throw classified('PIPELINE_MECHANISM_FAILURE',`PIPELINE_BOOTSTRAP_FIREBASE_DEFAULT_APP_TIMEOUT:failedResources=${JSON.stringify(failures)}`,'BOOTSTRAP_FIREBASE_DEFAULT_APP_TIMEOUT');}return {mainStatus,failures};}
async function loginContext(browser,matrix,member){const context=await browser.newContext({viewport:{width:matrix.width,height:matrix.height},locale:'es-GT'});let page=null;try{page=await context.newPage();const consoleErrors=[];page.on('pageerror',e=>consoleErrors.push(clean(e)));page.on('console',m=>{if(m.type()==='error')consoleErrors.push(clean(m.text()));});const started=Date.now();mark(matrix.role.toUpperCase()+'_PAGE_GOTO');await bootstrapInitialDocument(page,matrix.role);await page.evaluate(()=>{try{localStorage.removeItem('orbit360_confidencialidad');localStorage.removeItem('orbit360_legal_aceptaciones');}catch{}});const token=await globalThis.__orbitAdminFinal.auth().createCustomToken(member.uid);mark(matrix.role.toUpperCase()+'_AUTH_CUSTOM_TOKEN_SIGNIN_WAIT');try{await page.evaluate(async t=>{await firebase.auth().signInWithCustomToken(t);},token);mark(matrix.role.toUpperCase()+'_AUTH_CUSTOM_TOKEN_SIGNIN_PASS');}catch(error){throw classified('ENVIRONMENT_FAILURE',`ENVIRONMENT_AUTH_CUSTOM_TOKEN_SIGNIN_FAILED:${clean(error&&error.message||error)}`,'AUTH_CUSTOM_TOKEN_SIGNIN_FAILED');}await page.waitForFunction(()=>!document.body.classList.contains('pre-auth')&&document.body.dataset.authStage==='inside',null,{timeout:35000});await page.waitForFunction(()=>{try{const s=Orbit.session&&Orbit.session.membershipProjectionStatus&&Orbit.session.membershipProjectionStatus();return !!(s&&s.ready===true&&s.tenantBound===true&&Orbit.auth&&Orbit.auth.productUser&&Orbit.auth.productUser.__labMembershipProjection===true);}catch{return false;}},null,{timeout:35000});const removed=await normalizeEphemeralSecurityOverlay(page);mark(matrix.role.toUpperCase()+'_EPHEMERAL_SECURITY_OVERLAY_NORMALIZED',{removed});return {context,page,consoleErrors,loginMs:Date.now()-started};}catch(error){try{await context.close();}catch{}throw error;}}
...
async function waitRouterReady(page,role,timeout=35000){const prefix=role.toUpperCase();mark(`${prefix}_ROUTER_READY_WAIT`);try{await page.waitForFunction(()=>{try{const host=document.getElementById('host');const d=window.OrbitHydrationContractDiagnostics;const s=d&&typeof d.status==='function'?d.status('inicio')||{}:{};const m=Orbit.session&&Orbit.session.membershipProjectionStatus?Orbit.session.membershipProjectionStatus():{};return !!(window.Orbit&&Orbit.router&&Orbit.route&&Orbit.route.key==='inicio'&&d&&typeof d.mounted==='function'&&d.mounted()&&s.ready===true&&!document.querySelector('.orbit-load-state')&&host&&(host.innerText||'').trim().length>60&&document.body.dataset.authStage==='inside'&&!document.body.classList.contains('pre-auth')&&m&&m.ready===true&&m.tenantBound===true);}catch{return false;}},null,{timeout});const state=await routerState(page);mark(`${prefix}_ROUTER_READY_PASS`,state);return state;}catch(error){await sleep(500);const state=await routerState(page);mark(`${prefix}_ROUTER_READY_POST_TIMEOUT_STATE`,state);if(routerReadyStateReady(state)){mark(`${prefix}_ROUTER_READY_LATE_PASS`,state);return {...state,lateReady:true};}const failedOwners=[];if(!state.orbitPresent||!state.routerPresent)failedOwners.push('router');if(state.route!=='inicio')failedOwners.push('route-inicio');if(!state.hydrationMounted)failedOwners.push('hydration-mounted');if(!state.hydrationReady)failedOwners.push('hydration-ready');if(state.loading)failedOwners.push('loading-cleared');if(Number(state.hostTextLength)<=60)failedOwners.push('inicio-host');if(!state.authInside)failedOwners.push('auth-inside');if(!state.membershipReady)failedOwners.push('membership-ready');const detail={...state,failedOwners};if(!state.authInside||!state.membershipReady)throw classified('PIPELINE_MECHANISM_FAILURE',`PIPELINE_ROUTER_READY_POST_LOGIN_STATE_INVALID:${JSON.stringify(detail)}`,'ROUTER_READY_POST_LOGIN_STATE_INVALID');throw classified('FUNCTIONAL_DEFECT',`FUNCTIONAL_ROUTER_READY_OWNER_TIMEOUT:${JSON.stringify(detail)}`,'FUNCTIONAL_ROUTER_READY_OWNER_TIMEOUT');}}
...
if(process.env.ORBIT360_MATRIX_ARTIFACT_VALIDATE_ONLY==='1'){const bootstrapSynthetic=syntheticBootstrapNavigationContract();const lateReadySynthetic=syntheticLateReadyRecoveryContract();console.log(JSON.stringify({status:'PASS_BLOCK1_FINAL_NATIVE_MATRIX_IMPORT',classification:'SOURCE_ARTIFACT_VALIDATED',sourceContract:SOURCE_CONTRACT,bootstrapSyntheticPass:bootstrapSynthetic.ok,firebaseSdkWithoutDefaultAppBlocked:bootstrapSynthetic.sdkWithoutDefaultAppBlocked,firebaseDefaultAppAuthReady:bootstrapSynthetic.defaultAppAuthReady,lateReadySyntheticPass:lateReadySynthetic.ok,routerLateReadyRecovered:lateReadySynthetic.routerLateReadyRecovered,routerUnreadyBlocked:lateReadySynthetic.routerUnreadyBlocked,clientDetailLateReadyRecovered:lateReadySynthetic.clientDetailLateReadyRecovered,clientRouteLateReadyRecovered:lateReadySynthetic.clientRouteLateReadyRecovered,clientRouteWrongParamBlocked:lateReadySynthetic.clientRouteWrongParamBlocked,bootstrapSynthetic,lateReadySynthetic,externalRuntimeDependenciesLoaded:false,firebaseAccess:false,browserExecuted:false,hostingTouched:false,writes:0,ok:bootstrapSynthetic.ok&&lateReadySynthetic.ok}));}
