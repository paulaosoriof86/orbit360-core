...
try{
 ...
 add('REQUEST_ABSENT',!fs.existsSync(REQUEST));
 ...
 const failed=checks.filter(x=>!x.ok);result={schemaVersion:'orbit360-fase-a-ops-leads-crm-composed-runtime-source-v1',gateId:'fase-a-ops-leads-crm-release-lab-v20260812',status:failed.length?'STOP_COMPOSED_RUNTIME_SOURCE':'PASS_FASE_A_COMPOSED_RUNTIME_PACKAGE_SOURCE',classification:failed.length?'PIPELINE_MECHANISM_FAILURE':'RUNTIME_PACKAGE_READY_REQUEST_ABSENT',checksPassed:checks.length-failed.length,checksFailed:failed.length,failedCheckIds:failed.map(x=>x.id),checks,requestPresent:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,authWrites:0,operationalWrites:0,browserExecuted:false,deployExecuted:false,productionTouched:false,ok:failed.length===0};
}catch(e){result={schemaVersion:'orbit360-fase-a-ops-leads-crm-composed-runtime-source-v1',status:'STOP_COMPOSED_RUNTIME_SOURCE',classification:'PIPELINE_MECHANISM_FAILURE',error:String(e&&e.message||e).slice(0,600),requestPresent:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,browserExecuted:false,deployExecuted:false,productionTouched:false,ok:false};}
