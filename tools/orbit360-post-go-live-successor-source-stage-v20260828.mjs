...
function safeCode(value){return String(value==null?'':value).replace(/[^A-Za-z0-9_.:\/-]/g,'_').slice(0,220);}
function validateProfile(head,profile,R){const spec=R.validationProfiles?.[profile];if(!spec||!Array.isArray(spec.validators)||!spec.validators.length)throw new Error('PRODUCT_GIT_STAGE_VALIDATION_PROFILE_INVALID');const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'orbit360-product-candidate-'));try{git(['worktree','add','--detach',tmp,head]);const results=[];for(const script of spec.validators){const r=spawnSync(process.execPath,[path.join(tmp,script),tmp],{cwd:tmp,encoding:'utf8',env:{...process.env,ORBIT360_ROOT:tmp}});let out={};try{out=JSON.parse(String(r.stdout||'').trim());}catch{}if(r.status!==0||out.ok!==true){const childClass=safeCode(out.classification||'UNKNOWN');const childStatus=safeCode(out.status||r.status||'UNKNOWN');const childCode=safeCode(out.code||out.failureCode||'NO_CHILD_CODE');throw new Error(`PRODUCT_GIT_STAGE_VALIDATOR_FAIL:${script}:${childClass}:${childStatus}:${childCode}`);}results.push({path:script,status:out.status,classification:out.classification||'PASS',ok:true});}return results;}finally{try{git(['worktree','remove','--force',tmp]);}catch{}try{fs.rmSync(tmp,{recursive:true,force:true});}catch{}}}
...
if(selftest){console.log(JSON.stringify(zero({ok:true,status:'POST_GO_LIVE_PRODUCT_GIT_STAGE_SELFTEST_PASS',classification:'PASS',transitionId:TRANSITION,gitCommitBindingRequired:true,stagedBaseMayPrecedeLedgerClaimCommits:true,perFileBlobBindingRequired:true,validatorChildClassificationPreserved:true,validatorChildStatusPreserved:true,validatorChildCodePreserved:true,sourceOnly:true}),null,2));process.exit(0);}
try{
