...
try {
  ...
  add('AUTHORIZATION_ACTIVE', lifecycle.authorization?.requiredForExecution === true && lifecycle.authorization?.activeRequest === true && lifecycle.authorization?.allowedExecutions === 1 && (lifecycle.authorization?.request === REQUEST || EPHEMERAL_POST_MEMBERSHIP_REQUEST) && lifecycle.authorization?.replayAllowed === false);
  add('REQUEST_SCHEMA', request.schemaVersion === 'orbit360-rc12-rootcause-cumulative-closure-request-v1' && request.status === 'AUTHORIZED_SINGLE_MACRO' && request.approved === true && request.allowedExecutions === 1 && request.consumed === false && request.retryAuthorized === false && (!EPHEMERAL_POST_MEMBERSHIP_REQUEST || (request.postRollbackFinal === true && request.parentAuthorizationRef === 'user_authorizes_single_final_post_rollback_rc12_20260804T0734-0600' && request.rollbackRecoveryRun === 30911627137)));
