...
async function sourceOnlySelftest(){
  const contract=staticContract(),tmp=fs.mkdtempSync(path.join(os.tmpdir(),'orbit360-p2-'));try{
    const candidate=await materializeCandidate(tmp,contract),target=targetFrom(candidate),reach=await publicReachability(target),plan=contract.plan;
    ...
    const ok=candidate.artifactId===Number(contract.L.successorCandidate.artifactId)&&candidate.sourceHead===contract.L.successorCandidate.sourceHead&&candidate.fileCount===Number(contract.L.successorCandidate.fileCount)&&rollbackPreflightPass&&plan.length===9&&reach.reachable;
    const p={schemaVersion:'orbit360-go-live-release-handler-selftest-v1',ok,status:ok?'GO_LIVE_RELEASE_HANDLER_SOURCE_ONLY_PASS':'GO_LIVE_RELEASE_HANDLER_SOURCE_ONLY_FAIL',classification:ok?'PASS':(reach.reachable?'PIPELINE_MECHANISM_FAILURE':'ENVIRONMENT_FAILURE'),handler:HANDLER,handlerReady:contract.s.handlerReady===true,candidateArtifactId:candidate.artifactId,candidateSourceHead:candidate.sourceHead,candidateManifestSha256:candidate.manifestSha256,candidateZipSha256:candidate.zipSha256,fileCount:candidate.fileCount,manifestStatus:candidate.manifest.status,fullRehashPass:true,targetDerivedFromCandidate:true,targetProjectBound:Boolean(target.projectId),targetTenantBound:Boolean(target.tenantId),publicEndpointReachable:reach.reachable,publicEndpointStatus:reach.status,releasePlan:plan,releasePlanPass:true,hostingOnlyDeploy:true,rulesDeploy:false,rollbackPreflightPass,rollbackAnchorCapturedBeforeRiskContract:true,providerReleaseIdentityDeferredToClaimedReadOnlyPreflight:true,authorizationConsumed:false,privilegedRiskObserved:false,runtimeExecuted:false,browserExecuted:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,authWrites:0,operationalWrites:0,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};handlerSelftestOutput(p);if(!ok)process.exitCode=41;
...
async function fullRemoteReadback(target,candidate){const files=[...candidate.manifest.files.map(x=>({path:x.path,sha256:x.sha256})),{path:'orbit360-package-manifest.json',sha256:candidate.manifestSha256}];let i=0,failed='';async function worker(){while(i<files.length&&!failed){const f=files[i++];try{const r=await fetch(`${target.liveUrl}/${f.path}?orbit360-release=${Date.now()}-${i}`,{headers:{'cache-control':'no-cache','pragma':'no-cache'},redirect:'follow'});if(!r.ok){failed=`${f.path}:HTTP_${r.status}`;break;}const b=Buffer.from(await r.arrayBuffer());if(shaBuf(b)!==f.sha256){failed=`${f.path}:SHA_MISMATCH`;break;}}catch{failed=`${f.path}:FETCH_FAILED`;break;}}}await Promise.all(Array.from({length:8},worker));if(failed)fail('PIPELINE_MECHANISM_FAILURE:RELEASE_REMOTE_FULL_REHASH_FAIL',failed);return {fileCount:files.length,fullRemoteRehashPass:true};}
...
async function actualRelease(){
  ...
  try{
    ...
    runTool(HOSTING_HELPER,['smoke'],env,'PIPELINE_MECHANISM_FAILURE:RELEASE_HOSTING_READBACK_FAILED');await fullRemoteReadback(target,candidate);
    ...
    const terminal={schemaVersion:'orbit360-go-live-release-terminal-v1',transitionId:'GO_LIVE_RELEASE_WINDOW',runId,ok:true,status:'PRODUCTION_SMOKE_PASS',classification:'PASS',failureCode:null,candidateArtifactId:candidate.artifactId,candidateSourceHead:candidate.sourceHead,candidateManifestSha256:candidate.manifestSha256,candidateZipSha256:candidate.zipSha256,target:{projectBound:true,tenantBound:true,siteBound:true},privilegedRiskObserved:true,deployExecuted:true,productionTouched:true,productionSmokePass:true,rollbackRequired:false,rollbackExecuted:false,rollbackSucceeded:false,firestoreRead:true,firestoreWrites:0,authWrites:0,operationalWrites:0,rulesDeploy:false,dataWrites:false,containsPII:false,containsSecrets:false,evidencePath};writeJson(terminalPath,terminal);console.log(JSON.stringify(terminal,null,2));return;
