...
async function sourceOnlySelftest(){
  const contract=staticContract(),tmp=fs.mkdtempSync(path.join(os.tmpdir(),'orbit360-p2-'));try{
    ...
    const browserBinderBeforeDecisionPass=browserStart>=0&&binderStart>browserStart&&normalizedDecision>binderStart&&!handlerSource.includes("if(br.status!==0)fail('FUNCTIONAL_DEFECT:RELEASE_PRODUCTION_BROWSER_SMOKE_FAILED'");
    ...
    const p={schemaVersion:'orbit360-go-live-release-handler-selftest-v3-browser-binder-before-decision',ok,status:ok?'GO_LIVE_RELEASE_HANDLER_SOURCE_ONLY_PASS':'GO_LIVE_RELEASE_HANDLER_SOURCE_ONLY_FAIL',classification:ok?'PASS':(reach.reachable?'PIPELINE_MECHANISM_FAILURE':'ENVIRONMENT_FAILURE'),handler:HANDLER,handlerReady:contract.s.handlerReady===true,candidateArtifactId:candidate.artifactId,candidateSourceHead:candidate.sourceHead,candidateManifestSha256:candidate.manifestSha256,candidateZipSha256:candidate.zipSha256,fileCount:candidate.fileCount,manifestStatus:candidate.manifest.status,fullRehashPass:true,hostingManifestPartitionPass:partition.partitionPass,hostingPublicFileCount:partition.publicFileCount,hostingExcludedFileCount:partition.excludedFileCount,hostingExcludedPaths:partition.excludedFiles.map(f=>f.path),internalDocsProtected:partition.internalDocsProtected,excludedFilesRemainNonPublicContract:excludedCheck.excludedNotPublic,targetDerivedFromCandidate:true,targetProjectBound:Boolean(target.projectId),targetTenantBound:Boolean(target.tenantId),publicEndpointReachable:reach.reachable,publicEndpointStatus:reach.status,releasePlan:plan,releasePlanPass:true,hostingOnlyDeploy:true,rulesDeploy:false,rollbackPreflightPass,rollbackAnchorCapturedBeforeRiskContract:true,browserBinder:BROWSER_BINDER,browserBinderBeforeDecisionPass,decisionUsesNormalizedBrowserEvidence:true,providerReleaseIdentityDeferredToClaimedReadOnlyPreflight:true,authorizationConsumed:false,privilegedRiskObserved:false,runtimeExecuted:false,browserExecuted:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,authWrites:0,operationalWrites:0,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};handlerSelftestOutput(p);if(!ok)process.exitCode=41;
