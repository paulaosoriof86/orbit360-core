...
const BROWSER_BINDER='tools/orbit360-bind-f2-browser-evidence-run-v20260824.mjs';
...
function staticContract(){
  for(const p of [LEDGER,REGISTRY,HANDLER,HOSTING_HELPER,IDENTITY_HELPER,INTEGRITY_SNAPSHOT,INTEGRITY_COMPARE,BROWSER_SMOKE,BROWSER_BINDER,FIREBASE_GO_LIVE,FIRESTORE_READONLY,FIREBASE_ROLLBACK])if(!fs.existsSync(A(p)))fail('PIPELINE_MECHANISM_FAILURE:RELEASE_HANDLER_DEPENDENCY_MISSING',p);
...
async function sourceOnlySelftest(){
  const contract=staticContract(),tmp=fs.mkdtempSync(path.join(os.tmpdir(),'orbit360-p2-'));try{
    ...
    const helperSource=fs.readFileSync(A(HOSTING_HELPER),'utf8'),handlerSource=fs.readFileSync(A(HANDLER),'utf8');
    ...
    const browserStart=handlerSource.indexOf("await runAsync('node',[BROWSER_SMOKE]");
    const binderStart=handlerSource.indexOf("run('node',[BROWSER_BINDER]");
    const normalizedDecision=handlerSource.indexOf("if(boundBrowser.ok!==true)");
    const preBinderSegment=browserStart>=0&&binderStart>browserStart?handlerSource.slice(browserStart,binderStart):'';
    const rawExitTerminalBeforeBinder=/br\.status\s*!==\s*0/.test(preBinderSegment);
    const browserBinderBeforeDecisionPass=browserStart>=0&&binderStart>browserStart&&normalizedDecision>binderStart&&!rawExitTerminalBeforeBinder;
    const ok=candidate.artifactId===Number(contract.L.successorCandidate.artifactId)&&candidate.sourceHead===contract.L.successorCandidate.sourceHead&&candidate.fileCount===Number(contract.L.successorCandidate.fileCount)&&partition.partitionPass&&partition.internalDocsProtected&&partition.publicFileCount>0&&partition.excludedFileCount>0&&excludedCheck.excludedNotPublic&&rollbackPreflightPass&&browserBinderBeforeDecisionPass&&plan.length===9&&reach.reachable;
    const p={schemaVersion:'orbit360-go-live-release-handler-selftest-v4-browser-binder-structural-segment',ok,status:ok?'GO_LIVE_RELEASE_HANDLER_SOURCE_ONLY_PASS':'GO_LIVE_RELEASE_HANDLER_SOURCE_ONLY_FAIL',classification:ok?'PASS':(reach.reachable?'PIPELINE_MECHANISM_FAILURE':'ENVIRONMENT_FAILURE'),handler:HANDLER,handlerReady:contract.s.handlerReady===true,candidateArtifactId:candidate.artifactId,candidateSourceHead:candidate.sourceHead,candidateManifestSha256:candidate.manifestSha256,candidateZipSha256:candidate.zipSha256,fileCount:candidate.fileCount,manifestStatus:candidate.manifest.status,fullRehashPass:true,hostingManifestPartitionPass:partition.partitionPass,hostingPublicFileCount:partition.publicFileCount,hostingExcludedFileCount:partition.excludedFileCount,hostingExcludedPaths:partition.excludedFiles.map(f=>f.path),internalDocsProtected:partition.internalDocsProtected,excludedFilesRemainNonPublicContract:excludedCheck.excludedNotPublic,targetDerivedFromCandidate:true,targetProjectBound:Boolean(target.projectId),targetTenantBound:Boolean(target.tenantId),publicEndpointReachable:reach.reachable,publicEndpointStatus:reach.status,releasePlan:plan,releasePlanPass:true,hostingOnlyDeploy:true,rulesDeploy:false,rollbackPreflightPass,rollbackAnchorCapturedBeforeRiskContract:true,browserBinder:BROWSER_BINDER,browserBinderBeforeDecisionPass,rawExitTerminalBeforeBinder,decisionUsesNormalizedBrowserEvidence:true,providerReleaseIdentityDeferredToClaimedReadOnlyPreflight:true,authorizationConsumed:false,privilegedRiskObserved:false,runtimeExecuted:false,browserExecuted:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,authWrites:0,operationalWrites:0,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};handlerSelftestOutput(p);if(!ok)process.exitCode=41;
...
async function actualRelease(){
  ...
  try{
    ...
    const proxy=await startProxy(target.liveUrl);try{
      ...
      const br=await runAsync('node',[BROWSER_SMOKE],{env:benv});
      if(!fs.existsSync(browserOut))fail('PIPELINE_MECHANISM_FAILURE:RELEASE_PRODUCTION_BROWSER_EVIDENCE_MISSING',safe(br.stderr||br.stdout));
      const bind=run('node',[BROWSER_BINDER],{env:benv,allowFail:true});if(bind.status!==0)fail('PIPELINE_MECHANISM_FAILURE:RELEASE_PRODUCTION_BROWSER_BINDER_FAILED',safe(bind.stderr||bind.stdout));
      const boundBrowser=JSON.parse(fs.readFileSync(browserOut,'utf8'));
      if(Number(boundBrowser.runId||0)!==runId||Number(boundBrowser.browserRunId||0)!==runId||boundBrowser.evidenceFreshness!=='current-run-only')fail('DATA_CONTRACT_FAILURE:RELEASE_PRODUCTION_BROWSER_RUN_BINDING_INVALID');
      if(boundBrowser.ok!==true){const cls=allowedClasses.has(String(boundBrowser.classification||''))?String(boundBrowser.classification):'FUNCTIONAL_DEFECT';fail(`${cls}:RELEASE_PRODUCTION_BROWSER_SMOKE_FAILED`,safe(boundBrowser.error||boundBrowser.status||br.stderr||br.stdout));}
