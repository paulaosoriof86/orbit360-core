...
for(const [k,v] of Object.entries({SOURCE_SHA,PROJECT_ID,HOSTING_SITE,TENANT_HINT})) if(!v) throw new Error('ENV_MISSING:'+k);
...
const requiredSite=['index.html','product-runtime-config.js','sw.js','core/product-app-p0.js','core/pwa.js','core/product-insurer-credential-provider-p0.js','data/tenant-runtime-config-index.js','data/store-firestore-product-readonly-p0.js','data/store-firestore-product-operational-p0.js','core/client-insurer-operational-directory-owner-v20260722.js'];
...
fs.mkdirSync(path.join(SITE,'__recovery__'),{recursive:true});fs.writeFileSync(path.join(SITE,'__recovery__/build.json'),JSON.stringify({product:'Gravicentra Insurance',gate:'I6.1',sourceSha:SOURCE_SHA,buildId,environmentRef:'firebase-hosting-preview-i6-1'})+'\n');
