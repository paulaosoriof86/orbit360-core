...
function subsetEqual(actual,expected){
  ...
  for(const [k,v] of Object.entries(expected)){
    ...
    if(v==='SERVER_TIMESTAMP'){
      if(!(a&&typeof a.toDate==='function')&&!(a instanceof Date)&&!clean(a))return false;
      continue;
    }
    if(JSON.stringify(comparable(a??null))!==JSON.stringify(comparable(v??null)))return false;
  }
  return true;
}

const result={
  schemaVersion:'orbit360-cobros-post-close-relations-readonly-evidence-v1',
  gateId:GATE,
  contractVersion:VERSION,
  tenantId:TENANT,
  projectId:PROJECT,
  status:'STARTED',
  classification:'READ_ONLY_POST_CLOSE_RELATION_VERIFICATION',
  requestVerified:false,
  lifecycleVerified:false,
  packageVerified:false,
  counts:{},
  cases:[],
  totals:{cases:0,direct:0,historical:0,policyRelations:0,receiptRelations:0,authorizationRelations:0,idempotencyRelations:0,policyStatesPreserved:0,noFinmovCases:0},
...
try{
  ...
  if(!Array.isArray(request.authorizationRefs)||request.authorizationRefs.length!==5||new Set(request.authorizationRefs).size!==5)fail('DATA_CONTRACT_FAILURE','REQUEST_REFS');
  result.requestVerified=true;

  const lifecycle=JSON.parse(fs.readFileSync(lifecyclePath,'utf8'));
  if(lifecycle.gateId!==GATE||lifecycle.gateContractVersion!==VERSION||lifecycle.status!=='POST_CLOSE_READONLY_VERIFICATION_ACTIVE'||lifecycle.executionProfile?.mode!=='READ_ONLY_POST_CLOSE_VERIFICATION'||lifecycle.executionProfile?.capabilities?.writes!==false||lifecycle.writeAuthorized!==false||lifecycle.requestSemanticallyConsumed!==true||lifecycle.requestFileMutationProhibited!==true||lifecycle.additionalExecutionProhibited!==true||lifecycle.productionAuthorized!==false||lifecycle.deployAuthorized!==false)fail('SECURITY_FAILURE','LIFECYCLE_NOT_SEALED_READONLY');
  result.lifecycleVerified=true;result.requestReplayBlocked=true;

  const packageBytes=fs.readFileSync(packagePath);const physicalSha=sha256(packageBytes);
  if(physicalSha!==expectedPackageSha||physicalSha!==request.privatePackage.sha256)fail('DATA_CONTRACT_FAILURE','PRIVATE_PACKAGE_SHA');
  const pkg=JSON.parse(packageBytes.toString('utf8'));
  if(pkg.schemaVersion!=='orbit360-cobros-controlled-write-private-package-v1'||pkg.gateId!==GATE||pkg.contractVersion!==VERSION||pkg.tenantId!==TENANT||pkg.projectId!==PROJECT||!Array.isArray(pkg.cases)||pkg.cases.length!==5)fail('DATA_CONTRACT_FAILURE','PRIVATE_PACKAGE_SCOPE');
  ...
  if(new Set(pkg.cases.map(x=>x.authorizationRef)).size!==5||new Set(pkg.cases.map(x=>x.idempotencyKey)).size!==5||new Set(pkg.cases.map(x=>x.cobroId)).size!==5||new Set(pkg.cases.map(x=>x.receiptId)).size!==5)fail('DATA_CONTRACT_FAILURE','PRIVATE_PACKAGE_UNIQUENESS');
  ...
  for(let i=0;i<pkg.cases.length;i++){
    const c=pkg.cases[i];
    const [policySnap,receiptSnap,cobroSnap]=await Promise.all([docRef(db,'polizas',c.policyId).get(),docRef(db,'recibosEsperados',c.receiptId).get(),docRef(db,'cobros',c.cobroId).get()]);
    if(!policySnap.exists||!receiptSnap.exists||!cobroSnap.exists)fail('DATA_CONTRACT_FAILURE','RELATED_DOCUMENT_MISSING_CASE_'+(i+1));
    const policy=policySnap.data(),receipt=receiptSnap.data(),cobro=cobroSnap.data();
    const policySnapshotOk=subsetEqual(policy,c.expectedPolicy);
    const cobroPayloadOk=subsetEqual(cobro,c.cobro);
    const receiptPayloadOk=c.category==='EXISTING_CANONICAL_RECEIPT'
      ? subsetEqual(receipt,{...c.expectedReceipt,...c.receiptPatch})
      : c.category==='HISTORICAL_RECEIPT_REINFORCED'&&subsetEqual(receipt,c.historicalReceipt);
    const policyRelationOk=cobro.polizaId===c.policyId&&receipt.polizaId===c.policyId;
    const receiptRelationOk=cobro.reciboId===c.receiptId&&receipt.id===c.receiptId&&receipt.cobroId===c.cobroId;
    const authorizationOk=cobro.authorizationRef===c.authorizationRef&&receipt.authorizationRef===c.authorizationRef&&request.authorizationRefs.includes(c.authorizationRef);
    const idempotencyOk=cobro.idempotencyKey===c.idempotencyKey&&receipt.idempotencyKey===c.idempotencyKey;
    ...
    const ok=policySnapshotOk&&cobroPayloadOk&&receiptPayloadOk&&policyRelationOk&&receiptRelationOk&&authorizationOk&&idempotencyOk&&controlsOk&&historicalOk;
    if(!ok)fail('DATA_CONTRACT_FAILURE','RELATION_INVALID_CASE_'+(i+1));
    result.cases.push({ordinal:i+1,category:c.category,policyExists:true,receiptExists:true,cobroExists:true,policySnapshotOk,cobroPayloadOk,receiptPayloadOk,policyRelationOk,receiptRelationOk,authorizationOk,idempotencyOk,controlsOk,historicalOk,noFinmov:true,ok:true});
    result.totals.cases++;if(c.category==='EXISTING_CANONICAL_RECEIPT')result.totals.direct++;else result.totals.historical++;
    result.totals.policyRelations++;result.totals.receiptRelations++;result.totals.authorizationRelations++;result.totals.idempotencyRelations++;result.totals.policyStatesPreserved++;result.totals.noFinmovCases++;
  }
  if(result.totals.cases!==5||result.totals.direct!==4||result.totals.historical!==1||result.totals.policyRelations!==5||result.totals.receiptRelations!==5||result.totals.authorizationRelations!==5||result.totals.idempotencyRelations!==5||result.totals.policyStatesPreserved!==5||result.totals.noFinmovCases!==5)fail('DATA_CONTRACT_FAILURE','POST_CLOSE_TOTALS');
  result.status='POST_CLOSE_RELATIONS_PASS';result.classification='GO_LAB_COBROS_POST_CLOSE_RELATIONS_READONLY';result.ok=true;
