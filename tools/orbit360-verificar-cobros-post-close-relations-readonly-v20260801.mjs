...
function isPlainObject(v){return Boolean(v)&&typeof v==='object'&&!Array.isArray(v)&&typeof v.toDate!=='function'&&!(v instanceof Date);}
function fieldEqual(actualValue,expectedValue){
  if(expectedValue==='SERVER_TIMESTAMP')return Boolean((actualValue&&typeof actualValue.toDate==='function')||(actualValue instanceof Date)||clean(actualValue));
  if(isPlainObject(expectedValue)){if(!isPlainObject(actualValue))return false;return Object.entries(expectedValue).every(([k,v])=>fieldEqual(actualValue[k],v));}
  return JSON.stringify(comparable(actualValue??null))===JSON.stringify(comparable(expectedValue??null));
}
...
try{
  ...
  for(let i=0;i<pkg.cases.length;i++){
    ...
    const expectedReceiptAfter=c.category==='EXISTING_CANONICAL_RECEIPT'?{...c.expectedReceipt,...c.receiptPatch}:c.historicalReceipt;
    const policyMismatchedKeys=mismatchKeys(policy,c.expectedPolicy);const cobroMismatchedKeys=mismatchKeys(cobro,c.cobro);const receiptMismatchedKeys=mismatchKeys(receipt,expectedReceiptAfter);
    const policySnapshotOk=policyExists&&policyMismatchedKeys.length===0;const cobroPayloadOk=cobroExists&&cobroMismatchedKeys.length===0;const receiptPayloadOk=receiptExists&&receiptMismatchedKeys.length===0;
    ...
    result.cases.push({ordinal:i+1,category:c.category,policyExists,receiptExists,cobroExists,policySnapshotOk,policyMismatchedKeys,cobroPayloadOk,cobroMismatchedKeys,receiptPayloadOk,receiptMismatchedKeys,policyRelationOk,receiptRelationOk,authorizationOk,idempotencyOk,controlsOk,historicalOk,noFinmov:result.counts.finmovs===0,ok});
