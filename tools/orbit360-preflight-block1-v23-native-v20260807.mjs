...
function fail(code, detail, canonicalStatus = '') {
  const output = {
    schemaVersion: 'orbit360-v23-native-block1-preflight-v1', gateId: GATE_ID, contractVersion: CONTRACT_VERSION,
    mode: MODE, status: 'VALIDATOR_STALE', classification: 'PIPELINE_MECHANISM_FAILURE', failed: 1,
...
for (const file of [OVERLAY, LIFECYCLE, CANONICAL_PREFLIGHT, MATRIX, OBSERVER, ADJUDICATOR, SOURCE_TEST]) {
...
if (overlay.gateId !== GATE_ID || overlay.contractVersion !== CONTRACT_VERSION || overlay.block !== 1) fail('V23_GATE_OVERLAY_MISMATCH', 'gate/version/block', 'PASS');
if (!overlay.mechanism || overlay.mechanism.nativeRuntimeArtifact !== MATRIX || overlay.mechanism.generatedFromPriorArtifact !== false || overlay.mechanism.textualTransform !== false || overlay.mechanism.sourceSurgery !== false) fail('V23_NATIVE_MECHANISM_CONTRACT_INVALID', '', 'PASS');
...
if (MODE === 'source') {
  ...
  const output = {
    schemaVersion: 'orbit360-v23-native-block1-preflight-v1', gateId: GATE_ID, contractVersion: CONTRACT_VERSION,
    mode: MODE, status: 'PASS_V23_SOURCE_PREFLIGHT', classification: 'PIPELINE_MECHANISM_FAILURE_CORRECTED_SOURCE_ONLY', failed: 0,
    canonicalPreflightStatus: 'PASS', nativeRuntimeArtifact: MATRIX, exactBlockingRoutes: ['inicio','cliente360','aseguradoras'],
    executionAuthorized: false, secretAccessAuthorized: false, firestoreReadAuthorized: false, writeAuthorized: false,
    runtimeAuthorized: false, browserAuthorized: false, hostingDeployAuthorized: false, hostingDeploysMaximum: 0,
    functionsDeployAuthorized: false, rulesDeployAuthorized: false, productionAuthorized: false,
    secretsRead: false, firestoreRead: false, runtimeExecuted: false, browserExecuted: false, deployExecuted: false,
...
const output = {
  ...
  canonicalPreflightStatus: 'PASS', requestVersion: REQUEST_VERSION, nativeRuntimeArtifact: MATRIX,
