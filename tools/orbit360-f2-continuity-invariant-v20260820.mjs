...
const activeProjection={
  ledger:{stateVersion:L.stateVersion,activeState:L.activeState,nextAction:L.nextAction,authorizationBoundary:L.authorizationBoundary,continuityControl:L.continuityControl},
  live:stripHistory(live),
  index:stripHistory(idx),
  lifecycle:stripHistory(life)
};
const activeText=JSON.stringify(activeProjection);
const ordinalFree=!/(?:REQUEST|\bR)\d+/i.test(activeText);
const historicalArtifactAbsentFromActive=!activeText.includes(String(c.historicalArtifactId));
const guardKeys=Object.keys(life.guards||{});
const forbiddenGuardKeys=['candidateArtifactId','candidateZipSha256','candidateManifestSha256','candidateSourceHead','historicalCandidateArtifactId','request13MaterializationAllowed'];
const legacyGuardKeysAbsent=forbiddenGuardKeys.every(k=>!guardKeys.includes(k))&&!guardKeys.some(k=>/^request\d+/i.test(k));
const successorGuards=life.guards?.successorCandidateRequired===true&&life.guards?.successorCandidateArtifactId===null&&life.guards?.successorCandidateSourceHead===null&&life.guards?.candidateCertificationRequired===true&&life.guards?.historicalCandidateRuntimeReuseAllowed===false;
const sourcePrerequisiteDecoupled=!Object.prototype.hasOwnProperty.call(life,'sourceOnlyPrerequisite')&&life.successorSourceOnlyPrerequisite?.successorCandidateRequired===true&&life.successorSourceOnlyPrerequisite?.successorCandidateArtifactId===null&&life.successorSourceOnlyPrerequisite?.runtimeAllowed===false;
...
const checks={
  ...
  indexProjection:idx.canonicalCurrent?.stateVersion===L.stateVersion&&idx.canonicalCurrent?.status===L.activeState.status&&idx.operationalCurrent?.nextActionId===L.nextAction.id&&idx.operationalCurrent?.successorCandidateArtifactId===null,
  ...
  readmeCurrent:readme.includes('StateVersion: `ORBIT360-F2-CONTINUITY-CURRENT`')&&readme.includes('identificadores de artifact consumidos pertenecen únicamente a historia/evidencia')&&!readme.includes('Request12'),
  changelogCurrent:change.includes('[F2-CONTINUITY-CURRENT]')&&change.includes('guards activos desacoplados'),
  prStateCurrent:prState.includes('ORBIT360-F2-CONTINUITY-CURRENT')&&prState.includes('no depende del ordinal de ningún Request ni de una candidata histórica'),
  prAvailable:!requirePr||pr.length>0,
  prCurrent:!requirePr||(pr.includes('ORBIT360-F2-CONTINUITY-CURRENT')&&pr.includes('no depende del ordinal de ningún Request ni de una candidata histórica')&&pr.includes(L.nextAction.id))
};
const failed=Object.entries(checks).filter(([,v])=>!v).map(([k])=>k),ok=failed.length===0;
const evidence={schemaVersion:'orbit360-f2-continuity-audit-v3',ok,status:ok?'ORBIT360_CONTINUITY_ACTIVE_GUARDS_AUDIT_PASS':'ORBIT360_CONTINUITY_ACTIVE_GUARDS_AUDIT_FAIL',classification:ok?'PASS':'PIPELINE_MECHANISM_FAILURE',code:ok?null:'CONTINUITY_ACTIVE_STATE_OR_GUARD_DECOUPLING_FAILURE',stateVersion:L.stateVersion,activeStateOrdinalFree:ordinalFree,historicalArtifactAbsentFromActive,lifecycleLegacyGuardKeysAbsent:legacyGuardKeysAbsent,successorCandidateArtifactId:life.guards?.successorCandidateArtifactId??null,historicalRequestOrdinal:h.requestOrdinal,historicalRunId:h.runId,historicalArtifactId:c.historicalArtifactId,rootCauseCode:L.sourceRootCauseResolution.code,writerOffenders:offenders,checks,failed,productMutation:false,dataMutation:false,runtimeExecuted:false,browserExecuted:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,authWrites:0,operationalWrites:0,deployExecuted:false,publicationExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};
