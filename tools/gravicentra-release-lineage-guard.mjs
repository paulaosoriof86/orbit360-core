...
const git=(...args)=>execFileSync('git',args,{encoding:'utf8'}).trim(); const read=p=>fs.readFileSync(p,'utf8');
...
if(pending){
  ...
  need(t.previewCallable==='orbit360ProductInsurerCredentialCommandPreview','PENDING_PREVIEW_CALLABLE_INVALID');
  need(t.previewRegion==='us-east1','PENDING_PREVIEW_REGION_INVALID');
  ...
  need(t.previewOperationalWrites===false,'PENDING_PREVIEW_WRITE_BOUNDARY_INVALID');
  need(t.productionBackendTouched===false,'PENDING_PRODUCTION_BACKEND_TOUCHED');
}else need(LOCK.status!=='CANDIDATE_SOURCE_PENDING_I3','CERTIFIED_STATE_STILL_PENDING_I3');
...
function exactEnvPin(file,key){const text=read(file),re=new RegExp('^  '+key+': (.+)$','gm'),hits=[...text.matchAll(re)].map(m=>m[1].trim());need(hits.length===1,'CONTROL_PLANE_PIN_CARDINALITY_FAIL:'+file+':'+key+':'+hits.length);return hits[0];}
function validateControlPlane(){
  ...
  need(i3text.includes('functions:$PREVIEW_FUNCTION'),'I3_PREVIEW_FUNCTION_DEPLOY_MISSING');
  need(!i3text.includes('functions:$PROD_FUNCTION'),'I3_PRODUCTION_FUNCTION_DEPLOY_FORBIDDEN');
  need(exactEnvPin(i3,'PREVIEW_FUNCTION')==='orbit360ProductInsurerCredentialCommandPreview','I3_PREVIEW_FUNCTION_PIN_INVALID');
  need(exactEnvPin(i3,'PREVIEW_REGION')==='us-east1','I3_PREVIEW_REGION_PIN_INVALID');
  need(exactEnvPin(i3,'PROD_FUNCTION')==='orbit360ProductInsurerCredentialCommand','I3_PRODUCTION_FUNCTION_PIN_INVALID');
  need(exactEnvPin(i3,'PROD_REGION')==='us-central1','I3_PRODUCTION_REGION_PIN_INVALID');
...
async function validatePreview(){const url=String(cert.previewUrl).replace(/\/$/,'')+'/__recovery__/build.json?releaseLock='+Date.now();const res=await fetch(url,{headers:{'cache-control':'no-cache','accept-encoding':'identity','user-agent':'Gravicentra-Release-Lineage-Guard/3.0'}});need(res.ok,'RELEASE_LOCK_PREVIEW_MARKER_HTTP_'+res.status);const marker=await res.json();need(marker&&marker.sourceSha===cert.sourceSha,'RELEASE_LOCK_PREVIEW_SOURCE_MISMATCH');need(marker&&marker.buildId===cert.buildId,'RELEASE_LOCK_PREVIEW_BUILD_MISMATCH');}
...
const exportsMap={SOURCE_SHA:source,BUILD_ID:String(cert.buildId),PREVIEW_URL:String(cert.previewUrl),HOSTED_PAYLOAD_DIGEST:String(cert.hostedPayloadDigest),BUNDLE_DIGEST:String(cert.bundleDigest),I3_RUN_ID:String(cert.i3RunId),I3_ARTIFACT_ID:String(cert.artifactId)};
for(const [k,v] of Object.entries(exportsMap)){if(process.env.GITHUB_ENV)fs.appendFileSync(process.env.GITHUB_ENV,`${k}=${v}\n`);if(process.env.GITHUB_OUTPUT)fs.appendFileSync(process.env.GITHUB_OUTPUT,`${k.toLowerCase()}=${v}\n`);}
console.log('GRAVICENTRA_RELEASE_LINEAGE_GUARD=PASS');console.log('MODE='+MODE);console.log('CANDIDATE_SOURCE_SHA='+candidate);console.log('CERTIFIED_SOURCE_SHA='+certified);console.log('PENDING_I3='+pending);console.log('DRIFT_BASE_SOURCE_SHA='+source);console.log('POST_SOURCE_CHANGED_FILES='+changed.length);console.log('PRODUCTION_TOUCHED=false');console.log('DATA_TOUCHED=false');
