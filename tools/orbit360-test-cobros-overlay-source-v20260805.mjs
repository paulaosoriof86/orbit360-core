...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'orbit360-cobros-overlay-'));
...
try {
  ...
  const planillaRows = [
    ...
    { Póliza: 'POL-131', Moneda: 'GTQ', Periodo: '2026-06', 'Prima neta': -25, 'Decisión CRM': 'CANDIDATE_REVERSAL' },
    { Póliza: 'POL-132', Moneda: 'GTQ', Periodo: '2026-06', 'Prima neta': 0, 'Decisión CRM': 'OMIT_ZERO_COMMISSION' },
    { Póliza: 'POL-133', Moneda: 'GTQ', Periodo: '2026-06', 'Prima neta': 80, 'Decisión CRM': 'CANDIDATE_PERIOD_ONLY' }
  ];
  ...
  fs.writeFileSync(planillaFile, JSON.stringify({ rows: planillaRows }), 'utf8');
  ...
  const evidence = {
    schemaVersion: 'orbit360-cobros-overlay-source-test-v1',
    status: failedCheckIds.length ? 'FAIL_COBROS_OVERLAY_SOURCE_ONLY' : 'PASS_COBROS_OVERLAY_SOURCE_ONLY',
