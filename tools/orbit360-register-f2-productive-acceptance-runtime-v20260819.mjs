...
need(req.schemaVersion===REQUEST_SCHEMA&&req.status==='RUNTIME_ATTEMPT_RESERVED_PREFLIGHT_PENDING'&&req.allowedExecutions===1&&req.consumed===false&&req.authorizationFrozen===true&&req.replayAllowed===false&&req.historical===false&&req.runtimeAttemptAccepted===true&&req.runtimeAttemptReserved===true&&req.privilegedRiskBoundaryEntered===false&&Number(req.runtimeAttemptCount)===0,'F2_RUNTIME_REGISTER_REQUEST_NOT_RESERVED_UNCONSUMED_V4');
...
need(ledger.authorizationBoundary?.activeRequestPath===REQUEST&&ledger.authorizationBoundary?.runtimeAttemptAccepted===true&&ledger.authorizationBoundary?.privilegedRiskBoundaryEntered!==true&&Number(ledger.authorizationBoundary?.runtimeRunId)===runId,'F2_RUNTIME_REGISTER_LEDGER_RESERVED_BINDING_INVALID');
...
console.log(JSON.stringify({
  ...
  runtimeAttemptReserved:true,
  oneShotConsumed:false,
  allowedExecutions:1,
  privilegedRiskBoundaryEntered:false,
