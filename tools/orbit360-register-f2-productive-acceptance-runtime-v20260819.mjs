...
need(req.schemaVersion===REQUEST_SCHEMA&&req.status==='MATERIALIZED_SOURCE_ONLY_AWAITING_PREFLIGHT'&&req.allowedExecutions===1&&req.consumed===false&&req.authorizationFrozen===true&&req.replayAllowed===false&&req.historical===false,'F2_RUNTIME_REGISTER_REQUEST_NOT_ACTIVE_V3');
...
life.status='AUTHORIZED_ONCE_RUNTIME_REGISTERED_TRANSIENT_V3';
...
const newBlock=`    if (GATE_ID===F2_GATE_ID && expectedRequestVersion==='${EXPECTED}') {\n      if (request.schemaVersion!=='${REQUEST_SCHEMA}'||request.status!=='MATERIALIZED_SOURCE_ONLY_AWAITING_PREFLIGHT'||request.allowedExecutions!==1||request.consumed!==false||request.authorizationFrozen!==true||request.replayAllowed!==false||request.historical!==false) throw new Error('CANONICAL_REQUEST_NOT_ACTIVE_V3');\n    } else if (request.status!=='AUTHORIZED_ONCE'||request.allowedExecutions!==1||request.consumed!==false||request.authorizationFrozen!==false||request.replayAllowed!==false) throw new Error('CANONICAL_REQUEST_NOT_ACTIVE');`;
