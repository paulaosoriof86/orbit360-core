...
for(const name of files){const rel='.github/workflows/'+name,text=fs.readFileSync(path.join(DIR,name),'utf8'),signals=[];
 ...
 if(unique.length)inspected.push({path:rel,signals:unique,remoteMutation,chaining});if(rel!==CANONICAL&&remoteMutation.length)offenders.push({path:rel,reason:'UNAUTHORIZED_REMOTE_MUTATOR',signals:unique});if(chaining.length)offenders.push({path:rel,reason:'WORKFLOW_CHAINING_FORBIDDEN',signals:unique});
 if(rel===CANONICAL){
...
const canonicalCount=files.filter(name=>'.github/workflows/'+name===CANONICAL).length;if(files.length!==1||canonicalCount!==1)offenders.push({path:CANONICAL,reason:'SINGLE_WORKFLOW_INVARIANT_BROKEN',totalWorkflowFiles:files.length});
const result={schemaVersion:'orbit360-workflow-control-surface-audit-v5-semantic',ok:offenders.length===0,status:offenders.length?'WORKFLOW_CONTROL_SURFACE_AUDIT_FAIL':'WORKFLOW_CONTROL_SURFACE_AUDIT_PASS',canonicalWorkflow:CANONICAL,totalWorkflowFiles:files.length,workflowsWithSignals:inspected.length,unauthorizedControlWorkflows:offenders.length,offenders,inspected,rule:'Exactly one base-owned generic intent workflow may mutate branch/PR. Execution PR changes one intent JSON only. Gate/provider order is validated by step IDs and explicit dependency; artifact/authorization may not be hardcoded in workflow.',runtimeExecuted:false,browserExecuted:false,secretAccess:false,firestoreRead:false,operationalWrites:0,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};console.log(JSON.stringify(result,null,2));if(!result.ok)process.exit(41);
