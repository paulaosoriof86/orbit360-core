...
for(const name of files){
 ...
 const remoteMutation=unique.filter(x=>['contents:write','actions:write','git-push','gh-pr-mutation','workflow-dispatch-command','contents-api-mutation','git-ref-api-mutation'].includes(x));
 const chaining=unique.filter(x=>['actions:write','workflow-dispatch-event','workflow-run-event','workflow-dispatch-command'].includes(x));
 if(unique.length)inspected.push({path:rel,signals:unique,remoteMutation,chaining});
 if(rel!==CANONICAL&&remoteMutation.length)offenders.push({path:rel,reason:'UNAUTHORIZED_REMOTE_MUTATOR',signals:unique,remoteMutation,chaining});
 if(chaining.length)offenders.push({path:rel,reason:'WORKFLOW_CHAINING_FORBIDDEN',signals:unique,remoteMutation,chaining});
}
const canonicalCount=files.filter(name=>'.github/workflows/'+name===CANONICAL).length;
if(files.length!==1||canonicalCount!==1)offenders.push({path:CANONICAL,reason:'SINGLE_WORKFLOW_INVARIANT_BROKEN',signals:[],remoteMutation:[],chaining:[],totalWorkflowFiles:files.length});
const result={schemaVersion:'orbit360-workflow-control-surface-audit-v4',ok:offenders.length===0,status:offenders.length?'WORKFLOW_CONTROL_SURFACE_AUDIT_FAIL':'WORKFLOW_CONTROL_SURFACE_AUDIT_PASS',canonicalWorkflow:CANONICAL,totalWorkflowFiles:files.length,workflowsWithSignals:inspected.length,unauthorizedControlWorkflows:offenders.length,offenders,inspected,rule:'Exactly one canonical workflow may mutate branch/PR. Cross-workflow dispatch, workflow_run chaining and actions:write are forbidden. Macro-3 runtime must execute inline after one-shot accept.',runtimeExecuted:false,browserExecuted:false,secretAccess:false,firestoreRead:false,operationalWrites:0,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};
