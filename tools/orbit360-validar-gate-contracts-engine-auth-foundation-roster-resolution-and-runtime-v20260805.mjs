...
try {
  ...
  add('STORE_AUTHORITY', store.includes("'asesores'") && store.includes("return CANONICAL_SET.has(collection)\n      ? `tenants/${tenantId}/data/${collection}/items`\n      : `tenantId/${tenantId}/${collection}`") && !store.slice(store.indexOf('const CANONICAL_COLLECTIONS'), store.indexOf('];', store.indexOf('const CANONICAL_COLLECTIONS'))).includes("'asesores'"));
