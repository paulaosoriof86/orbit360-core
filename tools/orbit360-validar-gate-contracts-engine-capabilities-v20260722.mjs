...
function finish(status, checks, exitCode, gate, overlay, metadata = {}) {
  ...
  const payload = {
    schemaVersion:'orbit360-gate-contract-preflight-v13-phase-capabilities',
...
const owners = replaceOwners ? [].concat(overlay.canonicalOwners || []) : (() => {
...
const requiredFiles = replaceRequiredFiles ? unique(overlay.requiredFiles) : unique([...(gate.requiredFiles || []), ...(overlay.requiredFiles || [])]);
const replaceRuntimeContracts = overlay.replaceRuntimeVersionContracts === true;
const runtimeContracts = replaceRuntimeContracts ? [].concat(overlay.runtimeVersionContracts || []) : mergeContracts(gate.runtimeVersionContracts || [], overlay.runtimeVersionContracts || []);
...
const expectedCapabilities = PHASE_PROFILES[executionPhase] || null;
...
check('EXECUTION_PHASE_ALLOWED', Boolean(expectedCapabilities), executionPhase || 'missing');
check('EXECUTION_BRANCH_LOCK_DECLARED', workflowLocks.branch === true, JSON.stringify(workflowLocks));
if (expectedCapabilities) {
  for (const [key, expected] of Object.entries(expectedCapabilities)) check(`PHASE_CAPABILITY:${executionPhase}:${key}`, capabilities[key] === expected, `${key}=${String(capabilities[key])};expected=${String(expected)}`);
}
...
if (executionPhase === 'LAB_DATA_CONTRACT_REPAIR_DRYRUN' || executionPhase === 'LAB_DATA_CONTRACT_REPAIR_APPLY') {
  check('DATA_REPAIR_PROJECT_LOCK_ENABLED', workflowLocks.firebaseProject === true, JSON.stringify(workflowLocks));
  check('DATA_REPAIR_CHANNEL_LOCK_DISABLED', workflowLocks.hostingChannel === false, JSON.stringify(workflowLocks));
}
...
if (exists(gate.workflow)) {
  ...
  if (workflowLocks.firebaseProject === true) check('WORKFLOW_PROJECT_LOCK_ENABLED', workflow.includes(gate.environment.firebaseProjectId), gate.environment.firebaseProjectId);
  else check('WORKFLOW_PROJECT_LOCK_NOT_APPLICABLE', !workflow.includes(gate.environment.firebaseProjectId), `phase=${executionPhase}`);
  if (workflowLocks.hostingChannel === true) check('WORKFLOW_CHANNEL_LOCK_ENABLED', workflow.includes(gate.environment.hostingChannel), gate.environment.hostingChannel);
  else check('WORKFLOW_CHANNEL_LOCK_NOT_APPLICABLE', !workflow.includes(gate.environment.hostingChannel), `phase=${executionPhase}`);
  ...
  check('WORKFLOW_NO_REPOSITORY_WRITE', !workflow.includes('git commit') && !workflow.includes('git push'), gate.workflow);
  if (executionPhase === 'STATIC_PREFLIGHT') check('WORKFLOW_STATIC_VALIDATION_ONLY', workflow.includes('VALIDATION_ONLY'), gate.workflow);
  if (executionPhase === 'LAB_DATA_CONTRACT_REPAIR_DRYRUN') {
    check('WORKFLOW_READ_ONLY_DRYRUN', workflow.includes('READ_ONLY_DRY_RUN') && workflow.includes('orbit360-restituir-directorio-operativo-v2-v20260722.mjs') && workflow.includes('--dry-run') && !workflow.includes('--apply'), gate.workflow);
  }
  if (executionPhase === 'LAB_DATA_CONTRACT_REPAIR_APPLY') {
    check('WORKFLOW_ATOMIC_APPLY', workflow.includes('ATOMIC_APPLY') && workflow.includes('orbit360-restituir-directorio-operativo-v2-v20260722.mjs') && workflow.includes('--apply'), gate.workflow);
  }
  if (executionPhase === 'LAB_HOSTING_DELIVERY') check('WORKFLOW_HOSTING_DELIVERY', workflow.includes('HOSTING_DELIVERY'), gate.workflow);
}
...
if (exists(FREEZE_REL)) {
  ...
  if (freeze) {
    ...
    if (executionPhase === 'LAB_DATA_CONTRACT_REPAIR_DRYRUN') check('FREEZE_DRYRUN_AUTHORIZED', freeze.stateClarification && freeze.stateClarification.operationalDirectoryDryRun === 'AUTHORIZED_ONCE' && freeze.stateClarification.operationalDirectoryApply === 'NOT_AUTHORIZED', freeze.status || 'missing');
    if (executionPhase === 'LAB_DATA_CONTRACT_REPAIR_APPLY') check('FREEZE_APPLY_AUTHORIZED', freeze.stateClarification && freeze.stateClarification.operationalDirectoryApply === 'AUTHORIZED_ONCE', freeze.status || 'missing');
  }
}
for (const rel of ['orbit360-platform/data/store.js','orbit360-platform/data/store-firestore-lab.local.js','orbit360-platform/core/auth.js','orbit360-platform/core/importa.js','firestore.rules']) check(`PROTECTED_PATH_PRESENT:${rel}`, exists(rel), rel);
...
const metadata = {
  ...
  capabilityProfile:expectedCapabilities || {}
};
const failed = checks.filter(item => !item.ok);
