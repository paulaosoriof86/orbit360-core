...
const REQUEST_VERSION=process.env.ORBIT360_COBROS_FULL_REQUEST_VERSION||'cobros-full-ledger-write-lab-v20260811-r1';
...
try{
  ...
  if(request.privatePackage?.driveFileId!==PACKAGE_ID||request.privatePackage?.sizeBytes!==PACKAGE_SIZE||request.privatePackage?.expectedReader!==EXPECTED_READER||request.privatePackage?.expectedReaderRole!=='reader'||request.privatePackage?.sha256!==PACKAGE_SHA||request.privatePackage?.logicalSha256!==PACKAGE_LOGICAL||request.privatePackage?.sourceLedgerDigest!==LEDGER_DIGEST)fail('DATA_CONTRACT_FAILURE','REQUEST_PACKAGE');
  result.requestVerified=true;result.checkpoint='REQUEST_VERIFIED';
  const bytes=fs.readFileSync(packagePath);if(bytes.length!==PACKAGE_SIZE)fail('DATA_CONTRACT_FAILURE','PRIVATE_PACKAGE_SIZE');if(sha256(bytes)!==PACKAGE_SHA)fail('DATA_CONTRACT_FAILURE','PRIVATE_PACKAGE_PHYSICAL_SHA');
  ...
  const sa=JSON.parse(process.env.SERVICE_ACCOUNT||'{}');if(sa.project_id!==PROJECT)fail('ENVIRONMENT_FAILURE','PROJECT_ID_MISMATCH');if(sa.client_email!==EXPECTED_READER)fail('ENVIRONMENT_FAILURE','SERVICE_ACCOUNT_IDENTITY_MISMATCH');
