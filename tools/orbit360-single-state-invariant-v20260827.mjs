...
try{
  ...
  if(!globalPg||globalPg.status!=='ACTIVE_SOURCE_ONLY_FAIL_CLOSED'||globalPg.validator!==PRODUCT_PRES||globalPg.registry!==PRODUCT_REG||globalPg.expectedPassStatus!=='CERTIFIED_PRODUCT_BASELINE_PRESERVATION_PASS'||Number(globalPg.baselineArtifactId)!==9504702901||globalPg.baselineSourceHead!=='8c9668d6d423e82826b0295431ec699390d79b4b'||globalPg.baselineManifestSha256!=='b1c98c1faf644b5d83cfe6e6bb1a602927c438cd85b7855cb0b1a6b98c08053c'||Number(globalPg.approvedModuleScriptCount)!==53||globalPg.preflightBeforeIntentInspection!==true||globalPg.productMutationOnFailure!==false||globalPg.dataMutationOnFailure!==false||globalPg.newBaselineRequiresExplicitAcceptance!==true) fail('CERTIFIED_PRODUCT_PRESERVATION_REGISTRY_BINDING_INVALID');
  if(G.status!=='ACTIVE_SOURCE_ONLY_FAIL_CLOSED'||G.baseline?.artifactId!==9504702901||G.baseline?.sourceHead!=='8c9668d6d423e82826b0295431ec699390d79b4b'||G.baseline?.manifestSha256!=='b1c98c1faf644b5d83cfe6e6bb1a602927c438cd85b7855cb0b1a6b98c08053c'||G.baseline?.fileCount!==194||G.approvedModuleScriptCount!==53||G.preservationRule?.historicalPassPlusSameArtifactMeansNoReprocess!==true||G.preservationRule?.visualAnomalyDoesNotAuthorizeRebuild!==true||G.preservationRule?.visualAnomalyDoesNotAuthorizeReimport!==true||G.preservationRule?.productDeltaRequiresNewExplicitlyAcceptedBaseline!==true||G.sourceFreeze?.expectedChangedFilesSinceBaseline!==0) fail('CERTIFIED_PRODUCT_PRESERVATION_SEMANTIC_REGISTRY_INVALID');
  if(!productGuard.includes('CERTIFIED_PRODUCT_BASELINE_PRESERVATION_PASS')||!productGuard.includes("git(['diff','--name-only'")||!productGuard.includes('UNAPPROVED_PRODUCT_SOURCE_DELTA')||!productGuard.includes('approvedModuleScriptCount')) fail('CERTIFIED_PRODUCT_PRESERVATION_GUARD_CONTRACT_INCOMPLETE');
  if(!wf.includes('ORBIT360_CERTIFIED_PRODUCT_PRESERVATION: tools/orbit360-certified-product-preservation-v20260827.mjs')||!wf.includes('CERTIFIED_PRODUCT_BASELINE_PRESERVATION_PASS')||!wf.includes('certified-product-preservation.json')) fail('CERTIFIED_PRODUCT_PRESERVATION_WORKFLOW_NOT_BOUND');
