...
const ROOTCAUSE_EVIDENCE = 'orbit360-platform/runtime-gate-crm-v20260716/block12-layout-probe-rootcause-v20260804.json';
const SOURCE_EVIDENCE = 'orbit360-platform/runtime-gate-crm-v20260716/block12-layoutfree-visual-contract-source-v20260804.json';
...
const REQUIRED = [
  LIFECYCLE, REQUEST, WORKFLOW, VISUAL, VIDEO_EVIDENCE, LAYOUT_EVIDENCE,
  ROOTCAUSE_EVIDENCE, SOURCE_EVIDENCE, PRIOR_FUNCTIONAL,
  'tools/orbit360-block12-visual-readonly-integrity-v20260804.mjs',
  'tools/orbit360-validar-layout-free-probe-sintetico-v20260804.mjs',
...
try {
  ...
  const layoutEvidence = readJson(LAYOUT_EVIDENCE);
  const rootCauseEvidence = readJson(ROOTCAUSE_EVIDENCE);
  ...
  add('LIFECYCLE_ACTIVE', lifecycle.status === 'OPERATIONAL_RUNTIME_LAB_VISUAL_LAYOUTFREE_READY' && lifecycle.singleGate === true && lifecycle.macroClosure === true);
  ...
  add('REQUEST_ACTIVE', request.schemaVersion === 'orbit360-block12-operational-runtime-lab-visual-layoutfree-request-v1' && request.status === 'AUTHORIZED_VISUAL_LAYOUTFREE_REACTIVATION_AFTER_SYNTHETIC_PASS' && request.approved === true && request.allowedExecutions === 1 && request.consumed === false && request.replayAllowed === false && request.retryAuthorized === true && request.previousRuntimeRunId === 30962756387 && request.previousFunctionalStatus === 'PASS' && request.previousFunctionalPassed === 18 && request.previousFunctionalFailed === 0 && request.previousVisualRunId === 30970150174 && request.previousVisualFailureCode === 'ROUTE_POLIZAS_DOM_SNAPSHOT_TIMEOUT' && request.previousVisualFailureClassification === 'PIPELINE_MECHANISM_FAILURE' && request.layoutSyntheticRunId === 30970624742 && request.layoutSyntheticStatus === 'LAYOUT_FREE_PROBE_SYNTHETIC_PASS' && request.videoSyntheticRunId === 30969402434 && request.videoSyntheticStatus === 'VIDEO_FRAME_SYNTHETIC_PASS' && request.functionalReplayForbidden === true && request.authorizationRef === lifecycle.authorization.source);
  ...
  add('EVIDENCE_AND_INTEGRITY', scope.snapshotBeforeAfter === true && scope.readonlyIntegrityBeforeAfter === true && scope.priorFunctionalPassEvidenceRequired === true && scope.functionalReplayForbidden === true && scope.sanitizedEvidence === true && scope.cumulativeVisualCandidate === true && scope.manualFrameReviewRequired === true);
  add('VIDEO_SYNTHETIC_PASS', videoEvidence.status === 'VIDEO_FRAME_SYNTHETIC_PASS' && videoEvidence.captureEngine === 'playwright-record-video-plus-ffmpeg-static-frame' && videoEvidence.deployExecuted === false && videoEvidence.ok === true);
  add('LAYOUT_SYNTHETIC_PASS', layoutEvidence.status === 'LAYOUT_FREE_PROBE_SYNTHETIC_PASS' && layoutEvidence.domNodes === 50000 && layoutEvidence.elapsedMs < 2000 && Array.isArray(layoutEvidence.forbiddenPresent) && layoutEvidence.forbiddenPresent.length === 0 && layoutEvidence.innerTextUsed === false && layoutEvidence.layoutApiUsed === false && layoutEvidence.deployExecuted === false && layoutEvidence.ok === true);
  add('ROOTCAUSE_BOUND', rootCauseEvidence.sourceRunId === 30970150174 && rootCauseEvidence.visual.failureCode === 'ROUTE_POLIZAS_DOM_SNAPSHOT_TIMEOUT' && rootCauseEvidence.visual.failureOwner === 'layout-dependent host innerText probe' && rootCauseEvidence.integrity.unchanged === true && rootCauseEvidence.ok === true);
  add('VIDEO_LAYOUTFREE_HARNESS', scope.visualVideoRecordingRequired === true && scope.ffmpegStaticRequired === true && scope.screenshotApisForbidden === true && scope.layoutDependentTextReadsForbidden === true && visual.includes('recordVideo:') && visual.includes("import ffmpegPath from 'ffmpeg-static'") && visual.includes('video.path()') && visual.includes("layoutProbe: 'host-firstElementChild-childElementCount-no-layout-text-read'") && visual.includes("domProbe: 'layout-free-mount-auth-probe'") && visual.includes('layoutDependentTextReadUsed: false') && !visual.includes('.innerText') && !visual.includes('getComputedStyle(') && !visual.includes('offsetParent') && !visual.includes('getBoundingClientRect(') && !visual.includes('page.screenshot(') && !visual.includes('Page.captureScreenshot') && !visual.includes('newCDPSession('));
  add('SOURCE_CONTRACT_PASS', sourceEvidence.status === 'LAYOUTFREE_VISUAL_CONTRACT_SOURCE_PASS' && sourceEvidence.contractVersion === VERSION && sourceEvidence.engine === expectedEngine && sourceEvidence.layoutSyntheticRunId === 30970624742 && sourceEvidence.videoSyntheticRunId === 30969402434 && sourceEvidence.ok === true);
  ...
  result = {
    ...
    classification: ok ? 'OPERATIONAL_RUNTIME_LAB_LAYOUTFREE_VISUAL_READY' : 'VALIDATOR_STALE',
    total: checks.length,
...
fs.writeFileSync(OUT, JSON.stringify({
  ...
  canonicalEngine: 'tools/orbit360-validar-gate-contracts-engine-block12-operational-runtime-layoutfree-lab-v20260804.mjs',
