...
const evidenceDir = path.join(reportsRoot, `VISUAL-OP2-PLATAFORMAS-V1221-${stamp}`);
const reportPath = path.join(reportsRoot, `VISUAL-OP2-PLATAFORMAS-V1221-${stamp}.txt`);
const resultsPath = path.join(evidenceDir, 'results.jsonl');
const profileRoot = path.join(os.tmpdir(), `o2p21-${stamp}-${process.pid}`);
...
log('============================================================');
log('ORBIT 360 - SMOKE FOCALIZADO PLATAFORMAS OP2 V1.221');
log(`URL: http://127.0.0.1:${port}`);
log('Solo 3 escenarios de Plataformas; no repite CRM ni las otras 12 vistas.');
log('La verificacion de credenciales queda limitada a la misma tarjeta accionada.');
...
document.getElementById('f').addEventListener('load',async()=>{
  ...
  try{
    ...
    let insurer=S&&S.get('aseguradoras','asg_smoke_op2_v1221');
    if(S&&!insurer){
      S.insert('aseguradoras',{
        id:'asg_smoke_op2_v1221',
        nombre:'Aseguradora Ficticia OP2',
        pais:'GT',
        portales:[{nombre:'Portal Ficticio',credentialRef:'cred_smoke_op2_v1221',usuarioHint:'op***',estadoAcceso:'Acceso disponible'}],
        cuentas:[],contactos:[],docs:[],actividad:[]
      });
    }
    insurer=S&&S.get('aseguradoras','asg_smoke_op2_v1221');
    ...
    let present=false,enabled=false,revealed=false,direct=false,restricted=false,buttons=false;
    if(ROLE!=='Asesor'){
      const button=row&&row.querySelector('[data-op2-view-credential]');
      present=!!button;
      enabled=!!(button&&!button.disabled);
      buttons=!!(row&&row.querySelector('[data-op2-view-credential],[data-op2-copy-password],[data-op2-copy-user]'));
      if(enabled){
        button.click();
        revealed=!!(await wait(()=>{
      ...
      if(R&&R.revealCredential){
        const out=await R.revealCredential('cred_smoke_op2_v1221',{
          module:'aseguradoras',
      ...
      restricted=!!(row&&row.querySelector('.asg218-restricted'));
      buttons=!!(row&&row.querySelector('[data-op2-view-credential],[data-op2-copy-password],[data-op2-copy-user]'));
    }
    ...
    const roleOk=ROLE==='Asesor'
      ? platform&&restricted&&!buttons&&!secretVisible&&policy.credentialsVisible===false
      : platform&&present&&enabled&&revealed&&direct&&secretVisible&&policy.credentialsVisible===true;
    await send({
      scenario:ID,
      role:ROLE,
      roleAvailable,
...
log('No se repitieron CRM ni las otras 12 vistas. Sin deploy ni datos reales.');
fs.writeFileSync(reportPath, logs.join('\n'));
