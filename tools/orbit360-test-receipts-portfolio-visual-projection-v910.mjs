...
add('COBROS_SEPARATE',p.includes("Orbit.store.where('cobros'")&&p.includes('Aún no hay cobros aplicados')&&p.includes('no equivale a un pago'));
...
add('PORTFOLIO_RECONCILED_SUMMARY',p.includes('carteraConciliada')&&p.includes('carteraConciliadaMonto')&&p.includes('reconciledAmount'));
...
add('PAYMENT_RECONCILIATION_LABEL',p.includes("return{t:'Cobro conciliado',c:'ok'")&&p.includes('isPaymentReconciled'));
add('PORTFOLIO_RECONCILIATION_LABEL',p.includes('Cartera conciliada con aseguradora')&&p.includes('isPortfolioReconciled'));
add('PORTFOLIO_RECONCILIATION_TRACE',p.includes('fuenteAutoridad')&&p.includes('matchQuality')&&p.includes('sourceRef')&&p.includes('requiereValidacion!==true'));
add('BALANCE_PAYMENT_SEMANTICS_SEPARATE',p.includes('Esto confirma cartera; no equivale a un pago.')&&p.includes('saldo pendiente confirmado'));
add('RECONCILIATION_COLUMN',p.includes('<th>Conciliación</th>')&&p.includes('reconciliationLabel(r,c)'));
