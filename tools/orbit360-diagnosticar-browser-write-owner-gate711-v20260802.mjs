...
async function selectRole(page,role){
  await page.evaluate(target=>{
    ...
    if(window.__orbitWriteOwnerDiagnostic)window.__orbitWriteOwnerDiagnostic.triggerRole=target;
  ...
  await page.evaluate(()=>{if(window.__orbitWriteOwnerDiagnostic)window.__orbitWriteOwnerDiagnostic.triggerRole='';});
}
...
try{
  ...
  await page.evaluate(()=>{
    ...
    function sanitizeStack(stack){return String(stack||'').split('\n').slice(0,14).map(line=>line.replace(/https?:\/\/[^/\s]+/g,'').replace(/[A-Za-z0-9_-]{24,}/g,'[redacted]').trim()).filter(Boolean);}
    const diagnostic={calls,triggerRole:'',sequence:0};
    ['insert','update','remove','setPref'].forEach(name=>{
      const original=S[name];
      S[name]=function(){
        const args=Array.from(arguments),collection=typeof args[0]==='string'?args[0]:'',payload=name==='insert'?args[1]:(name==='update'?args[2]:null),stack=sanitizeStack(new Error('WRITE_OWNER_TRACE').stack);
        diagnostic.sequence+=1;
        calls.push({sequence:diagnostic.sequence,operation:name,collection:String(collection||'').slice(0,80),payloadKeys:payloadKeys(payload),argumentCount:args.length,triggerRole:String(diagnostic.triggerRole||''),activeRole:Orbit.session&&Orbit.session.rol?String(Orbit.session.rol()||''):'',route:String(location.hash||'').split('?')[0],topFrame:String(stack.find(line=>!/WRITE_OWNER_TRACE/.test(line))||''),stack,at:new Date().toISOString(),blocked:true,backendReached:false});
        return null;
      };
      S[name].__guardedOriginal=original;
      S[name].__writeOwnerDiagnostic=true;
    });
    window.__orbitWriteOwnerDiagnostic=diagnostic;
  });

  for(const role of EXPECTED_ROLES)await selectRole(page,role);
  report.attempts=await page.evaluate(()=>((window.__orbitWriteOwnerDiagnostic&&window.__orbitWriteOwnerDiagnostic.calls)||[]));
  const rolesWithAttempts=[...new Set(report.attempts.map(item=>item.triggerRole).filter(Boolean))];
  report.checks.identity=true;
  report.checks.store=true;
  report.checks.legalSettled=true;
  report.checks.rolesAttempted=EXPECTED_ROLES.length;
  report.checks.rolesWithAttempts=rolesWithAttempts;
  report.checks.stackCaptured=report.attempts.length>0&&report.attempts.every(item=>Array.isArray(item.stack)&&item.stack.length>0&&item.topFrame);
  ...
  report.checks.allBlocked=report.attempts.length>0&&report.attempts.every(item=>item.blocked===true&&item.backendReached===false);
  report.checks.allRolesDiagnosed=EXPECTED_ROLES.every(role=>rolesWithAttempts.includes(role));
  report.status='GATE711_BROWSER_WRITE_OWNER_DIAGNOSTIC_CAPTURED';
  report.classification='OWNER_DIAGNOSTIC_READY';
  report.ok=report.attempts.length>=3&&report.checks.stackCaptured&&report.checks.collectionCaptured&&report.checks.allBlocked&&report.checks.allRolesDiagnosed;
