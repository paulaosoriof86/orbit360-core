...
const OUT=path.join(DIR,'m5-runtime-smoke-534-browser-summary.json');
const targetHash=String(process.env.ORBIT360_EXPECTED_RC_HASH||'');
...
const out={schemaVersion:'orbit360-m5-runtime-smoke-browser-adapter-534-v1',generatedAt:new Date().toISOString(),gateId:'block5-release-candidate-visualization-v20260728',contractVersion:'5.0.34',releaseCandidateHash:targetHash,probeImplementationContractVersion:'5.0.25',probeImplementationPath:LEGACY,probeDeclaredHistoricalReleaseCandidateHash:String(legacy&&legacy.releaseCandidateHash||''),targetCandidateBoundExternallyByCanonicalPreflight:true,ok,status:ok?'M5_RUNTIME_SMOKE_534_BROWSER_PASS':'M5_RUNTIME_SMOKE_534_BROWSER_FAIL',probeExitCode:Number.isInteger(run.status)?run.status:1,stage:String(legacy&&legacy.stage||'NOT_RUN'),failureStage:String(legacy&&legacy.failureStage||''),error:String(legacy&&legacy.error||'').slice(0,400),checks:derivedChecks,accessBoundary:legacy&&legacy.accessBoundary||{},dataBaseline:legacy&&legacy.dataBaseline||{},roleViews:views,mobileMenuVisibleModules:Number(legacy&&legacy.mobileMenuVisibleModules||0),academiaStaticWritePolicy:legacy&&legacy.academiaStaticWritePolicy||{},writeGuard:legacy&&legacy.writeGuard||{},browserParseDiagnostics:legacy&&legacy.browserParseDiagnostics||{},functionalMultirolEvidence,responsiveTitleResolverReady,normalizedBootstrapOwner,expectedMultirolCompatibilityVersion:'20260729.2',expectedCanonicalAccessOwnerVersion:'20260729.3',visibleTechnicalCopyPredicateVersion:String(legacy&&legacy.visibleTechnicalCopyPredicateVersion||''),responsiveTitleResolverVersion:String(legacy&&legacy.responsiveTitleResolverVersion||''),firestoreRead:true,firestoreWrites:0,operationalWrites:0,runtimeExecuted:true,browserExecuted:true,hostingDeploy:false,functionsDeploy:false,rulesDeploy:false,production:false,mergeMain:false,visualReviewExecuted:false,containsPII:false,containsSecrets:false};
