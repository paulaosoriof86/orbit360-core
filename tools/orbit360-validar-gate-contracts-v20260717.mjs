...
const GATE_CONFIG=Object.freeze({
  ...
  'block2-product-readonly-runtime-v20260723':{contractVersion:'2.2.0',lifecycle:'tools/orbit360-validator-lifecycle-contract-m2-runtime-v20260723.json',engine:'tools/orbit360-validar-gate-contracts-engine-m2-runtime-v20260723.mjs'}
});
...
try{
  ...
  const lifecycle=readJson(config.lifecycle);if(lifecycle.gateId!==GATE_ID)throw new Error('CANONICAL_GATE_MISMATCH');
  ...
  const profile=lifecycle.executionProfile||{},expected=PHASE_PROFILES[String(profile.phase||'')];if(!expected)throw new Error('CANONICAL_LIFECYCLE_PHASE_MISMATCH');
  ...
  exitCode=Number.isInteger(run.status)?run.status:41;if(run.error)throw run.error;if(!fs.existsSync(EVIDENCE_PATH))throw new Error('CANONICAL_ENGINE_EVIDENCE_MISSING');
