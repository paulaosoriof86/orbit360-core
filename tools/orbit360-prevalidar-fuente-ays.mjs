...
const VERSION = 'v1.2.0-ays-source-prevalidation-normalized-preview';
...
const preview = path.join(root, 'tools', 'orbit360-preview-normalizado-fuente-ays.mjs');
const scripts = {
...
if(!errors.length){
  ...
  if(stages.at(-1)?.ok) stages.push(run('normalized-preview', [scripts.preview, '--manifest', manifest], false));
...
const review = stages.some(s => /REQUIERE_VALIDACION|requiere_validacion|PREVIEW_REQUIERE_VALIDACION|WARN|Advertencias: [1-9]/i.test(s.output_tail));
...
const txt = [
  ...
  'Preview: ejecuta normalized-preview antes de cualquier writeToStore.',
  '============================================================',
  '',
