...
const out={schemaVersion:'orbit360-receipts-portfolio-write-static-test-v2',contractVersion:'9.1.0',status:failed.length?'STATIC_WRITE_BLOCKED':'STATIC_WRITE_READY',ok:failed.length===0,total:checks.length,passed:checks.length-failed.length,failed:failed.length,failedCheckIds:failed.map(c=>c.id),checks,operationalWrites:0,productionTouched:false,containsPII:false,containsSecrets:false};
