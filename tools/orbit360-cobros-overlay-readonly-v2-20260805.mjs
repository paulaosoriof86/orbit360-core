...
function matchingKey(row) {
  return [policyNumberKey(row), currency(row), period(row)].join('|');
}

function main() {
  const [baseFile, sequenceFile, planillaFile, outFile] = process.argv.slice(2);
  if (!baseFile || !sequenceFile || !planillaFile || !outFile) {
    console.error('Uso: node tools/orbit360-cobros-overlay-readonly-v2-20260805.mjs <base.json> <sequence.json> <planillas.json> <out.json>');
    process.exit(2);
  }
  ...
  const detailKeyCounts = new Map();
  for (const row of planillaRows) {
    if (classifyPlanillaRow(row) !== 'PLANILLA_DETAIL_CANDIDATE') continue;
    const key = matchingKey(row);
    detailKeyCounts.set(key, (detailKeyCounts.get(key) || 0) + 1);
  }

  const planillaCounts = {};
  const planillaMatchOutcomes = {
    proposed: 0,
    sourceKeyAmbiguousHold: 0,
    targetKeyAmbiguousHold: 0,
    noUnresolvedTarget: 0
  };
  const handledKeys = new Set();
  let exactPlanillaProposals = 0;

  for (const row of planillaRows) {
    let classification = classifyPlanillaRow(row);
    const key = matchingKey(row);
    if (classification === 'PLANILLA_DETAIL_CANDIDATE' && (detailKeyCounts.get(key) || 0) > 1) {
      classification = 'PLANILLA_DETAIL_AMBIGUOUS_HOLD';
    }
    planillaCounts[classification] = (planillaCounts[classification] || 0) + 1;
    if (classification !== 'PLANILLA_DETAIL_CANDIDATE') {
      if (classification === 'PLANILLA_DETAIL_AMBIGUOUS_HOLD') planillaMatchOutcomes.sourceKeyAmbiguousHold += 1;
      continue;
    }
    ...
    if (candidates.length === 1) {
      ...
      candidates[0].evidence.push('PLANILLA_DETAIL_UNIQUE_KEY');
      exactPlanillaProposals += 1;
      planillaMatchOutcomes.proposed += 1;
      unresolvedByPolicy.delete(key);
    } else if (candidates.length > 1) {
      planillaMatchOutcomes.targetKeyAmbiguousHold += 1;
    } else {
      planillaMatchOutcomes.noUnresolvedTarget += 1;
    }
  }
  ...
  const source = planillas.source || {};

  const output = {
    schemaVersion: 'orbit360-cobros-overlay-readonly-v2',
    ...
    outcomeCounts,
    planillaCounts,
    planillaMatchOutcomes,
    planillaSourceControl: {
      titleSha256: source.title ? sha(source.title) : '',
      inputMode: text(source.inputMode),
      reportedRows: Number(source.reportedRows || planillaRows.length),
      eligibleCrm: Number(source.eligibleCrm || 0),
      sourcePackages: Number(source.sourcePackages || 0),
      incompletePackages: Number(source.incompletePackages || 0),
      invoiceMissingPackages: Number(source.invoiceMissingPackages || 0)
    },
  ...
  console.log(JSON.stringify({ stage: output.stage, explained, unresolved: output.unresolvedPayments, outcomeCounts, planillaCounts, planillaMatchOutcomes }, null, 2));
}
