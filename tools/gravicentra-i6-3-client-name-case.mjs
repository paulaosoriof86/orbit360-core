...
try{
  ...
  if(mutations.length){
    const actor=await selectManager(db);
  ...
  if(committed){
    ...
    try{
      ...
      const actor=await selectManager(db),page=await browser.newPage({viewport:{width:1280,height:800}});await activate(page,auth,actor);
