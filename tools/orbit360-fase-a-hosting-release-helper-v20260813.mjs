...
try{
 ...
 const t=await token(),resp=await safe(`https://firebasehosting.googleapis.com/v1beta1/sites/${SITE}/releases?versionName=${encodeURIComponent(anchor)}`,{method:'POST',headers:{authorization:`Bearer ${t}`,accept:'application/json','content-type':'application/json'},body:'{}'});if(!resp.ok)throw new Error(`ROLLBACK_RELEASE_CREATE_FAILED_${resp.status}`);
 let r={},restored=false;for(let i=0;i<24;i++){r=await remote();restored=ASSETS.every(a=>r?.[a]?.sha256===b.publicAssets?.[a]?.sha256);if(restored)break;await sleep(5000);}const h=await hosting(),ok=restored&&h.ok;
 const p={schemaVersion:'orbit360-fase-a-hosting-rollback-v1',generatedAt:new Date().toISOString(),projectId:PROJECT,liveUrl:LIVE_URL,rollbackAnchor:anchor,hosting:h,publicAssets:r,checks:{rollbackReleaseCreated:true,publicAssetsRestored:restored,hostingReadable:h.ok},deployExecuted:true,productionTouched:true,rollbackExecuted:true,decision:ok?'ROLLED_BACK_SAFE':'ROLLBACK_FAILED_ESCALATE',ok};write(ROLLBACK,p);console.log(JSON.stringify(p,null,2));process.exit(ok?0:42);
