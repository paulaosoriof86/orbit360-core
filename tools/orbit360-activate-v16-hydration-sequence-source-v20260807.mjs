...
function initialFailClosedChecks() {
  ...
  const checks = {
    ...
    relayDisarmedOrConnectorReservedV16: relay.includes('ORBIT360_EXPECTED_REQUEST_VERSION: NONE_PENDING_FRESH_AUTHORIZATION') || relay.includes(`ORBIT360_EXPECTED_REQUEST_VERSION: ${REQUEST_VERSION}`)
  };
  const failed = Object.entries(checks).filter(([, ok]) => !ok).map(([id]) => id);
...
function writeEvidence(stage, checks, extra = {}) {
  const failed = Object.entries(checks).filter(([, ok]) => !ok).map(([id]) => id);
  const out = {
    schemaVersion: 'orbit360-v16-source-activation-hydration-sequence-v1', gateId: 'block2.7-visual-matrix-corrected-post-auth-lab-v20260805', requestVersion: REQUEST_VERSION,
    sourceAuthorizationBaseHead: BASE, status: stage, classification: 'SOURCE_ONLY_ACTIVATION_VALIDATED',
    sequence: ['HYDRATION_CONTRACT_MOUNTED','INICIO_REQUIRED_HYDRATION','INICIO_READY'], total: Object.keys(checks).length,
    passed: Object.keys(checks).length - failed.length, failed: failed.length, failedCheckIds: failed, checks, ...extra,
    secretsRead:false, firebaseAccess:false, firestoreReads:0, firestoreWrites:0, authWrites:0, operationalWrites:0,
    hostingTouched:false, browserExecuted:false, deployExecuted:false, productionTouched:false,
    containsPII:false, containsSecrets:false, containsPasswords:false, ok: failed.length === 0
  };
  writeJson(OUT, out); return out;
}

function prepareSourceStage() {
  const checks = { ...immutableRootfixChecks(), ...initialFailClosedChecks() };
  const lifecycle = json(LIFECYCLE); const overlay = json(OVERLAY); let relay = read(RELAY);
  ...
  Object.assign(lifecycle, {
    ownerVersion:'20260807.45-v16-source-activation-hydration-sequence', status:'AUTHORIZED_FRESH_REQUEST_ONLY_V16_PENDING_EXCLUSIVE_REQUEST',
    classification:'SOURCE_ONLY_ACTIVATION_VALIDATED', secondaryClassification:'V15_HYDRATION_ROOTFIX_REUSED_SOURCE_VALIDATED',
    authorizedBaseHead:BASE, expectedRequestVersion:REQUEST_VERSION,
    ...
    currentPhase:'SOURCE_ONLY_ACTIVATION', executionProfile:{mode:'SOURCE_ONLY_ACTIVATION',phase:'SOURCE_ONLY_ACTIVATION',capabilities},
    hostingDeploysMaximum:1, hostingBackupCloneAuthorized:true, hostingRollbackCloneAuthorizedOnFailure:true,
    priorHostingRestoreAuthorized:true, priorHostingRestoreChannel:'visual-matrix-corrected-backup-31135532118',
    priorHostingRestoreScript:'tools/orbit360-restore-visual-matrix-v13-baseline-before-runtime-v20260807.sh',
    activeRequest:false, requestRetired:true, requestConsumed:true, authorizationReserved:true, authorizationFrozen:false, replayAllowed:false,
    allowedExecutions:1, executionAuthorized:false, secretAccessAuthorized:false, firestoreReadAuthorized:false, writeAuthorized:false,
    browserAuthorized:false, hostingDeployAuthorized:false, functionsDeployAuthorized:false, rulesDeployAuthorized:false,
    productionAuthorized:false, mainAuthorized:false, mergeAuthorized:false, stopRetryActive:false,
    nextAction:'EXPLICIT_SOURCE_TO_RUNTIME_TRANSITION_ONLY_AFTER_V16_SOURCE_VALIDATION_PASS.'
  });
  lifecycle.sourcePrerequisites = { ...(lifecycle.sourcePrerequisites||{}), authorizedSourceHead:BASE, priorRequestVersion:PRIOR_REQUEST_VERSION,
    priorRequestConsumed:true, priorRequestReplayAllowed:false, v15InicioHydrationStopConsumerRootfixStatus:'PASS_V15_INICIO_HYDRATION_STOP_CONSUMER_ROOTFIX_SOURCE_ONLY',
    v16HydrationSequenceRequired:['HYDRATION_CONTRACT_MOUNTED','INICIO_REQUIRED_HYDRATION','INICIO_READY'], v16HydrationSequenceSourceCheck:'PASS',
    v16PriorRequestClosed:true, activationSourceStatus:'PASS_V16_ACTIVATION_SOURCE_ONLY_HYDRATION_SEQUENCE_AWARE',
    activationSourceRunId:Number(process.env.GITHUB_RUN_ID||0), activationSourceJob:'source-v16-hydration-sequence' };
  Object.assign(overlay, {
    requestVersion:REQUEST_VERSION, status:'AUTHORIZED_FRESH_REQUEST_ONLY_V16_PENDING_EXCLUSIVE_REQUEST', classification:'SOURCE_ONLY_ACTIVATION_VALIDATED',
    secondaryClassification:'V15_HYDRATION_ROOTFIX_REUSED_SOURCE_VALIDATED', checkpoint:'SOURCE_ACTIVATION_VALIDATED_HYDRATION_SEQUENCE', failureCode:'',
    rootCauseResolved:'V15_HYDRATION_RUNTIME_COMPOSITION_AND_STOP_CONSUMER_STATE_DRIFT_CORRECTED_SOURCE_ONLY', authorizedSourceHead:BASE,
    stopRetryActive:false, requestReusable:false, freshAuthorizationRequired:false, expectedNextRequestVersion:REQUEST_VERSION,
    runtimeAllowed:true, runtimeAllowedOnlyWithFreshExclusiveRequest:true, browserAllowed:false, hostingAllowed:false, productionAllowed:false,
    writesAllowed:false, functionsAllowed:false, rulesAllowed:false, reimportAllowed:false, secretsRead:false, firebaseAccess:false,
    hostingTouched:false, hostingDeploys:0, rollbackRequired:false, rollbackRestored:false, snapshotIntegrity:'VERIFIED_UNCHANGED_V15_ROLLBACK',
    firestoreReads:0, firestoreWrites:0, authWrites:0, operationalWrites:0, passVisualPostAuth:false, ok:true
  });
  relay = relay.replace('ORBIT360_EXPECTED_REQUEST_VERSION: NONE_PENDING_FRESH_AUTHORIZATION', `ORBIT360_EXPECTED_REQUEST_VERSION: ${REQUEST_VERSION}`)
               .replace('visual-matrix-v15-prior-hosting-restore-sanitized-v20260807.json','visual-matrix-v16-prior-hosting-restore-sanitized-v20260807.json');
  ...
  writeJson(LIFECYCLE,lifecycle); writeJson(OVERLAY,overlay); fs.writeFileSync(path.resolve(RELAY),relay,'utf8');
...
function validateSourceStage() {
  const lifecycle=json(LIFECYCLE), overlay=json(OVERLAY), relay=read(RELAY);
  const all={...immutableRootfixChecks(), lifecycleStatus:lifecycle.status==='AUTHORIZED_FRESH_REQUEST_ONLY_V16_PENDING_EXCLUSIVE_REQUEST',
    lifecycleClassification:lifecycle.classification==='SOURCE_ONLY_ACTIVATION_VALIDATED', lifecycleExpectedVersion:lifecycle.expectedRequestVersion===REQUEST_VERSION,
    ...
    overlayNoBrowserHostingYet:overlay.browserAllowed===false&&overlay.hostingAllowed===false, overlayExpectedVersion:overlay.expectedNextRequestVersion===REQUEST_VERSION,
    relayExpectedVersion:relay.includes(`ORBIT360_EXPECTED_REQUEST_VERSION: ${REQUEST_VERSION}`), relayV16RestoreEvidence:relay.includes('visual-matrix-v16-prior-hosting-restore-sanitized-v20260807.json')};
  const failed=Object.entries(all).filter(([,ok])=>!ok).map(([id])=>id); assert(!failed.length,'V16_SOURCE_STAGE_INVALID',failed.join(','));
...
function validateRuntimePending() {
  const lifecycle=json(LIFECYCLE), overlay=json(OVERLAY), relay=read(RELAY), request=json(REQUEST);
  const all={...immutableRootfixChecks(), lifecycleRuntimePending:lifecycle.status==='AUTHORIZED_ONCE_PENDING_EXCLUSIVE_REQUEST'&&lifecycle.currentPhase===RUNTIME_PHASE,
    ...
    relayExpectedVersion:relay.includes(`ORBIT360_EXPECTED_REQUEST_VERSION: ${REQUEST_VERSION}`), relayV16RestoreEvidence:relay.includes('visual-matrix-v16-prior-hosting-restore-sanitized-v20260807.json')};
  const failed=Object.entries(all).filter(([,ok])=>!ok).map(([id])=>id); assert(!failed.length,'V16_RUNTIME_PENDING_INVALID',failed.join(','));
...
if(MODE==='check-rootfix'){const checks={...immutableRootfixChecks(),...initialFailClosedChecks()};console.log(JSON.stringify({status:'PASS_V16_ROOTFIX_SEQUENCE_SOURCE_PRECONDITION',checks,ok:true},null,2));}
else if(MODE==='prepare'){prepareSourceStage();console.log(JSON.stringify({status:'PASS_V16_SOURCE_STAGE_PREPARED',requestVersion:REQUEST_VERSION,ok:true},null,2));}
else if(MODE==='validate-source-stage'){validateSourceStage();console.log(JSON.stringify({status:'PASS_V16_SOURCE_STAGE_VALIDATED',requestVersion:REQUEST_VERSION,ok:true},null,2));}
else if(MODE==='validate-runtime-pending'){validateRuntimePending();console.log(JSON.stringify({status:'PASS_V16_RUNTIME_PENDING_VALIDATED',requestVersion:REQUEST_VERSION,ok:true},null,2));}
