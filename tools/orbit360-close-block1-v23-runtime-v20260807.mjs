...
const matrixPass = !!(matrix && matrix.contractVersion===CONTRACT && matrix.canonicalRuntimeArtifact==='tools/orbit360-block1-native-matrix-v23-canonical-v20260807.mjs' && matrix.ok===true && matrix.stage==='PASS_BLOCK1_NATIVE_VISUAL_MATRIX' && matrix.classification==='PASS_VISUAL_POST_AUTH' && matrix.snapshotIntegrity==='VERIFIED_UNCHANGED' && matrix.totalRoleFailures===0 && matrix.firestoreWrites===0 && matrix.authWrites===0 && matrix.operationalWrites===0);
...
const preflightPass = !!(preflight && preflight.ok===true && preflight.status==='GO_GATE_CONTRACT' && preflight.contractVersion===CONTRACT);
const runtimePass = preflightPass && matrixPass && universePass && precheckPass && state.runtimeOutcome==='success' && state.rollbackOutcome!=='failure';
...
else if(preflight && preflight.ok===false){ classification=preflight.classification || 'VALIDATOR_STALE'; checkpoint='GO_GATE_CONTRACT'; }
