...
const base={numero:'POL-001',aseguradoraNombre:'Aseguradora Demo',clienteNombre:'Cliente Demo',vigenciaIni:'2026-01-01',vigenciaFin:'2026-12-31',pais:'GT',moneda:'GTQ',primaNeta:'1000',gastos:'50',iva:'126',formaPago:'mensual'};
...
const sinForma=P0.normalizePolicy({...base,formaPago:'',frecuencia:'',estadoPol:'Vigente'},{today:'2026-07-30'});assert.ok(sinForma.requiereValidacion&&sinForma.motivosValidacion.includes('forma_pago'));
...
const recibo=P0.expectedReceiptSeed(porRenovar,{n:1,total:1176,neta:1000,iva:126,vence:'2026-08-01'},0);assert.equal(recibo.estado,'esperado');assert.equal(recibo.confirmadoPago,false);assert.equal(recibo.carteraOperativa,false);
console.log(JSON.stringify({status:'PASS',canonicalIdentityStableAcrossTerms:true,termVersionDistinct:true,crossClientIdentityDistinct:true,renovadaHistorical:true,porRenovarActive:true},null,2));
