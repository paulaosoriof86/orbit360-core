...
export function milestoneKind(L) {
  ...
  if (phase === 'POST_GO_LIVE_OVERLAY_RELEASE_RUNNING' && status === 'AUTHORIZED_RELEASE_CLAIMED' && progress === 100 && next === 'RUN_POST_GO_LIVE_OVERLAY_RELEASE_WINDOW') return 'POST_GO_LIVE_OVERLAY_RELEASE_CLAIMED';
...
export function assertFollowupConsistency(L, fail) {
  ...
  if ((f.status === 'RESET_LINK_READY_FOR_PRIVATE_HANDOFF' || f.status === 'HUMAN_LOGIN_VERIFIED_LATENCY_OPEN') && !releaseNext.has(next)) {
    fail('ACCESS_RECOVERY_NEXT_ACTION_DESYNC');
  }
