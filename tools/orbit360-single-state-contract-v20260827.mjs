...
function collectIssues(root, R) {
  ...
  const required = [STATIC_GATE_REGISTRY,STATIC_GATE_CANONICAL_ROUTER,STATIC_GATE_LEGACY_ROUTER,STATIC_GATE_ENGINE_FENCE,STATIC_GATE_WORKFLOW,STATIC_GATE_LEDGER,STATIC_GATE_WRITER_REGISTRY,STATIC_GATE_INVARIANT,STATIC_GATE_STATE_CONTRACT,STATIC_GATE_FROZEN_BASELINE];
  required.forEach(p => { if (!exists(p)) issues.push(`CONTROL_DEPENDENCY_MISSING:${p}`); });
  if (issues.length) return issues;
  const C = R.canonicalControlPlane || {}, P = R.policies || {};
  if (R.schemaVersion !== 'orbit360-gate-contract-registry-v4-history-pointer-only' || R.status !== 'ACTIVE_STATIC_CONFIGURATION_HISTORY_POINTER_ONLY' || R.stateBearing !== false || R.dynamicStateForbidden !== true || R.historicalExecutionForbidden !== true || R.currentStateAuthority !== STATIC_GATE_LEDGER || R.sourceIdentityAuthority !== STATIC_GATE_FROZEN_BASELINE) issues.push('REGISTRY_CONTRACT_STALE');
  if (C.workflow !== STATIC_GATE_WORKFLOW || C.stateContract !== STATIC_GATE_STATE_CONTRACT || C.singleStateInvariant !== STATIC_GATE_INVARIANT || C.writerRegistry !== STATIC_GATE_WRITER_REGISTRY || C.canonicalGatePreflight !== STATIC_GATE_CANONICAL_ROUTER || C.legacyDelegate !== STATIC_GATE_LEGACY_ROUTER || C.historicalEngineFence !== STATIC_GATE_ENGINE_FENCE || C.frozenBaseline !== STATIC_GATE_FROZEN_BASELINE) issues.push('CONTROL_PLANE_BINDING_DRIFT');
  const router = text(STATIC_GATE_CANONICAL_ROUTER), legacy = text(STATIC_GATE_LEGACY_ROUTER), fence = text(STATIC_GATE_ENGINE_FENCE);
  ...
  if (!legacy.includes(LEGACY_ROUTER_FENCE) || /from ['"]node:child_process|require\(['"](?:node:)?child_process|spawnSync\(|execSync\(/i.test(legacy)) issues.push('LEGACY_ROUTING_NOT_FROZEN');
  ...
  for (const b of bindings) {
    ...
    if (!/^[a-f0-9]{40}$/.test(String(b.historicalSourceCommit || ''))) issues.push(`HISTORICAL_SOURCE_COMMIT_INVALID:${key}`);
    const liveFields = Object.keys(b).filter(k => FORBIDDEN_HISTORICAL_LIVE_FIELDS.has(k)); if (liveFields.length) issues.push(`HISTORICAL_BINDING_HAS_LIVE_TREE_DEPENDENCY:${key}:${liveFields.join(',')}`);
  }
  ...
  if (P.singleStaticGateRegistry !== true || P.singleMutableStateLedger !== true || P.dynamicStateMustComeFromLedger !== true || P.historicalExecutionForbidden !== true || P.historicalArtifactsArchivedBySourceCommit !== true || P.currentTreeHistoricalArtifactDependencyForbidden !== true || P.sourceIdentitiesOnlyInFrozenBaseline !== true || P.newGateMustUseSingleStateControlPlane !== true || P.productMutationOnParityFailure !== false || P.dataMutationOnParityFailure !== false || P.runtimeOnParityFailure !== false || P.deployOnParityFailure !== false) issues.push('FAIL_CLOSED_POLICY_INVALID');
  return issues;
}
...
export function assertStaticGateContractParity(root = process.cwd(), fail = code => { throw new Error(code); }) {
  ...
  const issues = collectIssues(root, R); if (issues.length) fail(`STATIC_GATE_CONTROL_PLANE_PARITY_FAIL:${issues.slice(0, 64).join('|')}`);
  const drift = structuredClone(R); if (!Array.isArray(drift.historicalBindings) || !drift.historicalBindings.length) fail('STATIC_GATE_SYNTHETIC_DRIFT_FIXTURE_MISSING');
  drift.historicalBindings[0].executionMode = 'ACTIVE_EXECUTION';
  const driftIssues = collectIssues(root, drift); if (!driftIssues.some(x => x.startsWith('BINDING_EXECUTION_NOT_FROZEN:'))) fail('STATIC_GATE_SYNTHETIC_DRIFT_NOT_BLOCKED');
  return {ok:true,status:'STATIC_GATE_CONTROL_PLANE_PARITY_PASS',bindingCount:R.historicalBindings.length,historicalArchivePointers:R.historicalBindings.length,noLiveHistoricalArtifactDependencies:true,sourceIdentityAuthority:STATIC_GATE_FROZEN_BASELINE,syntheticDrift:'FAIL_CLOSED_CONFIRMED',currentStateAuthority:STATIC_GATE_LEDGER};
}
...
const releaseNext=new Set(['POST_GO_LIVE_MONITORING','AWAIT_EXPLICIT_HUMAN_ACCESS_RECOVERY_AUTHORIZATION','VERIFY_HUMAN_EMAIL_PASSWORD_LOGIN_AND_START_POST_GO_LIVE_FUNCTIONAL_VALIDATION','DIAGNOSE_HUMAN_ACCESS_RECOVERY_ROOT_CAUSE_NO_GO_LIVE_REOPEN','DIAGNOSE_ACCESS_RECOVERY_SOURCE_PREP_NO_GO_LIVE_REOPEN','DIAGNOSE_POST_GO_LIVE_FUNCTIONAL_BLOCKERS','DIAGNOSE_SEMANTIC_SINGLE_STATE_ROOT_CAUSE_STOP_RETRY','REVIEW_STAGED_POST_GO_LIVE_PRODUCT_SUCCESSOR_SOURCE_ONLY','DIAGNOSE_POST_GO_LIVE_SUCCESSOR_STAGE_MECHANISM_ROOT_CAUSE_STOP_RETRY','VALIDATE_ACCEPTED_RUNTIME_CAPABILITY_COMPOSITION_SOURCE_ONLY','DIAGNOSE_POST_GO_LIVE_SUCCESSOR_ACCEPTANCE_MECHANISM_ROOT_CAUSE_STOP_RETRY','REVIEW_RUNTIME_CAPABILITY_OWNER_LINEAGE_SOURCE_ONLY','DIAGNOSE_RUNTIME_CAPABILITY_COMPOSITION_VALIDATION_MECHANISM_ROOT_CAUSE_STOP_RETRY']);
...
export function assertReleaseMilestoneFrozen(L,fail){const m=L.releaseMilestone;if(!m)return;if(m.closed!==true||m.immutable!==true||m.phase!==RELEASE_PHASE||m.status!==RELEASE_STATUS||Number(m.progress)!==100)fail('GO_LIVE_RELEASE_MILESTONE_REOPENED');if(m.goLiveAuthorizationDigest!=null&&!/^[a-f0-9]{64}$/.test(String(m.goLiveAuthorizationDigest)))fail('GO_LIVE_RELEASE_AUTHORIZATION_DIGEST_INVALID');}

export function freezeReleaseMilestone(L){if(!L.releaseMilestone){const prior=L.executionClaim?.transitionId==='GO_LIVE_RELEASE_WINDOW'&&/^[a-f0-9]{64}$/.test(String(L.executionClaim?.authorizationDigest||''))?String(L.executionClaim.authorizationDigest):null;L.releaseMilestone={closed:true,immutable:true,phase:RELEASE_PHASE,status:RELEASE_STATUS,progress:100,closedAtUtc:L.history?.goLiveReleaseTerminal?.reducedAtUtc||L.updatedAtUtc||null,...(prior?{goLiveAuthorizationDigest:prior}:{})};}return L;}

export function assertFollowupConsistency(L,fail){
  const f=L.postGoLiveAccessRecovery;if(!f)return;
  if(['POST_GO_LIVE_ACCESS_RECOVERY_SOURCE_PREP_RUNNING','POST_GO_LIVE_ACCESS_RECOVERY_RUNNING','POST_GO_LIVE_CONTROL_PLANE_METADATA_RECONCILE_RUNNING','POST_GO_LIVE_SEMANTIC_SINGLE_STATE_ROOTFIX_RUNNING','POST_GO_LIVE_PRODUCT_SUCCESSOR_SOURCE_STAGE_RUNNING','POST_GO_LIVE_PRODUCT_SUCCESSOR_ACCEPT_RUNNING','POST_GO_LIVE_RUNTIME_CAPABILITY_COMPOSITION_VALIDATE_RUNNING','POST_GO_LIVE_RUNTIME_CAPABILITY_VALIDATOR_STALE_ROOTFIX_RUNNING'].includes(String(L.activeState?.phase||'')))return;
  ...
  const allowed=['VERIFY_HUMAN_EMAIL_PASSWORD_LOGIN_AND_START_POST_GO_LIVE_FUNCTIONAL_VALIDATION','DIAGNOSE_POST_GO_LIVE_FUNCTIONAL_BLOCKERS','DIAGNOSE_SEMANTIC_SINGLE_STATE_ROOT_CAUSE_STOP_RETRY','REVIEW_STAGED_POST_GO_LIVE_PRODUCT_SUCCESSOR_SOURCE_ONLY','DIAGNOSE_POST_GO_LIVE_SUCCESSOR_STAGE_MECHANISM_ROOT_CAUSE_STOP_RETRY','VALIDATE_ACCEPTED_RUNTIME_CAPABILITY_COMPOSITION_SOURCE_ONLY','DIAGNOSE_POST_GO_LIVE_SUCCESSOR_ACCEPTANCE_MECHANISM_ROOT_CAUSE_STOP_RETRY','REVIEW_RUNTIME_CAPABILITY_OWNER_LINEAGE_SOURCE_ONLY','DIAGNOSE_RUNTIME_CAPABILITY_COMPOSITION_VALIDATION_MECHANISM_ROOT_CAUSE_STOP_RETRY'];
  if(f.status==='RESET_LINK_READY_FOR_PRIVATE_HANDOFF'&&!allowed.includes(next))fail('ACCESS_RECOVERY_NEXT_ACTION_DESYNC');
  if(f.status==='HUMAN_LOGIN_VERIFIED_LATENCY_OPEN'&&!allowed.filter(x=>x!=='VERIFY_HUMAN_EMAIL_PASSWORD_LOGIN_AND_START_POST_GO_LIVE_FUNCTIONAL_VALIDATION'&&x!=='DIAGNOSE_SEMANTIC_SINGLE_STATE_ROOT_CAUSE_STOP_RETRY').includes(next))fail('ACCESS_RECOVERY_NEXT_ACTION_DESYNC');
