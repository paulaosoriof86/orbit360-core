...
const SAFE_CODE_PREFIXES=['product_readiness_blocked','auth_','membresia_','store_','modo_','contrato_','politica_','tenant_','config_','secretos_','dependencia_','runtime_','snapshots_','product_store_','query_','firestore_','scope_','colecciones_','autorizacion_'];
function text(v){return String(v==null?'':v).trim();}
function unique(v){return [...new Set([].concat(v||[]).map(text).filter(Boolean))];}
function safeContractCode(value){const s=text(value);if(!s||s.length>360||!SAFE_CODE_PREFIXES.some(prefix=>s.toLowerCase().startsWith(prefix)))return '';return /^[A-Za-z0-9_:\-|,]+$/.test(s)?s:'';}
function sanitizedCode(value){const safe=safeContractCode(value);if(safe)return safe;return text(value).replace(/[A-Za-z0-9_-]{24,}/g,'[redacted]').replace(/[^A-Za-z0-9_:\-|,.[\] ]/g,'').slice(0,180);}
function sanitizedCodes(values){return unique(values).map(sanitizedCode);}
function extractReadinessErrors(values){const out=[];unique(values).forEach(value=>{const s=text(value);const prefix='product_readiness_blocked:';if(s.startsWith(prefix))s.slice(prefix.length).split('|').forEach(code=>{const safe=safeContractCode(code);if(safe)out.push(safe);});});return unique(out);}
...
function cleanError(error){return sanitizedCode(text(error&&(error.code||error.message||error)||error)).slice(0,360);}
...
async function executeRuntime(config,identity,adminAuth){
  ...
  const rawBootstrapErrors=unique(bootstrapStatus.errors);
  ...
  return {ok:result.ok===true&&result.ready===true&&result.storeInstalled===true&&result.writeAuthorized===false&&status.noFallback===true&&status.writeEnabled===false&&localWriteBlocked,bootstrapPhase:text(bootstrapStatus.phase),bootstrapErrors:sanitizedCodes(rawBootstrapErrors),readinessStatus:text(readiness.status),readinessErrors:unique([].concat(sanitizedCodes(readiness.errors),extractReadinessErrors(rawBootstrapErrors))),storeStatus:text(status.status),storeSnapshotErrorKeys:Object.keys(status.snapshotErrors||{}).sort(),storeDeniedCollections:unique(status.deniedCollections||[]),controlledExistingIdentity:readiness.controlledExistingIdentity===true,controlledExistingIdentityAccepted:readiness.controlledExistingIdentityAccepted===true,controlledAuthMarkerAccepted:readiness.controlledAuthMarkerAccepted===true,controlledMembershipMarkerAccepted:readiness.controlledMembershipMarkerAccepted===true,storeInstalled:result.storeInstalled===true,snapshotsAttached:result.snapshotsAttached===true,noFallback:status.noFallback===true,storeWriteEnabled:status.writeEnabled===true,localWriteBlocked,activeRoleAssigned:!!identity.activeRole,roleCount:identity.roles.length,countryCount:identity.countries.length};
}
async function main(){let app;let runtimeStarted=false;const base={schemaVersion:'orbit360-m2-existing-identity-runtime-v3',gateId:'block2-product-readonly-runtime-v20260723',contractVersion:'2.2.1',projectId:PROJECT_ID,tenantIdSource:'membership_only',existingIdentityOnly:true,newProjectRequired:false,createAuthUser:false,updateAuthUser:false,createMembership:false,updateMembership:false,rulesChanged:false,configurationWrites:0,operationalWrites:0,hostingDeploy:false,functionsDeploy:false,imports:false,policies:false,m3:false,mergeMain:false};
  try{
  ...
  }catch(error){const code=cleanError(error);writeEvidence({...base,ok:false,status:text(error&&error.message).startsWith('PIPELINE_MECHANISM_FAILURE')?'PIPELINE_MECHANISM_FAILURE':'DATA_CONTRACT_FAILURE',classification:text(error&&error.message).startsWith('PIPELINE_MECHANISM_FAILURE')?'PIPELINE_MECHANISM_FAILURE':'DATA_CONTRACT_FAILURE',error:code,runtimeExecuted:runtimeStarted,browserExecuted:false,bootstrapPhase:runtimeStarted?'runtime_exception':'not_started',bootstrapErrors:[code],readinessErrors:extractReadinessErrors([text(error&&error.message)]),storeSnapshotErrorKeys:[]});process.exitCode=41;}finally{if(app)await deleteApp(app).catch(()=>{});}
}
await main();
