...
const TARGETS = ['core/queries.js'];
const MANIFEST = 'orbit360-package-manifest.json';
const BASE_ZIP_SHA = '1ab5f3ea7f59cd0c2eb2bb1f5c0596a4bf3ca241f42016f74bf095ccbaf0f78e';
const BASE_SOURCE = '294ed22bdb564585b71fc59cefa1d04cdfa6b120';
const DELTA_SOURCE = '54f671e64b32c7b39100d79e770572a579e79ac7';
...
function validateBase(baseDir) {
  ...
  if (mf.status !== 'FASE_A_PRODUCT_R4S3_MINIMAL_SUCCESSOR_CERTIFIED') fail('base_manifest_status_mismatch');
  ...
  if (JSON.stringify(mf.deltaFiles) !== JSON.stringify(['core/access-scope.js'])) fail('base_r4s3_delta_contract_mismatch');
...
function validateSuccessor(baseDir, successorDir, baseMf) {
  ...
  if (mf.status !== 'FASE_A_PRODUCT_R4S4_MINIMAL_SUCCESSOR_CERTIFIED') fail('successor_status_mismatch');
...
const [mode, baseDir, successorDir, queriesReplacement, certFile] = process.argv.slice(2);
if (!mode || !baseDir || !successorDir || !certFile) fail('usage: prepare|verify <baseDir> <successorDir> <queriesReplacement-or-dash> <certFile>');
const baseMf = validateBase(baseDir);

if (mode === 'prepare') {
  const replacementByTarget = { 'core/queries.js': queriesReplacement };
  ...
  const mf = {
    ...baseMf,
    status: 'FASE_A_PRODUCT_R4S4_MINIMAL_SUCCESSOR_CERTIFIED',
    ...
    successorOrdinal: 4,
    packageLineage: 'R4S3 + Cliente360 bounded batched summary rootfix',
    files: rows,
  ...
  writeJson(certFile, {
    schemaVersion: 'orbit360-r4s4-minimal-successor-certification-v1',
    ok: false,
    status: 'R4S4_SUCCESSOR_STAGED_AWAITING_ZIP_VERIFY',
  ...
  Object.assign(cert, {
    ok: true,
    status: 'R4S4_MINIMAL_SUCCESSOR_DURABLE_CERTIFIED',
