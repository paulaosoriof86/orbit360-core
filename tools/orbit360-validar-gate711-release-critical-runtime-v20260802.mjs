...
const OUT = path.join(EVIDENCE_DIR, 'gate711-release-critical-runtime-v20260802.json');
const SHOTS = path.join(EVIDENCE_DIR, 'visual-sanitized-gate711-release-critical-v20260802');
...
const REQUIRED_STORE_API = Object.freeze(['all', 'get', 'where', 'insert', 'update', 'remove', '_emit']);

const report = {
  schemaVersion: 'orbit360-gate711-release-critical-runtime-v1',
  gateId: 'block7-canonical-runtime-cumulative-visual-lab-v20260801',
  baseContractVersion: '7.11.0',
  releaseCriticalScopeRevision: '7.11.1',
  ...
  academia: {
    ...
    globalBootstrapMustRemainHealthy: true,
    focusedRuntimePrerequisite: false,
    contentCompletenessBlocksRelease: false
  },
...
function stage(name) {
  report.stage = name;
  console.log('ORBIT360_GATE711_RELEASE_CRITICAL_STAGE:' + name);
}
...
async function routeSemantic(page, expected, label) {
  const state = await page.evaluate(source => {
    ...
    return { matched: new RegExp(source, 'i').test(text), textLength: text.length };
  }, expected.source);
  requireState(state.matched, 'ROUTE_SEMANTIC_MISMATCH', label);
  return state;
}
async function screenshot(page, name) {
  const file = path.join(SHOTS, name + '.png');
  const masks = [
    '#host .kcard', '#host [data-neg]', '#host [data-ges]', '#host .ops-toolbar input', '#host .ops-toolbar select',
    '#host table tbody', '#host .client-card', '#host .policy-card', '#host [data-client-id]', '#host [data-policy-id]'
  ].map(selector => page.locator(selector));
  await page.screenshot({ path: file, fullPage: true, mask: masks, maskColor: '#D8D8D8' });
  report.screenshots.push({ name: name + '.png', sanitized: true, maskedOperationalContent: true });
}
...
try {
  ...
  const auth = await bounded('existing_custom_token_signin', () => page.evaluate(async token => {
  ...
  const storeState = await page.evaluate(({ expected, requiredApi }) => {
    const store = Orbit.store;
    const counts = Object.fromEntries(Object.keys(expected).map(name => [name, (store.all(name) || []).length]));
    return {
      counts,
      apiMissing: requiredApi.filter(name => typeof store[name] !== 'function'),
      canonicalReadModel: store.__canonicalReadModelV79 === true,
      singleReadOwner: store.__singleReadOwner === true
    };
  }, { expected: EXPECTED_COUNTS, requiredApi: REQUIRED_STORE_API });
  requireState(storeState.apiMissing.length === 0, 'STORE_API_MISSING', storeState.apiMissing.join(','));
  requireState(storeState.canonicalReadModel && storeState.singleReadOwner, 'STORE_OWNER_INVALID');
  requireState(Object.entries(EXPECTED_COUNTS).every(([name, count]) => storeState.counts[name] === count), 'CANONICAL_COUNTS_MISMATCH');
  report.dataset = { operational: storeState.counts };
  report.store = storeState;
  report.checks.storeContract = true;
  report.checks.dataset = true;
  report.checks.singleReadOwner = true;
  ...
  await page.evaluate(() => {
    ...
    window.__orbitGate711ReleaseCriticalWriteGuard = { calls };
  });
  ...
  const plans = [
    {
      ...
      crm: [
        { hash: '#/cliente360', key: 'cliente360', semantic: /Cliente|Clientes|asegurado/i },
        { hash: '#/aseguradoras', key: 'aseguradoras', semantic: /Aseguradora|Aseguradoras|directorio/i },
        { hash: '#/polizas', key: 'polizas', semantic: /Póliza|Pólizas|cartera/i }
      ], ops: 'visible', leads: 'visible'
    },
    {
  ...
  for (const plan of plans) {
    ...
    for (const crmRoute of plan.crm) {
      ...
      report.roles[plan.role].crm[crmRoute.key] = { health, semantic };
  ...
  report.status = 'GATE711_RELEASE_CRITICAL_RUNTIME_PASS';
  report.classification = 'GO_LAB_RELEASE_CRITICAL_CRM_OPS_LEADS';
  report.ok = true;
} catch (error) {
  report.status = 'GATE711_RELEASE_CRITICAL_RUNTIME_FAIL';
