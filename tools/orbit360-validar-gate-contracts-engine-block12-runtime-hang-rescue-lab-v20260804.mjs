...
const REQUEST = process.env.ORBIT360_REQUEST_FILE || '.github/orbit360-requests/block12-runtime-hang-rescue-lab-v20260804.json';
...
try {
  ...
  add('LIFECYCLE_ACTIVE', lifecycle.status === 'RUNTIME_HANG_RESCUE_LAB_AUTHORIZED' && lifecycle.singleGate === true);
  add('CAPABILITIES_EXACT', caps.secrets === true && caps.firestoreRead === true && caps.writes === true && caps.authWrites === true && caps.deployDelete === true && caps.runtime === false && caps.browser === false && caps.rulesDeploy === false && caps.production === false);
  add('REQUEST_ACTIVE', request.schemaVersion === 'orbit360-block12-runtime-hang-rescue-request-v1' && request.status === 'AUTHORIZED_RUNTIME_HANG_RESCUE' && request.approved === true && request.allowedExecutions === 1 && request.consumed === false && request.replayAllowed === false && request.authorizationRef === lifecycle.authorization.source);
  ...
  result = {
    schemaVersion: 'orbit360-block12-runtime-hang-rescue-preflight-v1', gateId: GATE, contractVersion: VERSION,
