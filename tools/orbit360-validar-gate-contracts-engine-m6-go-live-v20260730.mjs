...
const files=[
 ...
 'tools/orbit360-validator-lifecycle-contract-m6-go-live-v20260730.json',
 'tools/orbit360-validar-gate-contracts-v20260717.mjs',
 '.github/workflows/orbit360-m6-product-go-live-v20260730.yml',
 'firebase.product-go-live.json','firestore.product-readonly.rules','storage.product-readonly.rules',
 'firebase.product-rollback-safe.json','firestore.product-deny-all.rules','storage.product-deny-all.rules',
...
try{
 ...
 add('SYNTAX',true);
 const auth=read('orbit360-platform/core/auth-product-runtime-p0.js'),providers=read('orbit360-platform/core/product-runtime-browser-providers-p0.js'),pre=read('orbit360-platform/core/product-prebootstrap-store-p0.js'),overlay=read('orbit360-platform/core/product-config-session-overlay-p0.js'),app=read('orbit360-platform/core/product-app-runtime-p0.js'),snapshot=read('tools/orbit360-m6-product-data-snapshot-readonly-v20260730.mjs'),smoke=read('tools/orbit360-m6-product-browser-smoke-v20260730.mjs'),workflow=read('.github/workflows/orbit360-m6-product-go-live-v20260730.yml');
 add('PRODUCT_OWNERS',auth.includes('paintIdentity')&&auth.includes('noLocalSession:true')&&!auth.includes('Andrea Beltrán')&&!localStorageOperational(auth)&&providers.includes("tenantSource:'membership_only'")&&providers.includes('MEMBERSHIP_IDENTITY_MISMATCH')&&!localStorageOperational(providers)&&pre.includes('volatile:true')&&!localStorageOperational(pre)&&overlay.includes('noLocalStorageFallback:true')&&!localStorageOperational(overlay)&&app.includes("['clientes','aseguradoras','gestiones','notificaciones']"));
 add('DATA_CONTRACT',snapshot.includes('tenantId/${TENANT}/asesores')&&snapshot.includes('advisorSourceOnly:true')&&smoke.includes('advisorBound')&&!smoke.includes('runtime.advisors===7'));
 add('WORKFLOW_SCOPE',workflow.includes('tools/orbit360-m6-go-live-request-v20260730.json')&&workflow.includes('firebase.product-go-live.json')&&workflow.includes('firebase.product-rollback-safe.json')&&workflow.includes('m6-deploy-attempted')&&workflow.includes('firestoreDataWrites!==false')&&workflow.includes('functionsDeploy!==false')&&workflow.includes("const keys=['clientes','aseguradoras','asesoresFuente','memberships','config']"));
 const go=JSON.parse(read('firebase.product-go-live.json')),rollback=JSON.parse(read('firebase.product-rollback-safe.json'));
 add('GO_LIVE_CONFIG',go.firestore&&go.firestore.rules==='firestore.product-readonly.rules'&&go.storage&&go.storage.rules==='storage.product-readonly.rules'&&go.hosting&&go.hosting.public==='orbit360-platform'&&Array.isArray(go.hosting.ignore)&&go.hosting.ignore.includes('rollback-safe/**'));
