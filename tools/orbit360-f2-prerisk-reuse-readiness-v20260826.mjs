...
const run=(root,rel,args=[],extraEnv={})=>{const r=spawnSync(process.execPath,[A(root,rel),...args],{cwd:root,encoding:'utf8',maxBuffer:32*1024*1024,env:{...process.env,ORBIT360_ROOT:root,...extraEnv}});return{status:r.status,json:parse(r.stdout)||{},stderr:String(r.stderr||'')}};
