...
try{
  ...
  // F2_RUNTIME_AUTHORIZATION_PERSIST consumes the ledger's reusable-auth pointer by design.
  // The paired preserved request remains an immutable event; rebind that already-validated
  // path in scratch only so REQUEST_MATERIALIZE can exercise its real reuse branch.
  sL.authorizationBoundary.reusableRequestPath=requestPath;fs.writeFileSync(A(scratch,LEDGER),JSON.stringify(sL,null,2)+'\n','utf8');
