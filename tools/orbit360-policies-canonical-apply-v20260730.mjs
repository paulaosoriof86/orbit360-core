...
function digestIds(ids){return sha256([...ids].sort().join('\n'));}
function validatePackage(pkg,refs){
  const errors=[];
  const add=(ok,msg)=>{if(!ok)errors.push(msg);};
  const {manifest,clients,insurers,policies,excluded}=pkg;
  add(manifest.schema_version==='orbit360-policies-canonical-private-v1','manifest_schema');
  add(manifest.tenant_id===TENANT,'manifest_tenant');
  add(Number(manifest.clients_to_create)===EXPECTED.clientsCreate,'manifest_clients');
  add(Number(manifest.restricted_insurers)===EXPECTED.insurersCreate,'manifest_insurers');
  add(Number(manifest.policies_to_create)===EXPECTED.policiesCreate,'manifest_policies');
  add(Number(manifest.policies_pending_quality)===EXPECTED.pendingPolicies,'manifest_pending');
  add(Number(manifest.excluded)===EXPECTED.excluded,'manifest_excluded');
  add(Number(manifest.receipts)===0&&Number(manifest.cartera)===0&&Number(manifest.cobros)===0,'manifest_scope');
  add(bool(manifest.real_write_executed)===false,'manifest_prewrite_only');
  ...
  for(const r of policies){
    add(clean(r.tenantId)===TENANT,'policy_tenant');add(!refs.policyIds.has(clean(r.id)),'policy_target_exists');add(allClientIds.has(clean(r.clienteId)),'policy_client_missing');add(allInsurerIds.has(clean(r.aseguradoraId)),'policy_insurer_missing');add(!clean(r.asesorId)||refs.advisorIds.has(clean(r.asesorId)),'policy_advisor_missing');
    add(clean(r.numero)&&clean(r.vigenciaInicio)&&clean(r.vigenciaFin),'policy_core_missing');add(r.carteraMaterializada===false&&r.recibosMaterializados===false&&r.cobroAplicado===false,'policy_materialization_forbidden');add(clean(r._sourceVersionKey),'policy_source_version_missing');
    if(r.requiereValidacion===true){pending++;add(clean(r.validationStatus)==='pendiente_completar','policy_pending_status');}
  }
  add(pending===EXPECTED.pendingPolicies,'pending_policy_count');
  return{errors,clientIds,insurerIds,policyIds,pending};
}
...
try{
  ...
  const bytes=fs.readFileSync(xlsxPath),physicalSha=sha256(bytes);result.packageSha256=physicalSha;if(expectedSha&&physicalSha!==expectedSha)fail('DATA_CONTRACT_FAILURE','PACKAGE_SHA_MISMATCH');
  const wb=XLSX.read(bytes,{type:'buffer'});const manifest=sheetRows(wb,'Manifest')[0]||{};result.logicalSha256=clean(manifest.logical_sha256);
  const pkg={manifest,clients:normalizeRows(sheetRows(wb,'ClientsToCreate')),insurers:normalizeRows(sheetRows(wb,'InsurersRestricted')),policies:normalizeRows(sheetRows(wb,'PoliciesToCreate')),excluded:normalizeRows(sheetRows(wb,'Excluded'))};
  ...
  const watched=['clientes','aseguradoras','asesores','polizas','recibosEsperados','carteraPrimas','cobros','finmovs'];for(const c of watched)result.before[c]=await countColl(db,c);
  if(result.before.clientes!==EXPECTED.clientsBefore||result.before.aseguradoras!==EXPECTED.insurersBefore||result.before.asesores!==EXPECTED.advisorsBefore||result.before.polizas!==EXPECTED.policiesBefore)fail('DATA_CONTRACT_FAILURE','BASELINE_COUNTS_CHANGED');
  ...
  if(mode==='DRY_RUN'){result.status='PREWRITE_READY';result.firestoreRead=true;result.firestoreWrites=0;result.operationalWrites=0;for(const c of watched)result.after[c]=result.before[c];}
    ...
    const created=[];try{
      await createChunked(db,pkg.clients.map(r=>({coll:'clientes',id:clean(r.id),data:r})),created);result.writes.clients=pkg.clients.length;
      await createChunked(db,pkg.insurers.map(r=>({coll:'aseguradoras',id:clean(r.id),data:r})),created);result.writes.insurers=pkg.insurers.length;
      await createChunked(db,pkg.policies.map(r=>({coll:'polizas',id:clean(r.id),data:r})),created);result.writes.policies=pkg.policies.length;
      for(const c of watched)result.after[c]=await countColl(db,c);
      const unchanged=['asesores','recibosEsperados','carteraPrimas','cobros','finmovs'];
      const ok=result.after.clientes===EXPECTED.clientsBefore+EXPECTED.clientsCreate&&result.after.aseguradoras===EXPECTED.insurersBefore+EXPECTED.insurersCreate&&result.after.polizas===EXPECTED.policiesCreate&&unchanged.every(c=>result.after[c]===result.before[c]);
      if(!ok)fail('DATA_CONTRACT_FAILURE','POSTWRITE_INVARIANT');
      const auditId=`policies_20260730_${physicalSha.slice(0,16)}`;const audit={tenantId:TENANT,batchId:auditId,sourceType:'polizas',packageSha256:physicalSha,logicalSha256:result.logicalSha256,counts:{clients:EXPECTED.clientsCreate,restrictedInsurers:EXPECTED.insurersCreate,policies:EXPECTED.policiesCreate,pendingPolicies:EXPECTED.pendingPolicies,excluded:EXPECTED.excluded},confirmedBy:clean(req.confirmedBy||'paula'),confirmedAt:new Date().toISOString(),reason:clean(req.reason||'Carga inicial canónica de pólizas A&S'),status:'written_controlled',rollbackAvailable:true,noReceiptWrites:true,noCarteraWrites:true,noCobroWrites:true};await db.collection('tenantId').doc(TENANT).collection('auditoriaImportaciones').doc(auditId).create(audit);created.push({coll:'auditoriaImportaciones',id:auditId});result.writes.audit=1;result.status='WRITE_PASS';result.firestoreRead=true;result.firestoreWrites=created.length;result.operationalWrites=created.length;
...
result.ok=result.status==='PREWRITE_READY'||result.status==='WRITE_PASS';fs.mkdirSync(new URL('.',`file://${process.cwd()}/${evidencePath}`).pathname,{recursive:true});fs.writeFileSync(evidencePath,JSON.stringify(result,null,2)+'\n','utf8');console.log(JSON.stringify(result,null,2));if(!result.ok)process.exit(41);
