...
const browser=read('m5-runtime-smoke-511-browser-summary.json');
const preflight=read('preflight-sanitizado.json');
const contract=read('m5-runtime-smoke-511-contract-summary.json');
...
const ok=Boolean(
  preflight&&preflight.status==='GO_GATE_CONTRACT'&&preflight.failed===0&&preflight.contractVersion==='5.0.11'&&
  contract&&contract.status==='M5_RUNTIME_SMOKE_511_CONTRACT_PASS'&&contract.failed===0&&contract.releaseCandidateHash===RC_HASH&&
  before&&before.ok===true&&after&&after.ok===true&&
  browser&&browser.ok===true&&browser.releaseCandidateHash===RC_HASH&&browser.contractVersion==='5.0.11'&&
...
const out={
  schemaVersion:'orbit360-m5-runtime-smoke-summary-v3',generatedAt:new Date().toISOString(),ok,
  status:ok?'M5_RUNTIME_SMOKE_511_CLOSED_SUCCESS':'M5_RUNTIME_SMOKE_511_FAILED_STOP_LINE',
  gateId:'block5-release-candidate-visualization-v20260728',contractVersion:'5.0.11',releaseCandidateHash:RC_HASH,
...
fs.writeFileSync(path.join(DIR,'m5-runtime-smoke-511-summary.json'),JSON.stringify(out,null,2)+'\n');
