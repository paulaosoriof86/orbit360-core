...
const report={schemaVersion:'orbit360-m6-product-browser-smoke-v1',gateId:'block6-go-live-product-v20260730',contractVersion:'6.1.0',generatedAt:new Date().toISOString(),stage:'init',checks:{},roleViews:{},networkWriteCandidates:[],visibleTechnicalCopyPredicateVersion:VISIBLE_TECHNICAL_COPY_PREDICATE_VERSION,firestoreRead:true,firestoreWrites:0,operationalWrites:0,functionsDeploy:false,production:true,containsPII:false,containsSecrets:false};
...
try{
  ...
  await page.addInitScript(pattern=>{window.__ORBIT_M6_TECH_PATTERN__=pattern;},TECHNICAL_COPY_PATTERN);
  ...
  await page.waitForFunction(()=>window.Orbit&&Orbit.productAppP0&&Orbit.productAppP0.isStarted&&Orbit.productAppP0.isStarted()===true,{timeout:60000,polling:100});
  await acceptLegal(page);
  report.stage='runtime_contract';const runtime=await page.evaluate(()=>{const s=Orbit.store&&Orbit.store._productStatus?Orbit.store._productStatus():{};const user=Orbit.auth&&Orbit.auth.productUser||{};const allowed=Orbit.session&&Orbit.session.allowedRoles?Orbit.session.allowedRoles():[];const header=document.querySelector('#tb-user .who b');return{ready:s.ready===true,status:String(s.status||''),noFallback:s.noFallback===true,writeEnabled:s.writeEnabled===true,productReadOnly:user.productReadOnly===true,tenantBound:Boolean(user.tenantId),allowedRoles:allowed.slice(),headerText:String(header&&header.textContent||'').trim(),clients:(Orbit.store.all('clientes')||[]).length,insurers:(Orbit.store.all('aseguradoras')||[]).length,advisors:(Orbit.store.all('asesores')||[]).length};});
  need(runtime.ready&&runtime.status==='ready-read-only'&&runtime.noFallback&&!runtime.writeEnabled&&runtime.productReadOnly&&runtime.tenantBound,'PRODUCT_RUNTIME_NOT_READY');
  need(runtime.clients===414&&runtime.insurers===26&&runtime.advisors===7,'PRODUCT_BASELINE_INVALID',JSON.stringify({c:runtime.clients,i:runtime.insurers,a:runtime.advisors}));
  REQUIRED_ROLES.forEach(r=>need(runtime.allowedRoles.includes(r),'ASSIGNED_ROLE_MISSING',r));
  need(runtime.headerText&&runtime.headerText!=='Andrea Beltrán','AUTHENTICATED_HEADER_IDENTITY_NOT_PAINTED');report.runtime=runtime;report.checks.runtime=true;
  ...
  report.stage='final';need(report.networkWriteCandidates.length===0,'NETWORK_WRITE_CANDIDATE_DETECTED',String(report.networkWriteCandidates.length));report.checks.roles=true;report.ok=true;report.status='M6_PRODUCT_BROWSER_SMOKE_PASS';
}catch(error){report.ok=false;report.status='M6_PRODUCT_BROWSER_SMOKE_FAIL';report.error=clean(error&&error.message||error);report.failureStage=report.stage;process.exitCode=41;}finally{if(browser)await browser.close().catch(()=>{});save();}
