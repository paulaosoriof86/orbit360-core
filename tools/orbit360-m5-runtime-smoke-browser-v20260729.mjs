...
const requireState=(v,code,detail='')=>{if(!v)throw new Error(`${code}${detail?':'+detail:''}`)};
...
async function validateAccessBoundary(page){
 const state=await page.evaluate(async()=>{const user=Orbit.auth&&Orbit.auth.productUser||null;const allowed=Orbit.session.allowedRoles();const before=Orbit.session.rol();let blockedReason='';const handler=event=>{blockedReason=String(event&&event.detail&&event.detail.reason||'')};document.addEventListener('orbit:session:blocked',handler,{once:true});const blocked=Orbit.session.set('Finanzas');await new Promise(resolve=>setTimeout(resolve,50));const after=Orbit.session.rol();const productAdvisor=String(user&&user.advisorId||'');const sessionAdvisor=String(Orbit.session.asesorId&&Orbit.session.asesorId()||'');return{sessionVersion:String(Orbit.session.VERSION||''),taxonomyVersion:String(Orbit.productRoleTaxonomyP0&&Orbit.productRoleTaxonomyP0.VERSION||''),membershipBound:Orbit.session.membershipBound(),requiresMembership:Orbit.session.requiresMembership(),allowedRoles:allowed.slice().sort(),activeRole:before,assignedRoleCount:allowed.length,writeAuthorized:Orbit.session.writeAuthorized===true,membershipWrites:Orbit.session.membershipWrites===true,unauthorizedRoleAllowed:Orbit.session.roleAllowed('Finanzas'),unauthorizedSetResult:blocked,blockedReason,roleAfterUnauthorizedProbe:after,roleUnchanged:before===after,advisorProjectionConsistent:productAdvisor===sessionAdvisor,advisorPresent:Boolean(sessionAdvisor)}});
 ...
 requireState(state.advisorProjectionConsistent,'ADVISOR_NOT_MEMBERSHIP_DERIVED');
 requireState(state.writeAuthorized===false&&state.membershipWrites===false,'ACCESS_OWNER_WRITE_CAPABILITY_INVALID');
 report.accessBoundary=state;report.checks.accessBoundary=true;
}
async function validateDataset(page){
 const data=await page.evaluate(()=>{const rows=Orbit.store.all('clientes')||[];const countries={GT:0,CO:0,REQUIERE_VALIDACION:0},types={Persona:0,Empresa:0};rows.forEach(row=>{const p=String(row.pais||'');if(Object.prototype.hasOwnProperty.call(countries,p))countries[p]+=1;const t=String(row.tipo||'');if(Object.prototype.hasOwnProperty.call(types,t))types[t]+=1});return{clients:rows.length,insurers:(Orbit.store.all('aseguradoras')||[]).length,advisors:(Orbit.store.all('asesores')||[]).length,countries,types,missingCurrency:rows.filter(x=>!String(x.moneda||'').trim()).length,invalidCountryCurrency:rows.filter(x=>(x.pais==='GT'&&x.moneda!=='GTQ')||(x.pais==='CO'&&x.moneda!=='COP')).length}});
 requireState(data.clients===414,'CLIENT_COUNT_INVALID',String(data.clients));requireState(data.insurers===26,'INSURER_COUNT_INVALID',String(data.insurers));requireState(data.advisors===7,'ADVISOR_COUNT_INVALID',String(data.advisors));
 ...
 report.dataBaseline=data;report.checks.dataBaseline=true;
}
async function validateResponsiveTitle(page,label){
 const state=await page.evaluate(()=>{const visible=e=>{if(!e)return false;const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>0&&r.height>0};const title=document.querySelector('.mod-band .mb-tt h2,.fichahdr h2,.m1-asg-hero h2');const r=title&&title.getBoundingClientRect();return{width:innerWidth,titleVisible:visible(title),titleWithinViewport:Boolean(r&&r.left>=-2&&r.right<=innerWidth+2),titleOverflow:Boolean(title&&title.scrollWidth>title.clientWidth+2),roleSelectorVisible:visible(document.getElementById('rol-sel')),technicalCopyVisible:/Firebase|Firestore|localStorage|mock|smoke|dry-run|backend|LAB/i.test(document.body.innerText)}});
...
async function validateClient360(page,label,role){
 await page.evaluate(()=>{location.hash='#/cliente360'});await page.waitForFunction(()=>location.hash==='#/cliente360'&&document.querySelector('#host'),null,{timeout:15000});
 await page.locator('#host table.tbl').first().waitFor({state:'visible',timeout:20000});
 const state=await page.evaluate(()=>{const all=Orbit.store.all('clientes')||[];const access=Orbit.access||{};const scope=access.dataScope?access.dataScope('cliente360'):'all';const actor=access.actorAdvisorId?String(access.actorAdvisorId()||''):'';const scoped=access.filter?access.filter('clientes',all,'cliente360'):all;return{scope,total:all.length,scoped:scoped.length,actorPresent:Boolean(actor),ownConsistent:scope!=='own'||scoped.every(row=>String(row.asesorId||'')===actor),tableVisible:Boolean(document.querySelector('#host table.tbl')),searchVisible:Boolean(document.getElementById('f-q')||document.querySelector('.tb-search input'))}});
 requireState(state.total===414&&state.scoped>0&&state.scoped<=414,'CLIENT_SCOPE_COUNT_INVALID',`${label}:${state.scoped}`);
 requireState(state.ownConsistent,'CLIENT_SCOPE_OWN_INCONSISTENT',label);requireState(state.tableVisible&&state.searchVisible,'CLIENT_UI_MISSING',label);
 const responsive=await validateResponsiveTitle(page,label+'Client360');report.roleViews[label]={role,client360:state,responsive};
}
async function validateInsurers(page,label,role){
 await page.evaluate(()=>{location.hash='#/aseguradoras'});const cards=page.locator('.asg-grid [data-asg]');await cards.first().waitFor({state:'visible',timeout:20000});const count=await cards.count();requireState(count===26,'INSURER_CARD_COUNT_INVALID',`${label}:${count}`);
 await cards.first().click();await page.locator('#asg-ficha').waitFor({state:'visible',timeout:15000});
 const state=await page.evaluate(()=>{const module=Orbit.modules&&Orbit.modules.aseguradoras;return{ownerReady:Boolean(module&&module.__ownerKnowledgeV20260717&&module.__tenantOrderV20260717&&module.__consumerGatesSeparatedV20260717),editVisible:Boolean(document.getElementById('af-editar')),saveVisible:Boolean(document.getElementById('af-guardar')),knowledgeTab:Boolean(document.querySelector('[data-tab="tarifas"]'))}});
 requireState(state.ownerReady&&state.knowledgeTab,'INSURER_OWNER_OR_KNOWLEDGE_MISSING',label);requireState(!state.saveVisible,'INSURER_SAVE_VISIBLE_IN_READ_MODE',label);
 if(role==='Asesor')requireState(!state.editVisible,'ADVISOR_INSURER_EDIT_VISIBLE');
 const responsive=await validateResponsiveTitle(page,label+'Insurers');Object.assign(report.roleViews[label],{insurers:{count,...state},insurerResponsive:responsive});
}
async function validateMobileMenu(page){
 await page.locator('#burger').click();await page.waitForTimeout(350);const labels=await page.locator('#sidebar .nav-link:visible').evaluateAll(nodes=>nodes.map(n=>String(n.textContent||'').replace(/\s+/g,' ').trim()));
 requireState(labels.length>1&&labels.some(x=>/Cliente/i.test(x))&&labels.some(x=>/Aseguradoras/i.test(x)),'MOBILE_MENU_INCOMPLETE',String(labels.length));report.mobileMenuVisibleModules=labels.length;report.checks.mobileMenu=true;
}
