...
const EXPECTED_DIAGNOSIS='DIAGNOSE_RUNTIME_CAPABILITY_COMPOSITION_VALIDATION_MECHANISM_ROOT_CAUSE_STOP_RETRY';
...
try{
  ...
  if(AState.phase!=='PRODUCTION_SMOKE_PASS'||AState.status!=='PRODUCTION_GO_LIVE_PASS'||AState.productFrozen!==true||AState.dataFrozen!==true)throw new Error('D3_MECHANISM_ROOTFIX_EXPECTED_FROZEN_POST_GO_LIVE_STATE_MISSING');
  if(next!==EXPECTED_DIAGNOSIS)throw new Error(`D3_MECHANISM_ROOTFIX_NEXT_ACTION_MISMATCH:${next}`);
  if(H.transitionId!==FAILED_TRANSITION||H.ok!==false||H.classification!=='PIPELINE_MECHANISM_FAILURE'||!Number.isInteger(Number(H.runId))||Number(H.runId)<=0)throw new Error('D3_MECHANISM_ROOTFIX_CURRENT_FAILURE_NOT_ELIGIBLE');
