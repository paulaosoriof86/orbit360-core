...
const L=J(P.ledger),G=J(P.authority);
...
if(transition==='CONTROL_PLANE_REGRESSION_REOPEN'){
  ...
  L.continuityControl={...L.continuityControl,status:'CONTROL_PLANE_REGRESSION_OPEN_STOP_RETRY',classification:'PIPELINE_MECHANISM_FAILURE',secondaryClassification:null,compositeInvariantStatus:'CONTROL_PLANE_REGRESSION_REQUIRES_FULL_PATH_REVALIDATION',evidenceFreshnessValidated:false,workflowHandshakeValidated:false,stopRetryMechanicallyEnforced:true,evidenceLifecycleClassWideValidated:false,exactF2SourcePathSelftestValidated:false,latestControlPlaneRegression:{runId:run,jobId:Number(F.jobId||0),technicalPullRequest:Number(F.technicalPullRequest||0),evidencePath:controlPlaneFailureEvidence,classification:F.classification,failureCode:F.failureCode,rootCauseCode:String(F.rootCause?.code||''),sameStageSecondFailure:F.sameStageSecondFailure?.active===true,replayAllowed:false,reducedAtLedgerRevision:L.revision,reducedAtUtc:ts,status:'OPEN_AWAITING_SOURCE_ONLY_ROOTFIX_AND_FRESH_HANDSHAKE'}};
  ...
  L.nextAction={id:'RUN_EXACT_F2_SOURCE_PATH_CLASSWIDE_EVIDENCE_SELFTEST',description:'Revalidate the exact F2 pre-authorization source path and class-wide evidence lifecycle before any fresh authorization.',runtimeAllowed:false};
...
L.nextAction={id:'AWAIT_EXPLICIT_F2_RUNTIME_AUTHORIZATION_ONE_SHOT',description:'Await one fresh explicit F2 runtime authorization only after exact-path class-wide control-plane closure.',runtimeAllowed:false};
