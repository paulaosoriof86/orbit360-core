...
async function waitDeferred(page){await page.waitForFunction(()=>{const st=Orbit?.store?._productStatus?.()||{},c=new Set(Array.isArray(st.serverConfirmedCollections)?st.serverConfirmedCollections:[]);return ['clientes','polizas','cobros','aseguradoras','vehiculos','recibosEsperados','carteraPrimas','negocios'].every(x=>c.has(x))&&(Orbit?.store?.all?.('vehiculos')||[]).length>0&&(Orbit?.store?.all?.('negocios')||[]).length>0&&(Orbit?.store?.all?.('recibosEsperados')||[]).length>0&&(Orbit?.store?.all?.('carteraPrimas')||[]).length>0;},null,{timeout:23000});}
...
async function reloadOnRoute(page,key){const started=Date.now();await page.reload({waitUntil:'domcontentloaded',timeout:20000});await page.waitForFunction(()=>!!Orbit?.productAppP0&&!!Orbit?.productRuntimeBrowserProvidersP0,null,{timeout:7000});await waitApp(page,key);await waitDeferred(page);return Date.now()-started;}
...
function snapshotScript(){
  ...
  return {
    ...
    counts:{clientes:(Orbit.store.all('clientes')||[]).length,polizas:(Orbit.store.all('polizas')||[]).length,vehiculos:(Orbit.store.all('vehiculos')||[]).length,recibosEsperados:receipts.length,carteraPrimas:cartera.length,cobros:cobros.length}
  };
}
...
try{
  ...
  await initial(page,token);await waitDeferred(page);const base=await page.evaluate(snapshotScript);need(base.activeRole===TARGET_ROLE,'BASE_ROLE_INVALID');need(base.vehicle&&base.policyByVehicle&&base.vehicle.clienteId===base.policyByVehicle.clienteId&&base.vehicle.polizaId===base.policyByVehicle.id,'BASE_VEHICLE_RELATION_INVALID');need(base.golden&&base.golden.numero==='AUTO39012'&&base.golden.prima===1800&&Math.abs(base.golden.primaTotal-2678.53)<0.001,'BASE_GOLDEN_POLICY_INVALID');need(base.sharedBusiness,'BASE_OPS_LEADS_SHARED_BUSINESS_MISSING');need(base.receipt&&base.cartera&&base.cobro,'BASE_FINANCIAL_READMODEL_MISSING');

  await gotoRoute(page,'#/cliente360?c='+encodeURIComponent(base.clientId)+'&t=vehiculos','cliente360');const msClient=await reloadOnRoute(page,'cliente360');const c1=await page.evaluate(snapshotScript);sameRecord(base,c1,'vehicle');sameRecord(base,c1,'policyByVehicle');need(await page.locator('#host .ftab[data-tab="vehiculos"].active').count()===1,'CLIENTE360_VEHICLE_TAB_NOT_RESTORED');need(await page.getByRole('button',{name:/^Ver póliza$/i}).count()>0,'CLIENTE360_VEHICLE_ACTION_NOT_RESTORED');ev.checks.cliente360Vehiculos={pass:true,reloadMs:msClient,before:sanitize(base).vehicle,after:sanitize(c1).vehicle};

  await gotoRoute(page,'#/ops','ops');need(await page.locator(`[data-neg="${base.sharedBusiness.id}"]`).count()>0,'OPS_SHARED_BUSINESS_NOT_RENDERED_BEFORE_RELOAD');const msOps=await reloadOnRoute(page,'ops');const o1=await page.evaluate(snapshotScript);sameRecord(base,o1,'sharedBusiness');need(await page.locator(`[data-neg="${base.sharedBusiness.id}"]`).count()>0,'OPS_SHARED_BUSINESS_NOT_RENDERED_AFTER_RELOAD');ev.checks.ops={pass:true,reloadMs:msOps,businessIdHash:hash(base.sharedBusiness.id),samePersistedRecord:true};

  await gotoRoute(page,'#/leads','leads');need(await page.locator(`[data-neg="${base.sharedBusiness.id}"]`).count()>0,'LEADS_SHARED_BUSINESS_NOT_RENDERED_BEFORE_RELOAD');const msLeads=await reloadOnRoute(page,'leads');const l1=await page.evaluate(snapshotScript);sameRecord(base,l1,'sharedBusiness');need(await page.locator(`[data-neg="${base.sharedBusiness.id}"]`).count()>0,'LEADS_SHARED_BUSINESS_NOT_RENDERED_AFTER_RELOAD');ev.checks.leads={pass:true,reloadMs:msLeads,businessIdHash:hash(base.sharedBusiness.id),samePersistedRecord:true,synchronizedWithOps:true};

  await gotoRoute(page,'#/polizas','polizas');const msPol=await reloadOnRoute(page,'polizas');const p1=await page.evaluate(snapshotScript);sameRecord(base,p1,'golden');need(p1.golden.numero==='AUTO39012'&&p1.golden.prima===1800&&Math.abs(p1.golden.primaTotal-2678.53)<0.001,'POLIZAS_GOLDEN_DRIFT_AFTER_RELOAD');ev.checks.polizas={pass:true,reloadMs:msPol,golden:sanitize(p1).golden};

  await gotoRoute(page,'#/cobros','cobros');const msCob=await reloadOnRoute(page,'cobros');const f1=await page.evaluate(snapshotScript);sameRecord(base,f1,'receipt');sameRecord(base,f1,'cartera');sameRecord(base,f1,'cobro');need(f1.counts.recibosEsperados>0&&f1.counts.carteraPrimas>0&&f1.counts.cobros>0,'FINANCIAL_COUNTS_INVALID_AFTER_RELOAD');ev.checks.financialReadModels={pass:true,reloadMs:msCob,receipt:sanitize(f1).receipt,cartera:sanitize(f1).cartera,cobro:sanitize(f1).cobro,counts:f1.counts,semanticSeparation:true};
