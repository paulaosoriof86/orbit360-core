...
try{
  ...
  const m6=read(files.m6),matrix=read(files.sourceMatrix),anti=read(files.antiDeviation),model=read(files.model),audit=read(files.audit),hub=read(files.hub),legacy=read(files.legacy),normalizer=read(files.normalizer),wire=read(files.wire),owner=read(files.owner),refinements=read(files.refinements),access=read(files.access);
  ...
  for(const rel of [files.module,files.legacy,files.normalizer,files.wire,files.owner,files.refinements,files.access,files.testNormalizer,files.testWire,files.testOwner,files.testOwnerB,files.validateOwner,files.validateOwnerB,files.testModel])execFileSync(process.execPath,['--check',rel],{cwd:ROOT,stdio:'pipe'});
  add('SYNTAX',true);
