...
try{
 ...
 check('GLOBAL_FREEZE',globalFreeze.status==='M5_RUNTIME_SMOKE_AUTHORIZED_ONCE_REQUEST_CREATED'&&globalFreeze.contractVersion==='5.0.5'&&globalFreeze.releaseCandidate?.remoteParity===true&&globalFreeze.authorization?.runtimeSmokeAuthorized===true&&globalFreeze.authorization?.allowedRuntimeSmokeExecutions===1&&globalFreeze.authorization?.runtimeSmokeRequestCreated===true&&globalFreeze.authorization?.hostingDeployAuthorized===false);
 check('AUTH_VERSION',auth.contractVersion==='5.0.5'&&auth.explicitAuthorization===true);
 check('AUTH_ONE_SHOT',auth.runtimeSmokeAuthorized===true&&auth.allowedExecutions===1&&auth.requestCreated===true&&auth.packageCheck?.runId===30413254641&&auth.packageCheck?.status==='M5_RUNTIME_SMOKE_PACKAGE_CHECK_PASS');
 ...
 check('FREEZE_STATUS',freeze.status==='REQUEST_CREATED_AWAITING_ONE_RUNTIME_SMOKE_LAB'&&freeze.authorization?.runtimeSmokeAuthorized===true&&freeze.authorization?.allowedExecutions===1&&freeze.authorization?.requestCreated===true&&freeze.packageCheck?.runId===30413254641&&freeze.packageCheck?.status==='PASS');
 ...
 check('LIFECYCLE',lifecycle.gateContractVersion==='5.0.5'&&lifecycle.executionProfile?.phase==='LAB_RUNTIME_GATE'&&lifecycle.authorization?.requestCreated===true&&lifecycle.baseline?.packageCheckRunId===30413254641);
