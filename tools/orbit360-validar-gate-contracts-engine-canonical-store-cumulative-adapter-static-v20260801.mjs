...
try{
  ...
  add('AUTHORIZATION',lifecycle.authorization?.explicit===true&&lifecycle.authorization?.allowedExecutions===2&&lifecycle.authorization?.attemptsUsed===1&&lifecycle.authorization?.resumeAttempt===2&&lifecycle.authorization?.additionalAttemptsAfterResume===false&&lifecycle.authorization?.consumed===false&&lifecycle.authorization?.authorizationRef==='user_proceed_definitive_solutions_no_trial_error_20260801');
  add('FIRST_ATTEMPT_CLASSIFIED',lifecycle.attempts?.first?.run===30732248630&&lifecycle.attempts?.first?.classification==='MIXED_VALIDATOR_STALE_AND_DORMANT_SCOPE_FALSE_POSITIVE'&&lifecycle.attempts?.first?.productDefectConfirmed===false&&lifecycle.attempts?.first?.firestoreWrites===0);
  add('REQUEST',request.schemaVersion==='orbit360-canonical-store-cumulative-adapter-static-request-v1'&&request.gateId===GATE&&request.contractVersion===VERSION&&request.approved===true&&request.allowedExecutions===1&&request.consumed===false&&request.resumeAttempt===2);
  ...
  add('SCOPE',request.scope?.canonicalCollections===7&&request.scope?.singleReadOwner==='Orbit.store'&&request.scope?.preservePublicApi===true&&request.scope?.excludeSeedsOperationally===true&&request.scope?.preserveRequiresValidation===true&&request.scope?.scanAllModules===true&&request.scope?.allowedRuntimeDeltaFiles===5);
