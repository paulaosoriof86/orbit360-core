...
try{
  ...
  add('AUTHORIZATION',lifecycle.authorization?.explicit===true&&lifecycle.authorization?.allowedExecutions===1&&lifecycle.authorization?.consumed===false&&lifecycle.authorization?.authorizationRef==='user_proceed_definitive_solutions_no_trial_error_20260801');
  add('REQUEST',request.schemaVersion==='orbit360-canonical-store-cumulative-adapter-static-request-v1'&&request.gateId===GATE&&request.contractVersion===VERSION&&request.approved===true&&request.allowedExecutions===1&&request.consumed===false);
