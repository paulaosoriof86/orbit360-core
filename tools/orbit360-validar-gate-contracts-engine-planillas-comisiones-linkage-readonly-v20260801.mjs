...
check('LIFECYCLE_STATUS',lifecycle.status==='PLANILLAS_RECEIPT_LINK_DIAGNOSTIC_ACTIVE');
check('MODE',lifecycle.executionProfile?.mode==='READ_ONLY_PLANILLAS_RECEIPT_LINK_DIAGNOSTIC'&&marker.mode==='READ_ONLY_PLANILLAS_RECEIPT_LINK_DIAGNOSTIC');
...
check('POLICY_IDENTITY',lifecycle.policyIdentityDiagnostic?.run===30719208561&&lifecycle.policyIdentityDiagnostic?.resolved===49&&lifecycle.policyIdentityDiagnostic?.holds===16&&lifecycle.policyIdentityDiagnostic?.previousHoldsResolved===39);
check('RECEIPT_STATIC',lifecycle.receiptResolverStatic?.run===30719316572&&lifecycle.receiptResolverStatic?.checks===14&&lifecycle.receiptResolverStatic?.fixtures===6&&lifecycle.receiptResolverStatic?.status==='STATIC_RECEIPT_LINK_RESOLVER_PASS');
check('EXPECTED_RECEIPTS',lifecycle.expectedReceiptDiagnostic?.processed===49&&lifecycle.expectedReceiptDiagnostic?.resolved===5&&lifecycle.expectedReceiptDiagnostic?.holds===44&&lifecycle.expectedReceiptDiagnostic?.excludedPolicyHolds===16&&lifecycle.expectedReceiptDiagnostic?.existingCobroLinked===0);
...
check('SECURITY',lifecycle.security?.noPaymentDateReceiptSelection===true&&lifecycle.security?.strongReferenceBeforeAmount===true);
...
const evidence={schemaVersion:'orbit360-planillas-comisiones-receipt-link-gate-preflight-v1',gateId:GATE,contractVersion:VERSION,status:ok?'GO_GATE_CONTRACT':'VALIDATOR_STALE',classification:ok?'PLANILLAS_RECEIPT_LINK_DIAGNOSTIC_READY':'PIPELINE_MECHANISM_FAILURE',canonicalPhase:'LAB_DATA_CONTRACT_REPAIR_DRYRUN',phase:'READ_ONLY_PLANILLAS_RECEIPT_LINK_DIAGNOSTIC',requestState:'ACTIVE_READONLY_RECEIPT_DIAGNOSTIC',privatePackageVerifiedByContract:ok,packageCanonicalization:'node-json-stable-sort-v1',sourceDryrunClosed:lifecycle.sourceDryrunClosed===true,sourceRows:Number(marker.sourceCut?.rowsObserved||0),policyResolved:Number(lifecycle.policyIdentityDiagnostic?.resolved||0),policyHolds:Number(lifecycle.policyIdentityDiagnostic?.holds||0),receiptDiagnosticRows:Number(lifecycle.expectedReceiptDiagnostic?.processed||0),expectedReceiptResolved:Number(lifecycle.expectedReceiptDiagnostic?.resolved||0),expectedReceiptHolds:Number(lifecycle.expectedReceiptDiagnostic?.holds||0),executionAuthorized:false,labWriteAuthorized:false,writeEligible:0,financeActivated:false,expectedBaseline:marker.expectedBaseline||{},failed:failed.length,failedCheckIds:failed,dataAccess:false,secretAccess:false,operationalWrites:0,evidenceWrites:1,secretsRead:false,firestoreRead:false,runtimeExecuted:false,browserExecuted:false,rulesApplied:false,deployExecuted:false,productionTouched:false,containsPII:false,containsPolicyNumbers:false,containsAmounts:false,containsIds:false,containsSecrets:false};
