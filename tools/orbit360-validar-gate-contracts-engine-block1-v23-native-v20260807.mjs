...
const MATRIX_REL = 'tools/orbit360-block1-native-matrix-v23-v20260807.mjs';
...
if(overlay.gateId!==GATE_ID || overlay.contractVersion!==CONTRACT || overlay.block!==1) stop('V23_OVERLAY_IDENTITY_MISMATCH',`${overlay.gateId}:${overlay.contractVersion}`,phase);
...
if(!overlay.mechanism || overlay.mechanism.nativeRuntimeArtifact!==MATRIX_REL || overlay.mechanism.generatedFromPriorArtifact!==false || overlay.mechanism.textualTransform!==false || overlay.mechanism.sourceSurgery!==false) stop('V23_NATIVE_MECHANISM_INVALID','overlay mechanism',phase);
...
const syntaxFiles=[MATRIX_REL,OBSERVER_REL,ADJUDICATOR_REL,SOURCE_TEST_REL];
...
if(importCheck.status!==0 || !/PASS_V23_NATIVE_MATRIX_IMPORT/.test(importCheck.stdout||'')) stop('V23_EXACT_NATIVE_IMPORT_FAILED',importCheck.stderr||importCheck.stdout,phase);
...
if(phase===SOURCE_PHASE){
  ...
  const output={schemaVersion:'orbit360-gate-contract-preflight-v23-native-block1',gateId:GATE_ID,contractVersion:CONTRACT,diagnosticRevision:'native-block1-matrix-owner-20260807-v1',executionPhase:SOURCE_PHASE,status:'PASS_GATE_CONTRACT_SOURCE_V23',classification:'PIPELINE_MECHANISM_FAILURE_CORRECTED_SOURCE_ONLY',total:18,passed:18,failed:0,failedCheckIds:[],canonicalOwnerScope:'v23_native_overlay_replace_historical_css_owner',runtimeContractScope:'v23_native_exact_artifact',sourceTransformed:false,nativeRuntimeArtifact:MATRIX_REL,sharedObserver:OBSERVER_REL,universeAdjudicator:ADJUDICATOR_REL,authorizedBaseHead:AUTHORIZED_BASE,requestPresent:false,executionAuthorized:false,secretAccessAuthorized:false,firestoreReadAuthorized:false,writeAuthorized:false,runtimeAuthorized:false,browserAuthorized:false,hostingDeployAuthorized:false,hostingDeploysMaximum:0,functionsDeployAuthorized:false,rulesDeployAuthorized:false,productionAuthorized:false,dataAccess:false,secretAccess:false,secretsRead:false,firestoreRead:false,operationalWrites:0,runtimeExecuted:false,browserExecuted:false,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false,ok:true}; write(output); console.log(JSON.stringify(output,null,2)); process.exit(0);
}
