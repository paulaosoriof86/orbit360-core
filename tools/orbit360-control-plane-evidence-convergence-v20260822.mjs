...
const C=J(P.contract),L=J(P.ledger),pkg=J(P.pkg),B=J(P.boundary),G=J(P.authority),SL=J(P.sourceLife),RL=J(P.runtimeLife),live=J(P.live),idx=J(P.index),ptr=J(P.pointer),M=J(P.metadata);
...
need(L.continuityControl?.canonicalityConvergenceVersion==='v1-20260822','LEDGER_CONVERGENCE_VERSION');
need(Number(L.progress?.productionRouteProgressPct)>=75,'LEDGER_PROGRESS_BELOW_MACRO2');
...
const workflowIsFreeze=wf.includes('MACRO3_SOURCE_REPAIR_FREEZE_20260823');
if(!workflowIsFreeze){
  need(wf.includes('CONTROL_PLANE_CONVERGENCE_V1')&&wf.includes('MACRO3_INLINE_F2_V1'),'WORKFLOW_CONVERGENCE_CONTRACT');
...
if(authActive){
  need(authorizationPersisted,'ACTIVE_AUTH_WITHOUT_RECORD');
  need(authorizationRecordPath&&exists(authorizationRecordPath),'ACTIVE_AUTH_RECORD_MISSING');
}
if(requestActive){
  need(authActive&&authorizationPersisted,'ACTIVE_REQUEST_WITHOUT_AUTH');
  need(activeRequestPath&&exists(activeRequestPath),'ACTIVE_REQUEST_FILE_MISSING');
}
if(attemptAccepted){
...
if(!authActive){
  need(!requestActive&&!authorizationPersisted&&!attemptAccepted,'INERT_BOUNDARY_HAS_ACTIVE_BINDING');
}
