...
need(files.receiptsProjection.includes('function rebuildReceiptClassIndex()')&&files.receiptsProjection.includes('invalidateReceiptClassIndex()'),'B2_R7_RECEIPT_CALENDAR_INDEX_MISSING');
need(!files.receiptsProjection.includes('tries>100')&&!files.receiptsProjection.includes('setInterval(function(){\n      tries++;reconcileOwners()'),'B2_R7_RECEIPT_RECONCILE_TIGHT_LOOP_REMAINS');
need(files.receiptsProjection.includes(".filter(activePolicy)")&&files.receiptsProjection.includes("Todas las pólizas"),'B2_R7_RECEIPT_FILTER_ACTIVE_POLICY_MISSING');
need(files.detail.includes("const schedule = receiptSchedule(policyId);")&&!files.detail.includes("const expected = (S().all('recibosEsperados') || []).filter(r => r.polizaId === policyId)"),'B2_R7_POLICY_DETAIL_RAW_RECEIPT_BYPASS_REMAINS');
need(files.detail.includes("S().__productReadOnlyP0 === true")&&files.detail.includes('gi-payment-overview')&&files.detail.includes('gi-payment-total'),'B2_R7_DETAIL_PERFORMANCE_OR_PAYMENT_HIERARCHY_MISSING');
need(files.client.includes('data-c360-authoritative-loading')&&files.client.includes("serverConfirmedCollections")&&files.client.includes("['clientes','polizas','cobros','asesores']"),'B2_R7_CLIENT_KPI_READY_BARRIER_MISSING');
need(files.client.includes("const key=plateKey?('plate:'+plateKey)")&&files.client.includes('function vehiclePlateKey'),'B2_R7_VEHICLE_PLATE_CANONICAL_GROUPING_MISSING');
need(files.client.includes('data-reported-payment-evidence')&&files.client.includes('Pago reportado · por validar'),'B2_R7_REPORTED_PAYMENT_VISIBLE_ROW_MISSING');
