...
const EXECUTION_ROOT = process.cwd();
const SOURCE_ROOT = path.resolve(process.env.ORBIT360_AUTH_MEMBERSHIP_SOURCE_ROOT || EXECUTION_ROOT);
const OUTPUT_INPUT = process.env.ORBIT360_AUTH_MEMBERSHIP_EVIDENCE ||
  'orbit360-platform/runtime-gate-crm-v20260716/rc12-auth-membership-antiregression.json';
const OUTPUT_PATH = path.isAbsolute(OUTPUT_INPUT)
  ? path.normalize(OUTPUT_INPUT)
  : path.resolve(EXECUTION_ROOT, OUTPUT_INPUT);
...
function read(rel) {
  return fs.readFileSync(path.resolve(SOURCE_ROOT, rel), 'utf8');
}
...
function compileJavascript(source, id) {
  try {
    ...
    return { id, ok: false, detail: String(error && error.message || error).slice(0, 700) };
  }
}
...
const checks = [
  ...
  check('SOURCE_ROOT_RESOLVED', path.isAbsolute(SOURCE_ROOT) && SOURCE_ROOT !== ''),
  check('OUTPUT_PATH_RESOLVED', path.isAbsolute(OUTPUT_PATH) && OUTPUT_PATH !== '')
];

const failed = checks.filter(item => !item.ok);
const result = {
  schemaVersion: 'orbit360-auth-membership-antiregression-rootfix-v3',
  ...
  sourceRootDigest: sha256(SOURCE_ROOT),
  outputDirectoryDigest: sha256(path.dirname(OUTPUT_PATH)),
  outputPathAbsolute: path.isAbsolute(OUTPUT_PATH),
  sourceRootIndependentFromOutput: path.dirname(OUTPUT_PATH) !== SOURCE_ROOT,
...
if (!fs.existsSync(OUTPUT_PATH)) throw new Error('EVIDENCE_OUTPUT_NOT_OBSERVABLE');
