...
try{
  ...
  add('SYNTAX',true);
  add('SMOKE_VERSION',smoke.includes("schemaVersion:'orbit360-m6-product-browser-smoke-v2'")&&smoke.includes("contractVersion:'6.1.4'")&&smoke.includes("validatorRevision:'20260730.2'"));
  ...
  const out={schemaVersion:'orbit360-gate-contract-preflight-m6-smoke-validator-remediation-v1',gateId:GATE,contractVersion:VERSION,executionPhase:'M6_PRODUCT_SMOKE_VALIDATOR_REMEDIATION_STATIC',status:failed.length?'VALIDATOR_STALE':'GO_GATE_CONTRACT',classification:failed.length?'VALIDATOR_STALE':null,total:checks.length,passed:checks.length-failed.length,failed:failed.length,failedCheckIds:failed.map(c=>c.id),checks,productFrozen:true,sourceRun:30519954902,sourceArtifact:8750370291,rootCause:'PLAYWRIGHT_WAITFORFUNCTION_OPTIONS_PASSED_AS_ARG',smokeValidatorRevision:'20260730.2',productRemoteFailClosed:true,storageDeferredFailClosed:true,dataAccess:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,operationalWrites:0,runtimeExecuted:false,browserExecuted:false,rulesApplied:false,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};
