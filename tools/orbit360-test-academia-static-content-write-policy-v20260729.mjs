...
const store={
  ...
  init(seed){durableCalls.push({method:'init',collection:'seed'});return this;},
  on(_collection,callback){listener=callback;return()=>{};},
  _emit(){}
};
const events=[];
const context={window:{Orbit:{store}},document:{dispatchEvent(event){events.push(event&&event.type||'');}},CustomEvent:class CustomEvent{constructor(type,init){this.type=type;this.detail=init&&init.detail||{};}},setTimeout(fn){fn();return 1;},clearTimeout(){},console};
...
check('POLICY_INSTALLED',store.__academiaStaticWritePolicyVersion==='20260729.2');
check('POLICY_EVENT',events.includes('orbit:academia-static-write-policy'));

store.init({cursos:[{id:'cur_seed_static',titulo:'Curso seed',cat:'Producto',lecciones:[],progreso:0}],lecciones:[{id:'m1_seed_lesson',titulo:'Lección seed',secciones:[]}],evaluaciones:[{id:'eval_seed_static',titulo:'Evaluación seed',preguntas:[]}]});
check('SEED_STATIC_VISIBLE',store.get('cursos','cur_seed_static')?.titulo==='Curso seed'&&store.get('lecciones','m1_seed_lesson')&&store.get('evaluaciones','eval_seed_static'));
check('SEED_NO_DURABLE_WRITES',durableCalls.filter(call=>call.method!=='init').length===0,JSON.stringify(durableCalls));

store.insert('cursos',{id:'cur_static_test',titulo:'Curso estático',cat:'Producto',lecciones:[],_cv:999});
check('STATIC_COURSE_TRANSIENT',durableCalls.filter(call=>call.method!=='init').length===0,JSON.stringify(durableCalls));
...
check('STATIC_LESSON_EVALUATION_CONFIG_TRANSIENT',durableCalls.filter(call=>call.method!=='init').length===0,JSON.stringify(durableCalls));
...
check('STATUS_SANITIZED',status&&status.version==='20260729.2'&&Array.isArray(status.recent)&&status.recent.every(row=>!('payload' in row)&&!('data' in row)));
check('NO_SECRET_OR_PII_LITERALS',!/(password|contraseña|secret|token|@)/i.test(JSON.stringify(status)));

const failed=checks.filter(item=>!item.ok);
const out={schemaVersion:'orbit360-academia-static-content-write-policy-test-v2',ok:failed.length===0,status:failed.length?'FAIL':'PASS',passed:checks.length-failed.length,total:checks.length,failed:failed.length,failedCheckIds:failed.map(item=>item.id),checks,durableCalls,transientStatus:status,containsPII:false,containsSecrets:false};
