...
try {
  ...
  for (const route of routes) {
    ...
    try {
      await page.screenshot({ path: file, fullPage: false, animations: 'disabled', caret: 'hide', timeout: 20000 });
    } catch (error) {
      throw new Error(`PIPELINE_MECHANISM_FAILURE:VISUAL_SCREENSHOT_${route}:${safe(error)}`);
    }
  ...
  save({ schemaVersion: 'orbit360-block12-cumulative-visual-v2', status: ok ? 'CUMULATIVE_VISUAL_LAB_PASS' : 'CUMULATIVE_VISUAL_LAB_FAIL', classification: ok ? 'GO_LAB_CUMULATIVE_VISUAL_CANDIDATE' : 'FUNCTIONAL_DEFECT', routes: results, routeCount: results.length, pageErrors, screenshotMode: 'viewport-stable', animationsDisabled: true, customTokenEphemeral: true, authWrites: 0, firestoreWrites: 0, realTenantWrites: 0, hostingPreviewOnly: true, productionTouched: false, ok });
  if (!ok) process.exitCode = 42;
} catch (error) {
  const classification = ((safe(error).match(/(SECURITY_FAILURE|FUNCTIONAL_DEFECT|VALIDATOR_STALE|DATA_CONTRACT_FAILURE|ENVIRONMENT_FAILURE|PIPELINE_MECHANISM_FAILURE)/) || [])[1] || 'PIPELINE_MECHANISM_FAILURE');
  save({ schemaVersion: 'orbit360-block12-cumulative-visual-v2', status: 'CUMULATIVE_VISUAL_LAB_FAIL', classification, error: safe(error), currentRoute, routes: results, routeCount: results.length, pageErrors, screenshotMode: 'viewport-stable', animationsDisabled: true, authWrites: 0, firestoreWrites: 0, realTenantWrites: 0, productionTouched: false, ok: false });
  process.exitCode = 41;
} finally {
