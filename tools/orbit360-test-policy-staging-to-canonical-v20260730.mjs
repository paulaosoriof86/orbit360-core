...
const refs={
  clients:[
    {id:'cli_exist',nombre:'Cliente Existente',numeroDocumento:'123',asesorId:'adv_paula',pais:'GT',moneda:'GTQ'}
  ],
  insurers:[
    {id:'ins_uni',nombre:'Seguros Universales',aliases:['Universales'],pais:'GT'}
  ],
  advisors:[
    {id:'adv_paula',nombre:'Paula Osorio'}
  ],
  policies:[]
};
const candidate={
  ...
  clients:[
    {nombre:'Cliente Nuevo',documento:'',correo:'nuevo@example.test',telefono:'55550000',whatsapp:'55550000',pais:'GT',moneda:'GTQ',vendedor:'Paula Osorio',calidad_datos:'pendiente_completar',source_refs:['fixture#1']}
  ],
  policies:[
    ...
    {numero_poliza:'POL-003',cliente_match_fuente:'Cliente Nuevo',asegurado:'Cliente Nuevo',cliente_nuevo_candidato:true,aseguradora:'Universales',vigencia_inicio:'2026-03-01',vigencia_fin:'2027-03-01',estado_fuente:'Vigente',estado_propuesto:'Vigente',pais:'GT',moneda:'GTQ',prima_neta:3000,prima_total:3360,forma_pago:'Transferencia',frecuencia_pago:'Mensual',ramo:'SALUD',subramo:'Gastos Medicos',tipo_ramo:'Individual',vendedor:'Paula Osorio',source_version_key:'POL-003|UNIVERSALES|CLIENTE NUEVO|2026-03-01|2027-03-01',source_refs:['fixture#4']}
  ]
};
...
for(const op of plan.operations.filter(x=>x.collection==='polizas')){
  ...
  assert.equal(op.data.aseguradoraId,'ins_uni');
...
console.log(JSON.stringify({status:'PASS',stagingToCanonical:true,existingClientReused:true,newClientCreatedOnce:true,newClientPoliciesReuseId:true,insurerResolutionFailClosed:true,excludedPreserved:2,receiptsWritten:0,carteraWrites:0,cobroWrites:0,firestoreWrites:0,operationalWrites:0},null,2));
