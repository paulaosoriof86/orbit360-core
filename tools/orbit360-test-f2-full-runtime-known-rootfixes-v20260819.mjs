...
// Direct source audit: no runtime source patching, no historical core delegation.
import fs from 'node:fs';
import path from 'node:path';
import {execFileSync} from 'node:child_process';

const ROOT=process.env.ORBIT360_ROOT?path.resolve(process.env.ORBIT360_ROOT):process.cwd(),REQUEST_PATH=String(process.env.ORBIT360_REQUEST_FILE||'').trim();
