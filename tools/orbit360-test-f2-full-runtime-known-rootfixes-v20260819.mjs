...
const workflowText=()=>{const raw=String(process.env.ORBIT360_F2_WORKFLOW_SOURCE_FILE||'').trim(),p=raw?path.resolve(raw):path.join(ROOT,DEFAULT_WORKFLOW_PATH);need(fs.existsSync(p),'PIPELINE_MECHANISM_FAILURE:F2_EXECUTING_WORKFLOW_SOURCE_MISSING');return fs.readFileSync(p,'utf8');};
