...
try{
  ...
  need(engine.includes('request.requestOrdinal')&&engine.includes('AUTH_RECORD')&&engine.includes('request.authorizationRecordSha256')&&engine.includes('F2_PERSISTED_AUTHORIZATION'),'VALIDATOR_STALE:F2_RUNTIME_ENGINE_PERSISTED_AUTHORIZATION_BINDING_MISSING');
  need(Number(REQUEST.requestOrdinal)===11&&REQUEST.authorizationRecordPath==='.github/orbit360-authorizations/f2-productive-acceptance-runtime-browser-readonly-request11-v20260820.json'&&/^[0-9a-f]{64}$/.test(String(REQUEST.authorizationRecordSha256||'')),'VALIDATOR_STALE:F2_RUNTIME_REQUEST11_AUTHORIZATION_BINDING_INVALID');
