...
try{
  ...
  need(workflow.includes('f2-productive-acceptance-runtime-browser-readonly-successor-*.json')&&workflow.includes('F2_PRODUCTIVE_ACCEPTANCE_RUNTIME_BROWSER_READONLY_V2'),'PIPELINE_MECHANISM_FAILURE:F2_WORKFLOW_NOT_V2_REACHABLE');
  ...
  need(register.includes('F2_RUNTIME_LIFECYCLE_REGISTERED_TRANSIENT_V2')&&register.includes('authorizationIdentityDigest'),'VALIDATOR_STALE:F2_REGISTER_NOT_V2');
  ...
  write({schemaVersion:'orbit360-f2-full-runtime-known-rootfixes-selftest-v6',ok:true,status:'F2_FULL_RUNTIME_KNOWN_ROOTFIXES_SOURCE_AUDIT_PASS',classification:'PASS',validatorRevision:'F2_AUTH_IDENTITY_V2_ORDINAL_FREE_V6',candidateArtifactId:Number(r.candidateArtifactId),candidateSourceHead:source,authorizationIdentityDigest:r.authorizationIdentityDigest,dynamicCandidateBinding:true,persistedAuthorizationBinding:true,ordinalOperationalSemantics:false,routerReadinessRootfix:true,storeGetRootfix:true,routeObservability:true,crossTenantGuard:true,deployCommandPresent:false,browserExecuted:false,runtimeExecuted:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,authWrites:0,operationalWrites:0,containsPII:false,containsSecrets:false});
}catch(error){write({schemaVersion:'orbit360-f2-full-runtime-known-rootfixes-selftest-v6',ok:false,status:'F2_FULL_RUNTIME_KNOWN_ROOTFIXES_SOURCE_AUDIT_FAIL',classification:String(error?.message||error).split(':')[0]||'VALIDATOR_STALE',error:String(error?.message||error).slice(0,900),browserExecuted:false,runtimeExecuted:false,secretAccess:false,firestoreRead:false,writes:0,containsPII:false,containsSecrets:false});process.exitCode=41;}
