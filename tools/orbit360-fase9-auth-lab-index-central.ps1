...
$Report = Join-Path $Reports "FASE9-AUTH-FIREBASE-LAB-INDEX-CENTRAL-$Stamp.txt"
$ServerJs = Join-Path $Tmp "fase9-auth-lab-server-$Stamp.js"
$ServerOut = Join-Path $Tmp "fase9-auth-lab-server-out-$Stamp.log"
$ServerErr = Join-Path $Tmp "fase9-auth-lab-server-err-$Stamp.log"
$ResultJson = Join-Path $Tmp "fase9-auth-lab-result-$Stamp.json"
$ChromeProfile = Join-Path $Tmp "chrome-fase9-auth-lab-$Stamp"
...
Add-Report "ORBIT 360 - FASE 9 - AUTH FIREBASE LAB SOBRE INDEX CENTRAL"
...
try {
  ...
  Add-Report "== 2. Verificar archivos Fase 9 =="
  ...
  foreach ($Path in @($Index, $Loader, $Auth)) {
  ...
  if (Test-Path $ConfigLocal) {
    ...
    Add-Report "BLOQUEO: no existe core/auth-firebase.config.local.js local. No se puede validar Auth/Firebase LAB real sin config local."
    Add-Report "Resultado Fase 9: BLOQUEADO_CONFIG_LOCAL"
    throw "Config local Firebase no encontrada"
  }
  Add-Report ""

  Add-Report "== 3. Enlazar loader LAB en index.html si falta =="
  ...
  if ($IndexText -notmatch "core/backend-lab-loader\.js") {
    ...
    $Insert = $Needle + "`r`n  <script src=`"core/backend-lab-loader.js?v1268`"></script>"
    if ($IndexText.Contains($Needle)) {
      $IndexText = $IndexText.Replace($Needle, $Insert)
      Set-Content -Path $Index -Value $IndexText -Encoding UTF8
      Add-Report "OK: loader LAB insertado antes de data/store.js."
    } else {
      throw "No se encontró punto de inserción para backend-lab-loader.js"
    }
  } else {
    Add-Report "OK: index.html ya tiene backend-lab-loader.js"
  }
  ...
  Add-Report "Orden: loader=$LoaderPos store=$StorePos lab=$LabPos seed=$SeedPos"
  if (-not ($LoaderPos -ge 0 -and $StorePos -gt $LoaderPos -and $LabPos -gt $StorePos -and $SeedPos -gt $LabPos)) {
    throw "Orden loader/store/lab/seed incorrecto"
  }
  Add-Report "OK: orden loader -> store -> lab -> seed correcto."
  Add-Report ""

  Add-Report "== 4. Validar sintaxis JS =="
  foreach ($Js in @($Loader, $Auth, (Join-Path $App "data\store-firestore-lab.local.js"))) {
...
const smokeHtml = `<!doctype html><html><head><meta charset="utf-8"><title>Orbit Fase 9 Auth LAB</title><style>body{font-family:Arial;margin:24px;background:#f6f7f8}.card{background:#fff;border:1px solid #ddd;border-radius:12px;padding:18px}pre{background:#111;color:#0f0;padding:12px;border-radius:8px;white-space:pre-wrap}</style></head><body><div class="card"><h1>Orbit 360 - Fase 9 Auth LAB</h1><p>Si aparece login dentro de la ventana, ingresa con el usuario LAB. El smoke esperará hasta 90 segundos.</p><pre id="out">Esperando...</pre><iframe id="app" src="/index.html?orbitBackend=firestore-lab&tenant=alianzas-soluciones&smoke=fase9" style="width:100%;height:720px;border:1px solid #ccc;border-radius:10px"></iframe></div><script>
...
async function post(obj){out.textContent=JSON.stringify(obj,null,2); try{await fetch('/__fase9_result',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(obj)}); out.textContent+='\\n\\nResultado enviado. Puedes cerrar esta ventana.';}catch(e){out.textContent+='\\nERROR POST: '+(e.message||String(e));}}
function inspect(){try{const w=document.getElementById('app').contentWindow; const store=w.Orbit&&w.Orbit.store; const backend=w.OrbitBackend||w.ORBIT_BACKEND||{}; const status=backend.status?backend.status():(store&&store._labStatus?store._labStatus():{}); const api=['all','get','where','find','insert','update','remove','on','_emit','pref','setPref','init','reseed','raw']; const apiStatus={}; api.forEach(k=>apiStatus[k]=!!(store&&typeof store[k]==='function')); let fb=false, apps=0, authUser=null; try{fb=!!w.firebase; apps=w.firebase&&w.firebase.apps?w.firebase.apps.length:0; const u=w.firebase&&w.firebase.auth?w.firebase.auth().currentUser:null; authUser=u?{uid:u.uid,email:u.email}:null;}catch(e){} const result={href:w.location.href,title:w.document.title,readyState:w.document.readyState,orbitExists:!!w.Orbit,storeExists:!!store,apiStatus,apiComplete:api.every(k=>apiStatus[k]),backendMode:backend.mode||null,backendTenant:backend.tenantId||backend.tenant||null,backendApiVersion:backend.apiVersion||null,collectionsLength:backend.collections?backend.collections.length:null,firebaseDetected:fb,firebaseApps:apps,authUser,status,errors,elapsedMs:Date.now()-start}; const ok=result.apiComplete&&result.backendMode==='firestore-lab'&&result.backendTenant==='alianzas-soluciones'&&result.firebaseDetected&&result.firebaseApps>0&&result.authUser&&result.status&&result.status.snapshotAttachedCount>0; if(ok||Date.now()-start>90000){result.contractOk=!!ok; post(result);} else {out.textContent='Esperando Auth/Firebase LAB... '+Math.round((Date.now()-start)/1000)+'s\\n'+JSON.stringify(result,null,2); setTimeout(inspect,1200);}}catch(e){if(Date.now()-start>90000)post({contractOk:false,error:String(e&&e.message||e),errors,elapsedMs:Date.now()-start}); else setTimeout(inspect,1200);}}
setTimeout(inspect,1500);
</script></body></html>`;
const server=http.createServer((req,res)=>{const url=new URL(req.url,'http://127.0.0.1'); if(req.method==='GET'&&url.pathname==='/__fase9_smoke.html')return send(res,200,'text/html; charset=utf-8',smokeHtml); if(req.method==='POST'&&url.pathname==='/__fase9_result'){let body=''; req.on('data',c=>body+=c); req.on('end',()=>{fs.writeFileSync(resultPath,body||'{}','utf8'); send(res,200,'application/json; charset=utf-8','{"ok":true}');}); return;} let filePath=decodeURIComponent(url.pathname); if(filePath==='/')filePath='/index.html'; const full=path.normalize(path.join(root,filePath)); if(!full.startsWith(path.normalize(root)))return send(res,403,'text/plain','Forbidden'); fs.readFile(full,(err,data)=>{if(err)return send(res,404,'text/plain; charset=utf-8','Not found: '+filePath); send(res,200,types[path.extname(full).toLowerCase()]||'application/octet-stream',data);});});
server.listen(port,'127.0.0.1',()=>console.log('FASE9_SERVER_READY http://127.0.0.1:'+port));
'@
  ...
  $SmokeUrl = "http://127.0.0.1:$Port/__fase9_smoke.html"
  ...
  if(-not $Got){ throw "No llegó resultado POST Fase 9" }
  ...
  if($Result.contractOk){
    Add-Report "RESULTADO FASE 9: COMPLETADO"
    git add orbit360-platform/index.html orbit360-platform/core/auth.js orbit360-platform/core/backend-lab-loader.js tools/orbit360-fase9-auth-lab-index-central.ps1
    git commit -m "feat: conectar Auth Firebase LAB en index central"
    git push -u origin $ExpectedBranch
    Add-Report "OK: commit/push Fase 9 completado."
  } else {
    Add-Report "RESULTADO FASE 9: FALLIDO_O_BLOQUEADO"
  ...
  Add-Report ""
  Add-Report "FIN FASE 9. Reporte: $Report"
