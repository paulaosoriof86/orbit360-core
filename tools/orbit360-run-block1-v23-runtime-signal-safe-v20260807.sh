...
jq -e '.status=="GO_GATE_CONTRACT" and .contractVersion=="1.0.41" and .ok==true and .universeAdjudicationRequiredBeforeHosting==true and .hostingDeploysMaximum==1 and .writeAuthorized==false' "$PREFLIGHT" >/dev/null || { echo 'STOP_V23_GO_GATE_CONTRACT_INVALID'; exit 41; }
...
if node "$TIMEOUT_RUNNER" --timeout-ms 1320000 --grace-ms 20000 -- node "$MATRIX_RUNNER" && jq -e '.contractVersion=="1.0.41" and .canonicalRuntimeArtifact=="tools/orbit360-block1-native-matrix-v23-canonical-v20260807.mjs" and .stage=="PASS_BLOCK1_NATIVE_VISUAL_MATRIX" and .classification=="PASS_VISUAL_POST_AUTH" and .ok==true and .totalRoleFailures==0 and .snapshotIntegrity=="VERIFIED_UNCHANGED" and (.roles|length)==3 and .firestoreWrites==0 and .authWrites==0 and .operationalWrites==0 and .productionTouched==false' "$MATRIX" >/dev/null; then MATRIX_OUTCOME='success'; else MATRIX_OUTCOME='failure'; stop; fi
