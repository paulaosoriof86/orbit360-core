...
const forbidden=[/backend-lab-/i,/store-firestore-lab/i,/client360-lab-/i,/__ORBIT_LAB_SAFE_MODE__/i,/orbitBackend=firestore-lab/i,/data\/store\.js(?:\?|["'])/i,/data\/seed\.js/i,/core\/auth\.js(?:\?|["'])/i,/blocked-until-authorized-runtime/i,/Acceso pendiente de activaci[oó]n/i,/Orbit\.store\.init\(Orbit\.SEED\)/,/localStorage\.(?:getItem|setItem)\(/i];
...
try{
 ...
 report.checks={functionalDom:report.missingIds.length===0,requiredRuntimeRefs:report.missingRefs.length===0,noForbiddenRuntime:report.forbiddenMatches.length===0,allEntrypointAssetsResolve:report.missingAssets.length===0,productMode:/data-orbit-product-mode=["']product-readonly["']/i.test(html),productInit:/Orbit\.productAppP0\.init\(\)/.test(html),productAppStartContract:/core\/product-app-p0\.js/.test(html),preAuthStoreFailClosed:/WRITE_BLOCKED_PRODUCT_PREAUTH_P0/.test(preauth)&&/all:function\(\)\{return\[\];\}/.test(preauth)&&/noFallback:true/.test(preauth)};
