...
if(!failures.length){
  ...
  need(wf.includes('CONTROL_PLANE_IDEMPOTENT_REPROJECTION_V1'),'IDEMPOTENT_REPROJECTION_GUARD_MISSING');
  ...
  need(wf.includes('FINAL_HEAD=\'${{ steps.intent.outputs.live_base }}\'')&&wf.includes('if [ "${#CHANGED[@]}" -gt 0 ]; then'),'IDEMPOTENT_ZERO_DIFF_REPROJECTION_CONTRACT_MISSING');
  ...
  need(conv.includes("TRANSIENT_PREFLIGHT='orbit360-platform/runtime-gate-crm-v20260716/macro3-mechanism-preflight-sanitized-v20260823.json'")&&conv.includes('TRANSIENT_PREFLIGHT_CLEANUP_FAIL'),'CONVERGENCE_TRANSIENT_PREFLIGHT_CLEANUP_MISSING');
  ...
  need(registry.policies?.transientPreflightEvidenceMustBeCleanedBeforePublication===true&&registry.policies?.terminalPublicationRequiresAcceptedStatePublication===true&&registry.policies?.prBodyPublicationRequiresTerminalPublication===true,'REGISTRY_PUBLICATION_CHAIN_POLICY_MISSING');
  need(registry.policies?.idempotentZeroDiffReprojectionAllowed===true&&registry.policies?.idempotentZeroDiffReprojectionRequiresLiveHeadCAS===true&&registry.policies?.externalProjectionMayAdvanceOnlyFromPublishedCanonicalState===true,'REGISTRY_IDEMPOTENT_PROJECTION_POLICY_MISSING');
  ...
  need(registry.publicationGuard?.transientPreflightEvidencePublicationAllowed===false&&registry.publicationGuard?.terminalPublicationRequiresAuthPublication===true&&registry.publicationGuard?.prBodyPublicationRequiresTerminalPublication===true&&registry.publicationGuard?.idempotentZeroDiffReprojectionUsesLiveHeadCAS===true,'REGISTRY_PUBLICATION_GUARD_DRIFT');
...
const out={schemaVersion:'orbit360-macro3-mechanism-preflight-v7-publication-chain-idempotency-aware',ok:failures.length===0,status:failures.length?'MACRO3_MECHANISM_PREFLIGHT_FAIL':'MACRO3_MECHANISM_PREFLIGHT_PASS',classification:failures.length?'PIPELINE_MECHANISM_FAILURE':'PASS',failures:[...new Set(failures)],singleWorkflowRequired:true,genericIntentRouterRequired:true,hardeningCloseModeRequired:true,canonicalOwnerV24Required:true,executionPrMayModifyWorkflow:false,workflowDispatchForbidden:true,workflowRunChainingForbidden:true,actionsWriteForbidden:true,currentRunEvidenceRequired:true,terminalTruthRequired:true,gateOrderByStepIds:true,noSourceRewriteRequired:true,oneShotAcceptedBeforeRuntime:true,allowedExecutionsAfterAccept:0,terminalReducerRequired:true,transientPreflightCleanupRequired:true,publicationChainGuardRequired:true,idempotentZeroDiffReprojectionRequired:true,replayAllowed:false,runtimeExecuted:false,browserExecuted:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,authWrites:0,operationalWrites:0,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};
