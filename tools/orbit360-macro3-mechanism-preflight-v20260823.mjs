...
const P={
  ...
  lifecycle:'tools/orbit360-control-plane-evidence-lifecycle-v20260824.mjs'
};
const A=p=>path.join(ROOT,p);
const J=p=>JSON.parse(fs.readFileSync(A(p),'utf8').replace(/^\uFEFF/,''));
const failures=[],need=(v,c)=>{if(!v)failures.push(c);};
const runJson=(rel,args=[])=>{
  try{return JSON.parse(execFileSync(process.execPath,[A(rel),...args],{cwd:ROOT,encoding:'utf8',env:process.env}));}
...
if(!failures.length){
  try{contract=J(P.contract);registry=J(P.registry);authority=J(P.authority);ledger=J(P.ledger);}catch{need(false,'CONTROL_PLANE_MACHINE_CONTRACT_JSON_INVALID');}
}
if(!failures.length){
  ...
  for(const rel of components){
    need(typeof rel==='string'&&fs.existsSync(A(rel)),`ACTIVE_COMPONENT_MISSING:${String(rel)}`);
    if(typeof rel==='string'&&fs.existsSync(A(rel))&&rel.endsWith('.mjs')){
      try{execFileSync(process.execPath,['--check',A(rel)],{cwd:ROOT,stdio:'ignore'});}catch{need(false,`NODE_CHECK_FAIL:${rel}`);}
    }
  }
  ...
  noRewrite=runJson(P.noRewrite);
  need(noRewrite.ok===true&&noRewrite.scopeMode==='MACHINE_READABLE_CONTRACT_DERIVED','SOURCE_REWRITE_GUARD_FAIL');
  need(Number(noRewrite.activePathCount)===(contract.sourceRewriteGuardScope||[]).length,'SOURCE_REWRITE_SCOPE_DRIFT');

  lifecycle=runJson(P.lifecycle,['--phase','pre-auth','--assert-only']);
  need(lifecycle.ok===true&&lifecycle.strategy==='GIT_CHANGED_SURFACE_CLASS_WIDE_NOT_FILENAME_LIST','EVIDENCE_LIFECYCLE_CONTRACT_FAIL');
  ...
  need(Boolean(certPath)&&fs.existsSync(A(certPath)),'DURABLE_CERTIFICATION_POINTER_MISSING');
  if(certPath&&fs.existsSync(A(certPath))){
    const cert=J(certPath),fc=Number(cert.fileCount),dc=Number(cert.deltaCount),uc=Number(cert.unchangedFileCount),cc=Number(cert.checksPassed);
    ...
    need(Number(cert.artifactId)===Number(ledger.successorCandidate?.artifactId)&&cert.sourceHead===ledger.successorCandidate?.sourceHead&&cert.artifactDigest===ledger.successorCandidate?.artifactDigest,'DURABLE_CERTIFICATION_CANDIDATE_DRIFT');
  ...
  need(ledger.activeState?.runtimeAuthorized===false&&ledger.activeState?.runtimeReplayAllowed===false,'LEDGER_RUNTIME_BOUNDARY_OPEN');
  need(ledger.authorizationBoundary?.activeRuntimeAuthorization===false&&ledger.authorizationBoundary?.activeRequestPath==null&&(ledger.authorizationBoundary?.runtimeAttemptAccepted??false)===false,'LEDGER_AUTH_REQUEST_NOT_INERT');
  need(Number(ledger.progress?.productionRouteProgressPct)===75&&ledger.progress?.f2TerminalPass===false,'LEDGER_PROGRESS_NOT_FAIL_CLOSED');
}
const out={
  schemaVersion:'orbit360-macro3-mechanism-preflight-v9-semantic-contract-behavioral',
  ok:failures.length===0,
  status:failures.length?'MACRO3_MECHANISM_PREFLIGHT_FAIL':'MACRO3_MECHANISM_PREFLIGHT_PASS',
  classification:failures.length?'PIPELINE_MECHANISM_FAILURE':'PASS',
  failures:[...new Set(failures)],
  semanticContract:P.contract,
  validatorMode:'MACHINE_READABLE_CONTRACT_PLUS_BEHAVIORAL_EXECUTION',
  sourceShapeValidationUsed:false,
  candidateBinding:'DYNAMIC_FROM_CANONICAL_LEDGER',
  activeComponentCount:Array.isArray(contract.activeControlPlaneComponents)?contract.activeControlPlaneComponents.length:0,
  workflowSurfaceAuditPass:workflowAudit.ok===true,
  sourceRewriteGuardPass:noRewrite.ok===true,
  evidenceLifecycleContractPass:lifecycle.ok===true,
  singleWorkflowRequired:true,
  gateOrderByTechnicalStepIds:true,
  sourceTextMayNotProveBehavior:true,
  candidateHardcodingForbidden:true,
  operationalRevisionHardcodingForbidden:true,
  replayAllowed:false,
