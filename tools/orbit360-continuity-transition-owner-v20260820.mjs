...
if (transitionId === 'POST_CLOSE_RECONCILE_CONTROL_PLANE_METADATA') {
  ...
  if (pkg.status !== 'CLOSED_PASS') throw new Error('CP11_REQUIRES_CLOSED_PASS');
  if (pkg.controlPlaneClosure?.status !== 'CLOSED_PASS_RECONCILED') throw new Error('CP11_REQUIRES_RECONCILED_CONTROL_PLANE');
  if (pkg.resumeProtocol?.firstIncompleteStep !== 'CP-11') throw new Error('CP11_NOT_ACTIVE');
  const cp11=(pkg.steps||[]).find(s=>s.id==='CP-11');
  if (!cp11 || cp11.status!=='PENDING') throw new Error('CP11_NOT_PENDING');
  cp11.status='PASS';
  cp11.completedBy='tools/orbit360-continuity-transition-owner-v20260820.mjs';
  cp11.evidence=[BOUNDARY_REL,'tools/orbit360-cp11-authorization-boundary-audit-v20260820.mjs'];
  bumpCommon();
  pkg.phase='F2_RUNTIME_AUTHORIZATION_BOUNDARY_PREPARED_AWAITING_EXPLICIT_AUTHORIZATION';
  pkg.resumeProtocol.firstIncompleteStep='F2-RUNTIME-AUTHORIZATION';
  pkg.resumeProtocol.nextActionExact='AWAIT_EXPLICIT_F2_RUNTIME_AUTHORIZATION_ONE_SHOT';
  pkg.authorizationBoundary={path:BOUNDARY_REL,status:'PREPARED_SOURCE_ONLY_AWAITING_EXPLICIT_USER_AUTHORIZATION',ledgerRevision:ledger.revision,packageRevision:pkg.revision,authorized:false,requestMaterialized:false,runtimeAllowed:false};
  pkg.lock.authorizationAllowed=true;
  pkg.lock.runtimeAllowed=false;
  pkg.lock.requestMaterializationAllowed=false;
  pkg.lock.newRuntimeOrdinalAllowed=false;
  ledger.activeState.phase='F2_RUNTIME_AUTHORIZATION_BOUNDARY_PREPARED_AWAITING_EXPLICIT_AUTHORIZATION';
  ledger.activeState.status='F2_RUNTIME_AUTHORIZATION_NOT_YET_GRANTED';
  ledger.activeState.rootCauseStatus='CONTROL_PLANE_HARDENING_CLOSED_PASS_CP11_BOUNDARY_READY';
  ledger.authorizationBoundary.activeRuntimeAuthorization=false;
  ledger.authorizationBoundary.freshAuthorizationRequired=true;
  ledger.authorizationBoundary.authorizationBlockedByHardeningPackage=false;
  ledger.authorizationBoundary.nextRuntimeMaterializationAllowed=false;
  ledger.authorizationBoundary.newRuntimeRequestAllowed=false;
  ledger.authorizationBoundary.boundaryPath=BOUNDARY_REL;
  ledger.authorizationBoundary.boundaryPrepared=true;
  ledger.authorizationBoundary.authorizationIdentityRequired=true;
  ledger.productionReopeningPackage.status='CLOSED_PASS';
  ledger.productionReopeningPackage.authorizationAllowed=true;
  ledger.productionReopeningPackage.runtimeAllowed=false;
  ledger.productionReopeningPackage.requestMaterializationAllowed=false;
  ledger.productionReopeningPackage.firstIncompleteStep='F2-RUNTIME-AUTHORIZATION';
  ledger.productionReopeningPackage.nextActionExact='AWAIT_EXPLICIT_F2_RUNTIME_AUTHORIZATION_ONE_SHOT';
  ledger.nextAction={id:'AWAIT_EXPLICIT_F2_RUNTIME_AUTHORIZATION_ONE_SHOT',description:'Await one fresh explicit user authorization bound to the prepared CP11 authorization identity. Do not materialize a runtime request before that authorization.',runtimeAllowed:false};
  ledger.lanes.B_backend_security_gates='F2_RUNTIME_AUTHORIZATION_BOUNDARY_PREPARED_AWAITING_USER';
  result={ok:true,status:'ORBIT360_CP11_AUTHORIZATION_BOUNDARY_STATE_PREPARED',transitionId,fromLedgerRevision:expectedRevision,toLedgerRevision:ledger.revision,fromPackageRevision:expectedPackageRevision,toPackageRevision:pkg.revision,firstIncompleteStep:'F2-RUNTIME-AUTHORIZATION',authorizationAllowed:true,runtimeAllowed:false,requestMaterialized:false};
