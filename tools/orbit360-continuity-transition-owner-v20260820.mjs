...
if(transition==='CONTROL_PLANE_EVIDENCE_RECONCILE'){
  ...
  L.productionReopeningPackage.authorizationAllowed=false;L.productionReopeningPackage.requestMaterializationAllowed=true;L.productionReopeningPackage.firstIncompleteStep='F2-RUNTIME-REQUEST-MATERIALIZATION';L.productionReopeningPackage.nextActionExact='MATERIALIZE_SINGLE_F2_RUNTIME_REQUEST_SOURCE_ONLY';
  ...
  W(P.ledger,L);project();out={ok:true,status:'ORBIT360_F2_RUNTIME_AUTHORIZATION_PERSISTED_SOURCE_ONLY',authorizationRecordPath:authPath,ledgerRevision:L.revision,packageRevision:L.productionReopeningPackage.revision};
  ...
  L.productionReopeningPackage.requestMaterializationAllowed=false;L.productionReopeningPackage.firstIncompleteStep='F2-RUNTIME-ATTEMPT-ACCEPT';L.productionReopeningPackage.nextActionExact='ACCEPT_SINGLE_F2_RUNTIME_ATTEMPT';
  L.nextAction={id:L.productionReopeningPackage.nextActionExact,description:'Atomically accept the single F2 runtime attempt before preflight. Replay remains forbidden.',runtimeAllowed:false};L.activeState.phase='F2_RUNTIME_REQUEST_MATERIALIZED_AWAITING_ATTEMPT_ACCEPT';L.activeState.status='F2_RUNTIME_REQUEST_MATERIALIZED_SOURCE_ONLY';
  W(P.ledger,L);project();out={ok:true,status:'ORBIT360_F2_RUNTIME_REQUEST_MATERIALIZED_SOURCE_ONLY',requestPath,ledgerRevision:L.revision,packageRevision:L.productionReopeningPackage.revision};
  ...
  const auth=J(authPath),req=J(requestPath);if(auth.consumed||req.consumed||auth.historical||req.historical||auth.allowedExecutions!==1||req.allowedExecutions!==1||auth.runtimeAttemptAccepted!==false||req.runtimeAttemptAccepted!==false)throw new Error('AUTH_REQUEST_NOT_AVAILABLE_FOR_ONE_SHOT_ACCEPT');
  const ts=bump(),run=Number(runtimeRunId);
  for(const [p,x,isReq] of [[authPath,auth,false],[requestPath,req,true]]){x.allowedExecutions=0;x.runtimeAttemptAccepted=true;x.runtimeAttemptCount=1;x.runtimeRunId=run;x.runtimeAttemptAcceptedAt=ts;x.status=isReq?'RUNTIME_ATTEMPT_ACCEPTED_PREFLIGHT_PENDING':'RUNTIME_ATTEMPT_ACCEPTED_ONE_SHOT';W(p,x);}
  L.authorizationBoundary.runtimeAttemptAccepted=true;L.authorizationBoundary.runtimeRunId=run;L.authorizationBoundary.currentBoundaryStatus='RUNTIME_ATTEMPT_ACCEPTED_ONE_SHOT_PREFLIGHT_PENDING';
  L.activeState.phase='F2_RUNTIME_ATTEMPT_ACCEPTED_PREFLIGHT_PENDING';L.activeState.status='F2_RUNTIME_ATTEMPT_ACCEPTED_ONE_SHOT';L.activeState.runtimeAuthorized=false;L.activeState.runtimeReplayAllowed=false;
  ...
  W(P.ledger,L);project();out={ok:true,status:'ORBIT360_F2_RUNTIME_ATTEMPT_ACCEPTED_ONE_SHOT',runtimeRunId:run,requestPath,ledgerRevision:L.revision,packageRevision:L.productionReopeningPackage.revision,replayAllowed:false};
  ...
  L.authorizationBoundary={...L.authorizationBoundary,activeRuntimeAuthorization:false,freshAuthorizationRequired:classification!=='PASS',authorizationCarryForwardForbidden:true,nextRuntimeMaterializationAllowed:false,newRuntimeRequestAllowed:false,activeRequestPath:null,authorizationRecordPath:null,runtimeAttemptAccepted:false,runtimeRunId:null,currentBoundaryStatus:classification==='PASS'?'CONSUMED_PASS_AWAITING_GO_LIVE':'CONSUMED_FAIL_AWAITING_SOURCE_ONLY_ROOT_CAUSE'};
  L.history={...(L.history||{}),latestSealedConsumedRuntime:{requestPath,authorizationPath:authPath,runId:run,conclusion:classification==='PASS'?'success':'failure',observedClassification:classification,observedFailureCode:String(Tm.failureCode||Tm.error||classification),terminalEvidenceOk:Tm.ok===true,terminalEvidenceClassificationRaw:String(Tm.classification||''),browserMatrixPass:Tm.browserMatrixPass===true,integrityBeforeAfterPass:Tm.integrityBeforeAfterPass===true,browserRunId:Number(Tm.browserRunId||0),integrityRunId:Number(Tm.integrityRunId||0),allowedExecutions:0,consumed:true,replayAllowed:false,firestoreWrites:0,authWrites:0,operationalWrites:0,terminalEvidencePath:terminalEvidence}};
  if(classification==='PASS'){
  ...
  W(P.ledger,L);project();out={ok:true,status:'F2_RUNTIME_TERMINAL_RECONCILED_GENERIC',classification,ledgerRevision:L.revision,packageRevision:L.productionReopeningPackage.revision,replayAllowed:false};
