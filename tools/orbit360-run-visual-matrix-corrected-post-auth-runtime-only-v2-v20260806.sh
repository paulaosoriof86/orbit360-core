...
[[ "${ORBIT360_RUNTIME_EXECUTION_AUTHORIZED:-false}" == 'true' ]] || {
  echo 'SOURCE_ONLY_NOT_AUTHORIZED'
  exit 64
}
...
const value = {
  schemaVersion: 'orbit360-visual-matrix-runtime-state-v2',
...
persist() {
  ...
  timeout --signal=TERM --kill-after=10s 120s node "$SEALER" || true
  ...
  if ! git diff --cached --quiet; then
    ...
    timeout --signal=TERM --kill-after=10s 120s git push origin "HEAD:$BRANCH" || true
  fi
...
rollback_if_needed() {
  if [[ "$DEPLOY_ATTEMPTED" == '1' && "$ROLLBACK_OUTCOME" != 'success' ]]; then
    if [[ -n "$BACKUP_CHANNEL" && "$BACKUP_OUTCOME" == 'success' ]]; then
      if timeout --signal=TERM --kill-after=20s 480s npx firebase hosting:clone "$PROJECT:$BACKUP_CHANNEL" "$PROJECT:live" --project "$PROJECT" --non-interactive; then
...
if timeout --signal=TERM --kill-after=20s 480s npx firebase hosting:clone "$PROJECT:live" "$PROJECT:$BACKUP_CHANNEL" --project "$PROJECT" --non-interactive; then
...
DEPLOY_ATTEMPTED=1
write_runtime_state
if timeout --signal=TERM --kill-after=20s 480s npx firebase deploy --project "$PROJECT" --only hosting --non-interactive; then
...
if timeout --signal=TERM --kill-after=15s 420s node tools/orbit360-visual-runtime-rootfix-browser-precheck-v20260805.mjs \
  && jq -e '
...
if timeout --signal=TERM --kill-after=20s 1320s node "$SUPERVISOR_RUNNER" \
