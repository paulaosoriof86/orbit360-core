...
need(pkg.canonicalPlan===P.plan,'PACKAGE_CANONICAL_PLAN_DRIFT');
const resumeRule=String(pkg.resumeProtocol?.rule||'');
need(resumeRule.includes(P.plan),'PACKAGE_RESUME_PLAN_DRIFT');
...
const out={schemaVersion:'orbit360-control-plane-independent-readback-v6-canonical-resume-plan',ok:failures.length===0,status:failures.length?'CONTROL_PLANE_INDEPENDENT_READBACK_FAIL':'CONTROL_PLANE_INDEPENDENT_READBACK_PASS',classification:failures.length?'DATA_CONTRACT_FAILURE':'PASS',failures:[...new Set(failures)],stateFingerprint:fingerprint,ledgerRevision:L.revision||null,packageRevision:pkg.revision||null,candidateArtifactId:L.successorCandidate?.artifactId||null,productionRouteProgressPct:L.progress?.productionRouteProgressPct||null,canonicalPlan:P.plan,packageResumePlanValidated:resumeRule.includes(P.plan),terminalEvidencePath:terminalPath,latestRunId:run,terminalOk:E.ok===true,terminalClassification:E.classification||null,historicalFalsePass,falsePassInvalidationMatched,currentRunEvidenceBound:currentRunBound,terminalPassEvidence:passEvidence,prBodyValidated:Boolean(process.env.ORBIT360_PR_BODY_FILE)&&failures.length===0,runtimeExecuted:Boolean(E.runtimeExecuted),browserExecuted:Boolean(E.browserExecuted||E.browserMatrixPass),firestoreWrites:Number(E.firestoreWrites||0),authWrites:Number(E.authWrites||0),operationalWrites:Number(E.operationalWrites||0),deployExecuted:Boolean(E.deployExecuted),productionTouched:Boolean(E.productionHostingTouched),containsPII:false,containsSecrets:false};
