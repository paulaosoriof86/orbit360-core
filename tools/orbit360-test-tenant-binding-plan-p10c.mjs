...
function norm(v){return String(v||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'_')}
const Orbit={};
...
Orbit.knowledgeBindingGateP08={buildBindings:({rules,profiles})=>({
 bindings:profiles.map((p,i)=>{
   const match=rules.find(r=>norm(r.dimensiones&&r.dimensiones.tipoVehiculo)===norm(p.dimensiones&&p.dimensiones.tipoVehiculo));
   return {id:`b${i+1}`,dimensiones:p.dimensiones,status:match?'complete_requires_gate':'presentation_only',ruleIds:match?[match.id]:[]};
 }),
 summary:{total:profiles.length,complete:profiles.filter(p=>rules.some(r=>norm(r.dimensiones&&r.dimensiones.tipoVehiculo)===norm(p.dimensiones&&p.dimensiones.tipoVehiculo))).length,conflicts:0}
})};
