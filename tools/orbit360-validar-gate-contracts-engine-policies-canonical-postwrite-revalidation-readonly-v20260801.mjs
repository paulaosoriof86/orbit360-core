...
const VERSION='7.6.0';
const LIFECYCLE='tools/orbit360-validator-lifecycle-contract-policies-canonical-postwrite-revalidation-readonly-v20260801.json';
const REQUEST='.github/orbit360-requests/policies-canonical-postwrite-revalidation-readonly-v20260801.json';
const PREVIOUS='tools/orbit360-validator-lifecycle-contract-policies-canonical-controlled-write-lab-v20260801.json';
const EXECUTOR='tools/orbit360-revalidar-policies-canonical-postwrite-readonly-v20260801.mjs';
const WORKFLOW='.github/workflows/orbit360-policies-canonical-postwrite-revalidation-readonly-v20260801.yml';
...
try{
  ...
  const required=[LIFECYCLE,REQUEST,PREVIOUS,EXECUTOR,WORKFLOW,CUMULATIVE,'tools/orbit360-validar-gate-contracts-v20260717.mjs','tools/orbit360-policies-dual-path-provenance-lib-v20260801.mjs','tools/orbit360-policies-authority-canonical-dryrun-lib-v20260801.mjs'];
  ...
  add('LIFECYCLE',lifecycle.gateId===GATE&&lifecycle.gateContractVersion===VERSION&&lifecycle.status==='POLICIES_CANONICAL_POSTWRITE_REVALIDATION_READONLY_AUTHORIZED');
  ...
  add('AUTHORIZATION',lifecycle.authorization?.explicit===true&&lifecycle.authorization?.allowedExecutions===1&&lifecycle.authorization?.consumed===false&&lifecycle.authorization?.authorizationRef==='user_authorized_canonical_postwrite_revalidation_readonly_20260801');
  add('REQUEST',request.schemaVersion==='orbit360-policies-canonical-postwrite-revalidation-request-v1'&&request.gateId===GATE&&request.contractVersion===VERSION&&request.approved===true&&request.allowedExecutions===1&&request.consumed===false);
  add('BRANCH',request.branch==='ays/backend-tenant-lab-v99-20260703'&&request.pullRequest===5);
  add('PREVIOUS_CLOSED',previous.status==='POLICIES_CANONICAL_CONTROLLED_WRITE_CLOSED'&&previous.executionResult?.run===30726870258&&previous.executionResult?.artifact===8826656778&&previous.executionResult?.artifactDigest==='sha256:170f450128cd228e3a8ce53c072fcd91cfb38ff4ad6a0dc5ae73116b84e8b945'&&previous.executionResult?.createdDocuments===4377&&previous.executionResult?.postVerificationPassed===true);
  ...
  add('SCOPE',request.scope?.sourceTotal===4837&&request.scope?.targetTotal===4822&&request.scope?.operationalSharedIds===4817&&request.scope?.exactCreatedParity===4377&&request.scope?.semanticProjectionParity===440&&request.scope?.legacyOnlyValidationHolds===20&&request.scope?.canonicalOnlySeedHolds===5&&request.scope?.verifyRelationships===true&&request.scope?.verifySchemas===true);
