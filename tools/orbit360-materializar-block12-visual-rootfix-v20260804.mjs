...
const validateVisual = source => {
  const checks = {
    viewportCapture: source.includes("fullPage: false"),
    animationsDisabled: source.includes("animations: 'disabled'"),
    reducedMotion: source.includes("reducedMotion: 'reduce'"),
    routeFailureCode: source.includes('VISUAL_SCREENSHOT_${route}'),
    pageCloseFinally: source.includes('if (page) await page.close().catch'),
    browserCloseFinally: source.includes('if (browser) await browser.close().catch'),
    schemaV2: source.includes("schemaVersion: 'orbit360-block12-cumulative-visual-v2'")
  };
  const failed = Object.entries(checks).filter(([, ok]) => !ok).map(([id]) => id);
  if (failed.length) throw new Error(`PIPELINE_MECHANISM_FAILURE:VISUAL_ROOTFIX_INCOMPLETE:${failed.join(',')}`);
  return checks;
};

const visualChecks = validateVisual(read(VISUAL));
...
if (!registryMatches) {
  ...
  registry.runtimeGateExtensions.push({ gateId: GATE, contractVersion: VERSION, lifecycle: 'tools/orbit360-validator-lifecycle-contract-block12-operational-runtime-lab-v20260804.json', engine: ENGINE, status: 'OPERATIONAL_RUNTIME_LAB_VISUAL_ROOTFIX_READY' });
  registryMatches = 1;
}
