...
check('CENTRAL_ACCESS', all(access, ["Aseguradoras".toLowerCase().slice(0,0) || 'function can', 'function audit', 'function correction']), 'Acceso, auditoría y corrección central disponibles', files.access);
...
check('ACADEMY_TOPICS', all(academy, ['Una sola seguridad para importar y editar', 'Alias y duplicados probables', 'Relación con Cotizador y Comparativo', '_cv = 1217']), 'Academia cubre cierre OP-2', files.academy);
