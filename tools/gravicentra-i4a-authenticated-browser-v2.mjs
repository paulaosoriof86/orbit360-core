...
async function basic(page,canonicalPolicy){
  const counts=await page.evaluate(()=>({clientes:Orbit.store.all('clientes').length,polizas:Orbit.store.all('polizas').length,aseguradoras:Orbit.store.all('aseguradoras').length,cobros:Orbit.store.all('cobros').length,recibosEsperados:Orbit.store.all('recibosEsperados').length,carteraPrimas:Orbit.store.all('carteraPrimas').length}));
  ...
  const c=await page.evaluate(()=>{const batch=Orbit.clientProjection?.withReadBatch?.(['clientes'],x=>x)||{clientes:[]};return {rawTotal:Orbit.store.all('clientes').length,projectionTotal:Array.isArray(batch.clientes)?batch.clientes.length:0,visibleRows:document.querySelectorAll('#host table.tbl tbody tr.clickable').length,paginationText:(document.querySelector('#host .c360-pagination')?.textContent||'').replace(/\s+/g,' ').trim()};});
  need(c.visibleRows>0,'CLIENTE360_ROWS_NOT_MATERIALIZED');need(c.projectionTotal===c.rawTotal&&c.projectionTotal>0,'CLIENTE360_PROJECTION_TOTAL_MISMATCH');need(c.paginationText.includes('de '+c.projectionTotal),'CLIENTE360_PAGINATION_TOTAL_MISMATCH');
  const cBeat=await beat(page);need(cBeat.maxMs<1000,'CLIENTE360_EVENT_LOOP_BLOCKED');

  const pMs=await gotoRoute(page,'#/polizas',()=>Orbit?.route?.key==='polizas'&&!!document.querySelector('#host .page')&&!!document.querySelector('#host table'));
  await page.waitForTimeout(80);
  const polizas=await page.evaluate(target=>{
    const row=(Orbit.store.all('polizas')||[]).find(p=>String(p?.numero||'').trim()===target)||null;
    ...
    return {runtime:row?{id:row.id,numero:row.numero,prima:row.prima,primaNeta:row.primaNeta,primaTotal:row.primaTotal,moneda:row.moneda}:null};
  },TARGET_POLICY);
  ...
  polizas.dom=dom;polizas.canonical=canonicalPolicy;
  need(polizas.runtime,'POLIZAS_TARGET_RUNTIME_NOT_FOUND');
  need(num(polizas.runtime.primaTotal)!=null&&num(canonicalPolicy?.primaTotal)!=null&&Math.abs(num(polizas.runtime.primaTotal)-num(canonicalPolicy.primaTotal))<0.005,'POLIZAS_RUNTIME_PREMIUM_DIVERGENCE_FROM_CANONICAL');
  need(num(polizas.runtime.primaNeta)!=null&&num(canonicalPolicy?.primaNeta)!=null&&Math.abs(num(polizas.runtime.primaNeta)-num(canonicalPolicy.primaNeta))<0.005,'POLIZAS_RUNTIME_NET_PREMIUM_DIVERGENCE_FROM_CANONICAL');
  need(dom.found,'POLIZAS_TARGET_ROW_NOT_MATERIALIZED');
  ...
  polizas.expectedPremiumText=expected;
  need(dom.premiumCell===expected,'POLIZAS_DOM_PREMIUM_DIVERGENCE_FROM_RUNTIME');
  const pBeat=await beat(page);need(pBeat.maxMs<1000,'POLIZAS_EVENT_LOOP_BLOCKED');

  await gotoRoute(page,'#/cobros',()=>Orbit?.route?.key==='cobros'&&!!document.querySelector('#host .page'),10000);
  await page.waitForTimeout(100);
  const cobros=await page.evaluate(()=>({cobros:Orbit.store.all('cobros').length,recibosEsperados:Orbit.store.all('recibosEsperados').length,carteraPrimas:Orbit.store.all('carteraPrimas').length,visibleRows:document.querySelectorAll('#host table.tbl tbody tr').length,text:(document.querySelector('#host')?.textContent||'').replace(/\s+/g,' ').trim().slice(0,600)}));
  return {counts,cliente360:{routeMs:cMs,...c,heartbeat:cBeat},polizas:{routeMs:pMs,...polizas,heartbeat:pBeat},cobrosReadModel:cobros};
}
...
const app=initializeApp({credential:cert(serviceAccount()),projectId:PROJECT},'gravicentra-i4a-readonly-v5'),auth=getAuth(app),db=getFirestore(app);
