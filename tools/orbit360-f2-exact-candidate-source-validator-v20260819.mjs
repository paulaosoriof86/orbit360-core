...
try{
  ...
  const r=readJson(REQUEST_PATH),ledger=readJson(LEDGER_PATH),authority=readJson(AUTHORITY_PATH),c=ledger.successorCandidate||{},closure=ledger.macro2Closure||{},runId=Number(process.env.GITHUB_RUN_ID||0);
  ...
  need(r.schemaVersion===REQUEST_SCHEMA&&r.status==='RUNTIME_ATTEMPT_ACCEPTED_PREFLIGHT_PENDING'&&r.approved===true&&r.allowedExecutions===0&&r.consumed===false&&r.authorizationFrozen===true&&r.replayAllowed===false&&r.historical===false&&r.runtimeAttemptAccepted===true&&Number(r.runtimeAttemptCount)===1,'VALIDATOR_STALE:F2_V3_REQUEST_NOT_ACCEPTED_ONE_SHOT');
  need(runId>0&&Number(r.runtimeRunId)===runId&&Number(ledger.authorizationBoundary?.runtimeRunId)===runId&&ledger.authorizationBoundary?.runtimeAttemptAccepted===true,'VALIDATOR_STALE:F2_V3_REQUEST_RUN_BINDING_INVALID');
  need(/^[a-f0-9]{64}$/.test(String(r.authorizationIdentityDigest||'')),'VALIDATOR_STALE:F2_V3_REQUEST_AUTH_IDENTITY_INVALID');
  ...
  need(Number(r.candidateArtifactId)===EXPECT.artifactId&&r.candidateSourceHead===EXPECT.sourceHead&&r.candidateArtifactDigest===EXPECT.artifactDigest,'VALIDATOR_STALE:F2_V3_REQUEST_CANONICAL_CANDIDATE_MISMATCH');
  const durableCertificationPass=/^orbit360-macro2-candidate-artifact-metadata-v\d+$/.test(String(cert.schemaVersion||''))&&cert.status==='CANDIDATE_ARTIFACT_PUBLISHED_SOURCE_ONLY'&&cert.sourcePublished===true&&Number(cert.artifactId)===EXPECT.artifactId&&cert.sourceHead===EXPECT.sourceHead&&cert.artifactDigest===EXPECT.artifactDigest&&cert.zipSha256===EXPECT.zipSha256&&cert.manifestSha256===EXPECT.manifestSha256&&Number(cert.fileCount)===EXPECT.fileCount&&Number(cert.deltaCount)===9&&Number(cert.unchangedFileCount)===185&&cert.runtimeExecuted===false&&cert.browserExecuted===false&&cert.secretAccess===false&&cert.firestoreRead===false&&Number(cert.writes)===0&&cert.deployExecuted===false&&cert.productionTouched===false&&cert.containsPII===false&&cert.containsSecrets===false&&closure.status==='TRANSVERSAL_SOURCE_ACCEPTANCE_PASS'&&closure.evidencePath===CERT_PATH&&Number(closure.runId)===Number(cert.runId)&&Number(closure.candidateArtifactId)===EXPECT.artifactId&&closure.sourceHead===EXPECT.sourceHead&&closure.artifactDigest===EXPECT.artifactDigest&&Number(closure.checksPassed)===107&&Number(closure.deltaCount)===9&&Number(closure.fileCount)===194&&Number(closure.unchangedFileCount)===185;
