...
const CERT_PATH='orbit360-platform/runtime-gate-crm-v20260716/f2-successor-candidate-sourceonly-v20260820.json';
...
try{
  ...
  for(const p of [LEDGER_PATH,CERT_PATH])need(fs.existsSync(path.join(ROOT,p)),`PIPELINE_MECHANISM_FAILURE:F2_CANONICAL_IDENTITY_SOURCE_MISSING:${p}`);
  const r=readJson(REQUEST_PATH),ledger=readJson(LEDGER_PATH),cert=readJson(CERT_PATH),c=ledger.successorCandidate||{};
  ...
  need(cert.ok===true&&cert.status==='F2_SUCCESSOR_SOURCEONLY_CANDIDATE_CERTIFIED','DATA_CONTRACT_FAILURE:F2_SUCCESSOR_CERTIFICATION_MISSING');
  ...
  persist({schemaVersion:'orbit360-f2-exact-candidate-source-validation-v3',ok:true,status:'F2_EXACT_CANDIDATE_SOURCE_VALIDATION_PASS',classification:'PASS',candidateArtifactId:EXPECT.artifactId,candidateSourceHead:EXPECT.sourceHead,candidateArtifactDigest:EXPECT.artifactDigest,candidateZipSha256:EXPECT.zipSha256,candidateManifestSha256:EXPECT.manifestSha256,candidateManifestStatus:manifestStatus,candidateFileCount:EXPECT.fileCount,identityDerivedFromCanonicalCertification:true,v2RequestContainsNoDuplicatedManifestIdentity:true,fullRehashPass:true,entrypointRefsPass:true,readOnlyStoreGuardPass:true,inicioFiniteRootfixPass:true,routerReadinessRootfixPass:true,storeGetRootfixPass:true,browserExecuted:false,runtimeExecuted:false,secretAccess:false,dataAccess:false,firestoreWrites:0,authWrites:0,operationalWrites:0,packageRebuilt:false,deployExecuted:false,publicationExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false});
}catch(error){persist({schemaVersion:'orbit360-f2-exact-candidate-source-validation-v3',ok:false,status:'F2_EXACT_CANDIDATE_SOURCE_VALIDATION_FAIL',classification:String(error?.message||error).split(':')[0]||'PIPELINE_MECHANISM_FAILURE',error:String(error?.message||error).slice(0,700),candidateArtifactId:EXPECT.artifactId,browserExecuted:false,runtimeExecuted:false,secretAccess:false,dataAccess:false,firestoreWrites:0,authWrites:0,operationalWrites:0,packageRebuilt:false,deployExecuted:false,publicationExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false});process.exitCode=41;}
