...
const LEDGER_PATH='orbit360-platform/docs/orbit360-continuity-ledger-v20260820.json';
...
try{
  ...
  for(const p of [LEDGER_PATH,AUTHORITY_PATH])need(fs.existsSync(path.join(ROOT,p)),`PIPELINE_MECHANISM_FAILURE:F2_CANONICAL_IDENTITY_SOURCE_MISSING:${p}`);
  const r=readJson(REQUEST_PATH),ledger=readJson(LEDGER_PATH),authority=readJson(AUTHORITY_PATH),c=ledger.successorCandidate||{};
  const CERT_PATH=String(authority.candidateCertificationEvidence||'').trim();
  need(CERT_PATH&&fs.existsSync(path.join(ROOT,CERT_PATH)),'VALIDATOR_STALE:F2_CANONICAL_CERTIFICATION_POINTER_MISSING');
  const cert=readJson(CERT_PATH);
  need(r.requestVersion==='F2_PRODUCTIVE_ACCEPTANCE_RUNTIME_BROWSER_READONLY_V2'&&r.status==='MATERIALIZED_SOURCE_ONLY_AWAITING_PREFLIGHT'&&r.allowedExecutions===1&&r.consumed===false&&r.replayAllowed===false&&r.historical===false,'VALIDATOR_STALE:F2_V2_REQUEST_NOT_ACTIVE');
  ...
  need(EXPECT.artifactId>0&&/^[0-9a-f]{40}$/.test(EXPECT.sourceHead)&&/^[0-9a-f]{64}$/.test(EXPECT.artifactDigest)&&/^[0-9a-f]{64}$/.test(EXPECT.zipSha256)&&/^[0-9a-f]{64}$/.test(EXPECT.manifestSha256)&&Number.isInteger(EXPECT.fileCount)&&EXPECT.fileCount>0,'VALIDATOR_STALE:F2_CANONICAL_CANDIDATE_IDENTITY_INVALID');
  need(Number(authority.candidate?.artifactId)===EXPECT.artifactId&&authority.candidate?.sourceHead===EXPECT.sourceHead&&authority.candidate?.artifactDigest===EXPECT.artifactDigest,'VALIDATOR_STALE:F2_AUTHORITY_LEDGER_CANDIDATE_DRIFT');
  need(Number(r.candidateArtifactId)===EXPECT.artifactId&&r.candidateSourceHead===EXPECT.sourceHead&&r.candidateArtifactDigest===EXPECT.artifactDigest,'VALIDATOR_STALE:F2_V2_REQUEST_CANONICAL_CANDIDATE_MISMATCH');
  need(Number(cert.candidateArtifactId)===EXPECT.artifactId&&cert.candidateSourceHead===EXPECT.sourceHead&&cert.artifactDigest===EXPECT.artifactDigest&&cert.candidateZipSha256===EXPECT.zipSha256&&cert.candidateManifestSha256===EXPECT.manifestSha256&&Number(cert.fileCount)===EXPECT.fileCount,'DATA_CONTRACT_FAILURE:F2_CERTIFIED_IDENTITY_DRIFT');
  ...
  persist({schemaVersion:'orbit360-f2-exact-candidate-source-validation-v4',ok:true,status:'F2_EXACT_CANDIDATE_SOURCE_VALIDATION_PASS',classification:'PASS',candidateArtifactId:EXPECT.artifactId,candidateSourceHead:EXPECT.sourceHead,candidateArtifactDigest:EXPECT.artifactDigest,candidateZipSha256:EXPECT.zipSha256,candidateManifestSha256:EXPECT.manifestSha256,candidateCertificationEvidence:CERT_PATH,candidateManifestStatus:manifestStatus,candidateFileCount:EXPECT.fileCount,identityDerivedFromCanonicalCertification:true,certificationPathDerivedFromCanonicalAuthority:true,v2RequestContainsNoDuplicatedManifestIdentity:true,fullRehashPass:true,entrypointRefsPass:true,readOnlyStoreGuardPass:true,inicioFiniteRootfixPass:true,routerReadinessRootfixPass:true,storeGetRootfixPass:true,browserExecuted:false,runtimeExecuted:false,secretAccess:false,dataAccess:false,firestoreWrites:0,authWrites:0,operationalWrites:0,packageRebuilt:false,deployExecuted:false,publicationExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false});
}catch(error){persist({schemaVersion:'orbit360-f2-exact-candidate-source-validation-v4',ok:false,status:'F2_EXACT_CANDIDATE_SOURCE_VALIDATION_FAIL',classification:String(error?.message||error).split(':')[0]||'PIPELINE_MECHANISM_FAILURE',error:String(error?.message||error).slice(0,700),candidateArtifactId:EXPECT.artifactId,browserExecuted:false,runtimeExecuted:false,secretAccess:false,dataAccess:false,firestoreWrites:0,authWrites:0,operationalWrites:0,packageRebuilt:false,deployExecuted:false,publicationExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false});process.exitCode=41;}
