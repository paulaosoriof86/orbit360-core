...
const REQUEST='tools/orbit360-m6-recovery-6114-request-v20260730.json';
...
try{
  ...
  add('SMOKE_VALIDATOR',smoke.includes("contractVersion:'6.1.14'")&&smoke.includes("validatorRevision:'20260730.6'")&&smoke.includes('scrollIntoView')&&smoke.includes('centerInsideViewport')&&smoke.includes('hitDescriptor')&&smoke.includes('verifiedSemanticCardClick')&&smoke.includes("report.checks.insurerSemanticClick=true"));
  ...
  const out={schemaVersion:'orbit360-gate-contract-preflight-m6-recovery-6114-v1',gateId:GATE,contractVersion:VERSION,executionPhase:'M6_PRODUCT_GO_LIVE_RECOVERY_EXECUTION',status:failed.length?'VALIDATOR_STALE':'GO_GATE_CONTRACT',classification:failed.length?'VALIDATOR_STALE':null,total:checks.length,passed:checks.length-failed.length,failed:failed.length,failedCheckIds:failed.map(c=>c.id),checks,executionAuthorized:failed.length===0,allowedExecutions:failed.length?0:1,staticValidatorRemediationRun:STATIC_RUN,previousRecoveryRun:30549026522,previousRecoveryArtifact:8762009928,validatorRootCauseClosed:true,canonicalMigratedCollections:['clientes','aseguradoras'],logicalCountryField:'country',physicalCountryField:'pais',waitAllActiveCollections:true,semanticInsurerCardClick:true,viewportHitTest:true,smokeValidatorRevision:'20260730.6',boundedHostingReadiness:true,storageDeferredFailClosed:true,createStorageBucket:false,storageRulesDeploy:false,packageHash:sha(JSON.stringify(packageRows)),capabilityProfile:profile,dataAccess:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,operationalWrites:0,runtimeExecuted:false,browserExecuted:false,rulesApplied:false,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false};
  write(out);console.log(JSON.stringify(out,null,2));if(failed.length)process.exit(41);
}catch(error){write({schemaVersion:'orbit360-gate-contract-preflight-m6-recovery-6114-v1',gateId:GATE,contractVersion:VERSION,status:'VALIDATOR_STALE',classification:'VALIDATOR_STALE',failed:1,failedCheckIds:['M6_RECOVERY_6114_PREFLIGHT_EXCEPTION'],error:String(error&&error.message||error).slice(0,500),executionAuthorized:false,allowedExecutions:0,dataAccess:false,secretAccess:false,firestoreRead:false,firestoreWrites:0,operationalWrites:0,runtimeExecuted:false,browserExecuted:false,rulesApplied:false,deployExecuted:false,productionTouched:false,containsPII:false,containsSecrets:false});process.exit(41);}
