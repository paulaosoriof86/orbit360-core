...
const GATE_ID='block4-data-reconciliation-readonly-v20260725',CONTRACT_VERSION='4.2.0';
...
function country(row){return text(first(row,['country','pais','país'])).toUpperCase();}
...
async function main(){let app;const base={schemaVersion:'orbit360-m4-data-reconciliation-v1',gateId:GATE_ID,contractVersion:CONTRACT_VERSION,projectId:PROJECT_ID,tenantId:TENANT_ID,readOnly:true,writeAuthorized:false,writeExecuted:false,configurationWrites:0,membershipWrites:0,clientWrites:0,insurerWrites:0,auditWrites:0,rulesChanged:false,hostingDeploy:false,functionsDeploy:false,imports:false,policies:false,mergeMain:false};
 try{
  ...
  const freeze=JSON.parse(fs.readFileSync(FREEZE,'utf8'));if(freeze.status!=='M4_DURABLE_WRITER_DRYRUN_COMPLETED_DATA_VALIDATION_REQUIRED'||freeze.dryRunClosure&&freeze.dryRunClosure.requiresValidation!==65)throw new Error('DATA_CONTRACT_FAILURE:DRYRUN_FREEZE_INVALID');
  ...
  const [sourceClients,targetClients,sourceInsurers,targetInsurers]=await Promise.all([legacy.collection('clientes').get(),tenant.collection('data').doc('clientes').collection('items').get(),legacy.collection('aseguradoras').get(),tenant.collection('data').doc('aseguradoras').collection('items').get()]);
  const missing=sourceClients.docs.map(d=>plain(d.data()||{})).filter(r=>!currency(r));let resolvedGTQ=0,resolvedCOP=0,unresolved=0;const unresolvedCountries={};
  missing.forEach(row=>{const c=country(row);if(c==='GT')resolvedGTQ++;else if(c==='CO')resolvedCOP++;else{unresolved++;unresolvedCountries[c||'MISSING']=(unresolvedCountries[c||'MISSING']||0)+1;}});
  ...
  const payload={...base,remoteReadConfirmed:true,sourceCounts:{clientes:sourceClients.size,aseguradoras:sourceInsurers.size},targetCounts:{clientes:targetClients.size,aseguradoras:targetInsurers.size},currencyResolution:{missingCurrency:missing.length,resolvedGTQ,resolvedCOP,unresolved,unresolvedCountries,countryCurrencyMap:{GT:'GTQ',CO:'COP'},proposalDigest:hash({resolvedGTQ,resolvedCOP,unresolved,unresolvedCountries})},targetOnlyResolution:{total:items.length,clientTotal:clientOnly.length,insurerTotal:insurerOnly.length,recommendations,items,digest:hash(items)},secretValueCount:0};
  const checked=globalThis.Orbit.m4DataReconciliationP0.build(payload);write({...payload,...checked,ok:checked.ok,status:checked.status,classification:checked.ok?null:'DATA_CONTRACT_FAILURE'});if(!checked.ok)process.exitCode=41;
 }catch(error){const msg=text(error&&error.message||error),classification=msg.startsWith('PIPELINE_MECHANISM_FAILURE')?'PIPELINE_MECHANISM_FAILURE':'DATA_CONTRACT_FAILURE';write({...base,ok:false,status:classification,classification,error:msg.replace(/[A-Za-z0-9_-]{20,}/g,'[redacted]').slice(0,240),remoteReadConfirmed:false});process.exitCode=41;}finally{if(app)await deleteApp(app).catch(()=>{});}}
await main();
