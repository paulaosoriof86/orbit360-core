...
check('VALIDATOR_ROOT_PLATFORM', fs.existsSync(path.join(ROOT, 'modules/aseguradoras.js')), ROOT);
...
check('NO_RANDOM_MASKED_ACCOUNT', !has(mod, "numero: '****' + Math.floor"));
...
check('PASSWORD_NATIVE_EPHEMERAL', has(mod, 'data-ppass type="password"') && has(mod, 'st.credentialDrafts') && !/password\s*:\s*portal\./.test(mod));
check('SECURE_PROVIDER_MUTATION', has(mod, 'Orbit.secureImport.importInsurerDirectory') && has(mod, 'persistSecureCredentialChanges'));
check('SECURE_SOURCE_HASH_SHA256', has(mod, "window.crypto.subtle.digest('SHA-256'") && has(mod, 'const sourceHash = await secureSourceHash'));
...
check('NEW_INSURER_DEFAULT_ACTIVE', has(mod, 'vinculada: true, contactos: [], cuentas: [], portales: []'));
...
check('BOOTSTRAP_COMMENT_CLOSED', /\/\* Preflight 1\.0\.40:[^\n]+\*\//.test(bootstrap));
check('NO_OPERATIONAL_VALUES_ADDED_TO_OWNER', has(editOwner, 'operationalValuesInCode: false'));

const failed = checks.filter(item => !item.ok);
const report = {
  schemaVersion: 'orbit360-m1-dynamic-directory-contract-v3',
  contractVersion: '1.0.40',
  status: failed.length ? 'DYNAMIC_DIRECTORY_CONTRACT_V3_FAILED' : 'DYNAMIC_DIRECTORY_CONTRACT_V3_PASS',
  ok: failed.length === 0,
  total: checks.length,
  ...
  checks,
  rootCauseCorrection: {
    unterminatedBootstrapCommentFixed: true,
    validatorRootPlatformFixed: true,
    previousFailedPatchRunId: 30009334105,
    previousDiagnosticRunId: 30009737855,
    previousValidatorRootFailureRunId: 30010358983
  },
  capabilities: { secrets:false, firestoreRead:false, writes:false, runtime:false, browser:false, deploy:false, repositoryWrite:false, functionsDeploy:false, rulesDeploy:false, production:false },
  containsPII:false,
  containsSecrets:false
};
const out = path.join(ROOT, 'runtime-gate-crm-v20260716/dynamic-directory-contract-v3-sanitized.json');
